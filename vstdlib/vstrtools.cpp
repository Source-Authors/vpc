...
class DummyInitL10N {
 public:
  DummyInitL10N() {
    ...
    if (ret != CELL_OK) {
      Warning(
          "Cannot initialize l10n, unicode services will not work. Error %d\n",
          ret);
    }
  }

  ~DummyInitL10N() { cellSysmoduleUnloadModule(CELL_SYSMODULE_L10N); }
} s_dummyInitL10N;
#endif
...
int V_UTF8ToUnicode(const char *pUTF8, wchar_t *pwchDest,
  ...
  L10nResult result = UTF8stoUCS2s((const uint8 *)pUTF8, &cchSrc,
                                   (uint16 *)pwchDest, &cchResult);
...
int V_UnicodeToUTF8(const wchar_t *pUnicode, char *pUTF8,
  ...
  int cchResult = WideCharToMultiByte(CP_UTF8, 0, pUnicode, -1, pUTF8,
                                      cubDestSizeInBytes, NULL, NULL);
#elif defined(_PS3)
  size_t cchResult = cubDestSizeInBytes, cchSrc = V_wcslen(pUnicode) + 1;
  L10nResult result = UCS2stoUTF8s((const uint16 *)pUnicode, &cchSrc,
                                   (uint8 *)pUTF8, &cchResult);
  Assert(result == ConversionOK);
#elif POSIX
  int cchResult = 0;
  if (pUnicode && pUTF8) {
    ...
    int cchResult = -1;
    size_t nLenUnicde = (wcslen(pUnicode) + 1) *
                        sizeof(wchar_t);  // 4 bytes per wchar vs. 1 byte for
                                          // utf8 for simple english
...
//-----------------------------------------------------------------------------
int V_UnicodeToUCS2(const wchar_t *pUnicode, int cubSrcInBytes, char *pUCS2,
                    int cubDestSizeInBytes) {
  // TODO: MACMERGE: Figure out how to convert from 2-byte Win32 wchars to
  // platform wchar_t type that can be 4 bytes
#if defined(_WIN32) || defined(_PS3)
...
//-----------------------------------------------------------------------------
// Purpose: Converts a ucs-2 (windows wchar_t) string into a UTF8 (standard)
// string
//-----------------------------------------------------------------------------
VSTRTOOLS_INTERFACE int V_UCS2ToUTF8(const ucs2 *pUCS2, char *pUTF8,
  ...
  AssertValidStringPtr(pUTF8, cubDestSizeInBytes);
  AssertValidReadPtr(pUCS2);
  Assert(cubDestSizeInBytes >=
         1);  // must have at least 1 byte to write the terminator character

  pUTF8[0] = '\0';
#ifdef _WIN32
  // under win32 wchar_t == ucs2, sigh
  int cchResult = WideCharToMultiByte(CP_UTF8, 0, pUCS2, -1, pUTF8,
                                      cubDestSizeInBytes, NULL, NULL);
#elif defined(_PS3)
  size_t cchResult = cubDestSizeInBytes, cchSrc = V_wcslen(pUCS2) + 1;
  L10nResult result =
      UCS2stoUTF8s((const uint16 *)pUCS2, &cchSrc, (uint8 *)pUTF8, &cchResult);
  Assert(result == ConversionOK);
#elif defined(POSIX)
...
//-----------------------------------------------------------------------------
VSTRTOOLS_INTERFACE int V_UTF8ToUCS2(const char *pUTF8,
                                     [[maybe_unused]] int cubSrcInBytes,
                                     ucs2 *pUCS2, int cubDestSizeInBytes) {
  AssertValidStringPtr(pUTF8, cubDestSizeInBytes);
  AssertValidReadPtr(pUCS2);
...
#ifdef _WIN32
  // under win32 wchar_t == ucs2, sigh
  int cchResult = MultiByteToWideChar(CP_UTF8, 0, pUTF8, -1, pUCS2,
                                      cubDestSizeInBytes / sizeof(wchar_t));
#elif defined(_PS3)
  size_t cchResult = cubDestSizeInBytes / sizeof(uint16),
         cchSrc = cubSrcInBytes;
  L10nResult result =
      UTF8stoUCS2s((const uint8 *)pUTF8, &cchSrc, (uint16 *)pUCS2, &cchResult);
  Assert(result == ConversionOK);
  cchResult *= sizeof(uint16);
#elif defined(POSIX)
...
//-----------------------------------------------------------------------------
// Purpose: copies at most nMaxBytes of the UTF-8 input data into the
// destination, ensuring that a trailing multi-byte sequence isn't truncated.
//-----------------------------------------------------------------------------
VSTRTOOLS_INTERFACE void *V_UTF8_strncpy(char *pDest, const char *pSrc,
                                         size_t nMaxBytes) {
  ...
  if ((pDest[end] & 0xC0) == 0xC0) {
    for (int i = 6; i > 1; --i) {
      if ((char)(pDest[end] >> i) & 0x1) ++nBytesExpected;
    }
  }

  if (nBytesExpected != nBytesSeen) pDest[end] = 0;
