...
class CCvar : public CBaseAppSystem<ICvar> {
 public:
  ...
  virtual intp GetConsoleDisplayFuncCount() const;
 ...
 private:
  // Standard console commands -- DO NOT PLACE ANY HIGHER THAN HERE BECAUSE
  // THESE MUST BE THE FIRST TO DESTRUCT
  CON_COMMAND_MEMBER_F(
      CCvar, "find", Find,
      "Find concommands with the specified string in their name/help text.", 0)
#ifdef _DEBUG
  CON_COMMAND_MEMBER_F(CCvar, "ccvar_hash_report", HashReport,
                       "report info on bucket distribution of internal hash.",
                       0)
#endif
};
...
void CCvar::RegisterConCommand(ConCommandBase *variable) {
  ...
  if (pOther) {
    if (variable->IsCommand() || pOther->IsCommand()) {
      ...
      ConVar *pChildVar =
          const_cast<ConVar *>(static_cast<const ConVar *>(variable));
      ConVar *pParentVar =
          const_cast<ConVar *>(static_cast<const ConVar *>(pOther));
      ...
      if (s_pCVarQuery->AreConVarsLinkable(pChildVar, pParentVar)) {
        ...
        if (pChildVar->m_pszDefaultValue && pParentVar->m_pszDefaultValue &&
          ...
          if (V_stricmp(pChildVar->m_pszDefaultValue,
        ...
        pParentVar->m_nFlags |=
            pChildVar->m_nFlags &
            (FCVAR_MATERIAL_THREAD_MASK | FCVAR_ACCESSIBLE_FROM_THREADS);
        ...
        if (pChildVar->m_pszHelpString &&
            V_strlen(pChildVar->m_pszHelpString) != 0) {
          if (pParentVar->m_pszHelpString &&
              V_strlen(pParentVar->m_pszHelpString) != 0) {
            if (V_stricmp(pParentVar->m_pszHelpString,
        ...
        static int const nFlags[] = {FCVAR_CHEAT, FCVAR_REPLICATED,
                                     FCVAR_DONTRECORD, FCVAR_ARCHIVE,
        ...
        for (size_t k = 0; k < V_ARRAYSIZE(nFlags); ++k) {
          if ((pChildVar->m_nFlags & nFlags[k]) !=
            ...
            Warning(
                "Convar %s has conflicting %s flags (child: %s%s, parent: "
                "%s%s, parent wins)\n",
                variable->GetName(), szFlags[k],
                (pChildVar->m_nFlags & nFlags[k]) ? "has " : "no ", szFlags[k],
                (pParentVar->m_nFlags & nFlags[k]) ? "has " : "no ",
                szFlags[k]);
          }
        }
      }
  ...
  AssertMsg1(FindCommandBase(variable->GetName()) == NULL,
             "Console command %s added twice!", variable->GetName());
  m_CommandHash.Insert(variable);
}

void CCvar::AddSplitScreenConVars() {
  if (m_nMaxSplitScreenSlots == 1) return;

  for (ConCommandBase *pCommand = m_pConCommandList; pCommand;
       pCommand = pCommand->m_pNext) {
    if (pCommand->IsCommand()) continue;
    ...
    for (int i = 1; i < m_nMaxSplitScreenSlots; ++i) {
      ...
      if (info.m_Vars[i - 1].m_pVar) continue;

      // start at name2, etc.
      info.m_Vars[i - 1].m_VarName = CFmtStr("%s%d", pConVar->GetName(), i + 1);

      CSplitScreenAddedConVar *pVar = new CSplitScreenAddedConVar(
          i, info.m_Vars[i - 1].m_VarName.Get(), pConVar);
...
const ConCommand *CCvar::FindCommand(const char *pCommandName) const {
  ...
  if (!var || !var->IsCommand()) return NULL;
...
const char *CCvar::GetCommandLineValue(const char *pVariableName) {
  intp nLen = V_strlen(pVariableName);
...
void CCvar::CallGlobalChangeCallbacks(ConVar *var, const char *pOldString,
                                      float flOldValue) {
  intp nCallbackCount = m_GlobalChangeCallbacks.Count();
  for (intp i = 0; i < nCallbackCount; ++i) {
...
void CCvar::RevertFlaggedConVars(int nFlag) {
  for (CConCommandHash::CCommandHashIterator_t i = m_CommandHash.First();
       m_CommandHash.IsValidIterator(i); i = m_CommandHash.Next(i)) {
...
int CCvar::ProcessQueuedMaterialThreadConVarSets() {
  ...
  int nUpdateFlags = 0;
  intp nCount = m_QueuedConVarSets.Count();
...
Right now however I just want to get the thing working with as little
modification as possible so I can fix the vxconsole windows app itself and
hopefully get bidirectional TTY to our game. So, instead of the proper solution,
I'm just duct-taping everything together by simply passing a pointer to the
engine symbol this function needs whenever I call it. Blech. I'll fix it later.
-egr 4/29/10. (is it later than September 2010? go call egr and make fun of
him.)
*/
...
static bool ConVarSortFunc(ConCommandBase *const &lhs,
                           ConCommandBase *const &rhs) {
...
int CCvar::GetMaxSplitScreenSlots() const { return m_nMaxSplitScreenSlots; }

//-----------------------------------------------------------------------------
// Console command hash data structure
//-----------------------------------------------------------------------------
CConCommandHash::CConCommandHash() { Purge(true); }

CConCommandHash::~CConCommandHash() { Purge(false); }
...
CConCommandHash::CCommandHashHandle_t CConCommandHash::Find(
    const char *name, HashKey_t hashkey) const {
  ...
  for (datapool_t::IndexLocalType_t iElement = m_aBuckets[iBucket];
    ...
    if (element.m_uiKey == hashkey &&  // if hashes of strings match,
        V_stricmp(name, element.m_Data->GetName()) ==
            0)  // then test the actual strings
    {
      return iElement;
    }
  }
...
//-----------------------------------------------------------------------------
CConCommandHash::CCommandHashHandle_t CConCommandHash::Find(
    const ConCommandBase *cmd) const {
...
void CConCommandHash::Report(void) {
  ...
  for (int iBucket = 0; iBucket < kNUM_BUCKETS; ++iBucket) {
    ...
    CCommandHashHandle_t iElement =
        m_aBuckets[iBucket];  // get the head of the bucket
