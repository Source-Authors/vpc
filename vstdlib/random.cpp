...
int CUniformRandomStream::GenerateRandomNumber() {
  ...
  // We're seeing some strange memory corruption in the contents of
  // s_pUniformStream. Perhaps it's being caused by something writing past the
  // end of this array? Bounds-check in release to see if that's the case.
  if (j >= NTAB || j < 0) {
    DebuggerBreakIfDebugging();
    Warning(
        "CUniformRandomStream had an array overrun: tried to write to element "
        "%d of 0..31. Contact Tom or Elan.\n",
        j);
    j = (j % NTAB) & 0x7fffffff;
  }
...
float CUniformRandomStream::RandomFloatExp(float flMinVal, float flMaxVal,
                                           float flExponent) {
  // float in [0,1)
  float fl = AM * GenerateRandomNumber();
  if (fl > RNMX) {
...
float CGaussianRandomStream::RandomFloat(float flMean, float flStdDev) {
  AUTO_LOCK(m_mutex);
  IUniformRandomStream *pUniformStream =
      m_pUniformStream ? m_pUniformStream : s_pUniformStream;
  float fac, rsq, v1, v2;
