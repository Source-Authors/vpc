...
#define MEM_4BYTES_FROM_0_AND_3BYTES(x03bytes) \
  (((uint32)(x03bytes)) & 0x00FFFFFF)
#endif
...
class CKeyValuesSystem : public IKeyValuesSystem {
 public:
  ...
  // e.g.: SetKeyValuesExpressionSymbol( "LOWVIOLENCE", true ) - enables
  // [$LOWVIOLENCE]
  ...
  // symbol table access from code with case-preserving requirements (used for
  // key names)
  virtual HKeySymbol GetSymbolForStringCaseSensitive(
      HKeySymbol &hCaseInsensitiveSymbol, const char *name,
      bool bCreate = true);
...
CKeyValuesSystem::CKeyValuesSystem()
    : m_HashItemMemPool(sizeof(hash_item_t), 64, CUtlMemoryPool::GROW_FAST,
  ...
  m_Strings.Init("CKeyValuesSystem::m_Strings", 4 * 1024 * 1024, 64 * 1024, 0,
                 4);
...
void *CKeyValuesSystem::AllocKeyValuesMemory(size_t size) {
  ...
  if (!m_pMemPool) {
    m_pMemPool =
        new CUtlMemoryPool(m_iMaxKeyValuesSize, 1024, CUtlMemoryPool::GROW_FAST,
  ...
  while (1) {
    ...
    if (iResult > 0) {
      ...
      if (!bCreate) {
        ...
        // NOTE: this is not correct value, but it cannot be used to create a
...

      {
        ...
        *pnCaseResolveIndex = MEM_4BYTES_AS_0_AND_3BYTES(
            nNewAlternativeStringIndex);  // link previous spelling entry to the
                                          // new entry
...
// [$LOWVIOLENCE]
//-----------------------------------------------------------------------------
void CKeyValuesSystem::SetKeyValuesExpressionSymbol(const char *name,
                                                    bool bValue) {
  if (!name) return;
...
bool CKeyValuesSystem::GetKeyValuesExpressionSymbol(const char *name) {
  ...
  if (hSym != -1) {
    AUTO_LOCK(m_mutex);
    CUtlMap<HKeySymbol, bool>::IndexType_t idx =
        m_KvConditionalSymbolTable.Find(hSym);
  ...
  if (!V_stricmp(name, "ENGLISH") || !V_stricmp(name, "JAPANESE") ||
      !V_stricmp(name, "GERMAN") || !V_stricmp(name, "FRENCH") ||
      !V_stricmp(name, "SPANISH") || !V_stricmp(name, "ITALIAN") ||
      !V_stricmp(name, "KOREAN") || !V_stricmp(name, "TCHINESE") ||
      !V_stricmp(name, "PORTUGUESE") || !V_stricmp(name, "SCHINESE") ||
      !V_stricmp(name, "POLISH") || !V_stricmp(name, "RUSSIAN") ||
  ...
  if (!V_strnicmp(name, "CVAR_", 5)) {
    ...
    if (cvRef.IsValid()) return cvRef.GetBool();
  }
