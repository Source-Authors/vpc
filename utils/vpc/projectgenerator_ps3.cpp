...
CProjectGenerator_PS3::CProjectGenerator_PS3() {
  ...
  m_pVCProjGenerator->SetupGeneratorDefinition(this, "ps3.def",
                                               s_PS3PropertyNames);
}
...
bool CProjectGenerator_PS3::WriteFolder(CProjectFolder *pFolder) {
  ...
  // String() returns temporary object, so save in var to prevent stale memory
  // usage.
...
bool CProjectGenerator_PS3::WritePreBuildEventTool(
    CPreBuildEventTool *pPreBuildEventTool) {
  ...
  for (intp i = 0;
    ...
    switch (pPreBuildEventTool->m_PropertyStates.m_Properties[i]
      ...
      case PS3_PREBUILDEVENT_CommandLine: {
        ...
        // memory usage.
        CUtlString cl = m_XMLWriter.FixupXMLString(
            pPreBuildEventTool->m_PropertyStates.m_Properties[i]
                .m_StringValue.Get());
      ...
      case PS3_PREBUILDEVENT_ExcludedFromBuild:
        m_XMLWriter.Write(
            CFmtStrMax("ExcludedFromBuild=\"%s\"",
                       BoolStringToTrueFalseString(
                           pPreBuildEventTool->m_PropertyStates.m_Properties[i]
                               .m_StringValue.Get())));
        break;
    }
...
bool CProjectGenerator_PS3::WriteCustomBuildTool(
    CCustomBuildTool *pCustomBuildTool) {
  if (!pCustomBuildTool) {
    // not an error, some tools n/a for aconfig
    return true;
  }
  ...
  for (intp i = 0; i < pCustomBuildTool->m_PropertyStates.m_Properties.Count();
    ...
    switch (pCustomBuildTool->m_PropertyStates.m_Properties[i]
      ...
      case PS3_CUSTOMBUILDSTEP_CommandLine: {
        ...
        // memory usage.
        CUtlString cl = m_XMLWriter.FixupXMLString(
            pCustomBuildTool->m_PropertyStates.m_Properties[i]
                .m_StringValue.Get());
        m_XMLWriter.Write(CFmtStrMax("CommandLine=\"%s\"", cl.String()));
      } break;
      ...
      case PS3_CUSTOMBUILDSTEP_Outputs:
        m_XMLWriter.Write(CFmtStrMax(
            "Outputs=\"%s\"", pCustomBuildTool->m_PropertyStates.m_Properties[i]
                                  .m_StringValue.Get()));
        break;

      case PS3_CUSTOMBUILDSTEP_AdditionalDependencies:
        m_XMLWriter.Write(
            CFmtStrMax("AdditionalDependencies=\"%s\"",
                       pCustomBuildTool->m_PropertyStates.m_Properties[i]
                           .m_StringValue.Get()));
        break;
    }
...
bool CProjectGenerator_PS3::WriteSNCCompilerTool(CCompilerTool *pCompilerTool) {
  ...
  for (intp i = 0; i < pCompilerTool->m_PropertyStates.m_Properties.Count();
    ...
    switch (pCompilerTool->m_PropertyStates.m_Properties[i]
                .m_pToolProperty->m_nPropertyId) {
      case PS3_SNCCOMPILER_AdditionalIncludeDirectories:
        m_XMLWriter.Write(
            CFmtStrMax("AdditionalIncludeDirectories=\"%s\"",
                       pCompilerTool->m_PropertyStates.m_Properties[i]
                           .m_StringValue.Get()));
        break;

      case PS3_SNCCOMPILER_PreprocessorDefinitions:
        m_XMLWriter.Write(
            CFmtStrMax("PreprocessorDefinitions=\"%s\"",
                       pCompilerTool->m_PropertyStates.m_Properties[i]
                           .m_StringValue.Get()));
        break;
      ...
      case PS3_SNCCOMPILER_ForceIncludes:
        m_XMLWriter.Write(
            CFmtStrMax("ForcedIncludeFiles=\"%s\"",
                       pCompilerTool->m_PropertyStates.m_Properties[i]
                           .m_StringValue.Get()));
        break;
      ...
      case PS3_SNCCOMPILER_DisableSpecificWarnings:
        m_XMLWriter.Write(
            CFmtStrMax("DisableSpecificWarnings=\"%s\"",
                       pCompilerTool->m_PropertyStates.m_Properties[i]
                           .m_StringValue.Get()));
        break;
      ...
      case PS3_SNCCOMPILER_ObjectFileName:
        m_XMLWriter.Write(CFmtStrMax(
            "ObjectFile=\"%s\"", pCompilerTool->m_PropertyStates.m_Properties[i]
                                     .m_StringValue.Get()));
        break;
      ...
      case PS3_SNCCOMPILER_CreateUsePrecompiledHeader:
        if (nOrdinalValue == 1) {
          additionalOptions +=
              CFmtStrMax("--create_pch=&quot;%s&quot; ",
                         pCompilerTool->m_PropertyStates
                             .GetProperty(PS3_SNCCOMPILER_PrecompiledHeaderFile)
                             ->m_StringValue.String());
        } else if (nOrdinalValue == 2) {
          additionalOptions += "--pch --pch_dir=&quot;$(IntDir)&quot; ";
        } else if (nOrdinalValue == 3) {
          additionalOptions +=
              CFmtStrMax("--use_pch=&quot;%s&quot; ",
                         pCompilerTool->m_PropertyStates
                             .GetProperty(PS3_SNCCOMPILER_PrecompiledHeaderFile)
                             ->m_StringValue.String());
        }
      ...
      case PS3_SNCCOMPILER_AdditionalOptions:
        if (!pCompilerTool->m_PropertyStates.m_Properties[i]
                 .m_StringValue.IsEmpty()) {
          additionalOptions +=
              pCompilerTool->m_PropertyStates.m_Properties[i].m_StringValue;
          additionalOptions += " ";
        }
        break;
    }
  ...
  if (!additionalOptions.IsEmpty()) {
    m_XMLWriter.Write(
        CFmtStrMax("AdditionalOptions=\"%s\"", additionalOptions.Get()));
  }
  ...
  for (intp i = 0; i < pCompilerTool->m_PropertyStates.m_Properties.Count();
    ...
    switch (pCompilerTool->m_PropertyStates.m_Properties[i]
      ...
      case PS3_GCCCOMPILER_AdditionalIncludeDirectories: {
        // String() returns temporary object, so save in var to prevent stale
        // memory usage.
        CUtlString aid = m_XMLWriter.FixupXMLString(
            pCompilerTool->m_PropertyStates.m_Properties[i]
                .m_StringValue.Get());
        m_XMLWriter.Write(
            CFmtStrMax("AdditionalIncludeDirectories=\"%s\"", aid.String()));
      } break;
...
bool CProjectGenerator_PS3::WritePreLinkEventTool(
    CPreLinkEventTool *pPreLinkEventTool) {
  ...
  for (intp i = 0; i < pPreLinkEventTool->m_PropertyStates.m_Properties.Count();
    ...
    switch (pPreLinkEventTool->m_PropertyStates.m_Properties[i]
      ...
      case PS3_PRELINKEVENT_CommandLine: {
        ...
        // memory usage.
        CUtlString cl = m_XMLWriter.FixupXMLString(
            pPreLinkEventTool->m_PropertyStates.m_Properties[i]
                .m_StringValue.Get());
        m_XMLWriter.Write(CFmtStrMax("CommandLine=\"%s\"", cl.String()));
      } break;
...
bool CProjectGenerator_PS3::WriteSNCLinkerTool(CLinkerTool *pLinkerTool) {
  ...
  for (intp i = 0; i < pLinkerTool->m_PropertyStates.m_Properties.Count(); i++) {
    ...
    switch (pLinkerTool->m_PropertyStates.m_Properties[i]
                .m_pToolProperty->m_nPropertyId) {
      case PS3_SNCLINKER_OutputFile:
        m_XMLWriter.Write(CFmtStrMax(
            "OutputFile=\"%s\"",
            pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
        break;
      ...
      case PS3_SNCLINKER_GenerateMapFile:
        if (nOrdinalValue == 1) {
          additionalOptions +=
              CFmtStrMax("-Map=&quot;%s&quot; ",
                         pLinkerTool->m_PropertyStates
                             .GetProperty(PS3_SNCLINKER_MapFileName)
                             ->m_StringValue.String());
        } else if (nOrdinalValue == 2) {
          additionalOptions +=
              CFmtStrMax("-Map=&quot;%s&quot; -sn-full-map ",
                         pLinkerTool->m_PropertyStates
                             .GetProperty(PS3_SNCLINKER_MapFileName)
                             ->m_StringValue.String());
        }
      ...
      case PS3_SNCLINKER_LinkLibraryDependencies:
        m_XMLWriter.Write(
            CFmtStrMax("LinkLibraryDependencies=\"%s\"",
                       BoolStringToTrueFalseString(
                           pLinkerTool->m_PropertyStates.m_Properties[i]
                               .m_StringValue.Get())));
        break;

      case PS3_SNCLINKER_AdditionalOptions:
        if (!pLinkerTool->m_PropertyStates.m_Properties[i]
                 .m_StringValue.IsEmpty()) {
          additionalOptions +=
              pLinkerTool->m_PropertyStates.m_Properties[i].m_StringValue;
          additionalOptions += " ";
        }
        break;
    }
      ...
      case PS3_GCCLINKER_GenerateMapFile:
        if (nOrdinalValue == 1) {
          additionalOptions +=
              CFmtStrMax("-Map=&quot;%s&quot; ",
                         pLinkerTool->m_PropertyStates
                             .GetProperty(PS3_GCCLINKER_MapFileName)
                             ->m_StringValue.String());
        }
        break;
  ...
  for (intp i = 0; i < pLibrarianTool->m_PropertyStates.m_Properties.Count();
    ...
    switch (pLibrarianTool->m_PropertyStates.m_Properties[i]
      ...
      case PS3_LIBRARIAN_OutputFile:
        m_XMLWriter.Write(
            CFmtStrMax("OutputFile=\"%s\"",
                       pLibrarianTool->m_PropertyStates.m_Properties[i]
                           .m_StringValue.Get()));
        break;
...
bool CProjectGenerator_PS3::WritePostBuildEventTool(
    CPostBuildEventTool *pPostBuildEventTool) {
  ...
  for (intp i = 0;
    ...
    switch (pPostBuildEventTool->m_PropertyStates.m_Properties[i]
      ...
      case PS3_POSTBUILDEVENT_CommandLine: {
        ...
        // memory usage.
        CUtlString cl = m_XMLWriter.FixupXMLString(
            pPostBuildEventTool->m_PropertyStates.m_Properties[i]
                .m_StringValue.Get());
        m_XMLWriter.Write(CFmtStrMax("CommandLine=\"%s\"", cl.String()));
      } break;
...
const char *CProjectGenerator_PS3::BoolStringToTrueFalseString(
    const char *pValue) {
...
bool CProjectGenerator_PS3::WriteConfiguration(CProjectConfiguration *pConfig) {
  ...
  for (intp i = 0; i < pConfig->m_PropertyStates.m_Properties.Count(); i++) {
    switch (pConfig->m_PropertyStates.m_Properties[i]
      ...
      case PS3_GENERAL_ConfigurationType:
        m_XMLWriter.Write(CFmtStrMax(
            "ConfigurationType=\"%s\"",
            pConfig->m_PropertyStates.m_Properties[i].m_StringValue.Get()));
        break;
      ...
      case PS3_GENERAL_ExcludedFromBuild:
        m_XMLWriter.Write(
            CFmtStrMax("ExcludedFromBuild=\"%s\"",
                       BoolStringToTrueFalseString(
                           pConfig->m_PropertyStates.m_Properties[i]
                               .m_StringValue.Get())));
        break;
  ...
  if (!WritePreBuildEventTool(pConfig->GetPreBuildEventTool())) return false;

  if (!WriteCustomBuildTool(pConfig->GetCustomBuildTool())) return false;

  if (m_pVCProjGenerator->GetVSIType() == PS3_VSI_TYPE_SNC) {
    if (!WriteSNCCompilerTool(pConfig->GetCompilerTool())) return false;
  } else if (m_pVCProjGenerator->GetVSIType() == PS3_VSI_TYPE_GCC) {
    if (!WriteGCCCompilerTool(pConfig->GetCompilerTool())) return false;
  }

  if (!WritePreLinkEventTool(pConfig->GetPreLinkEventTool())) return false;

  if (m_pVCProjGenerator->GetVSIType() == PS3_VSI_TYPE_SNC) {
    if (!WriteSNCLinkerTool(pConfig->GetLinkerTool())) return false;
  } else if (m_pVCProjGenerator->GetVSIType() == PS3_VSI_TYPE_GCC) {
    if (!WriteGCCLinkerTool(pConfig->GetLinkerTool())) return false;
  }

  if (!WriteLibrarianTool(pConfig->GetLibrarianTool())) return false;

  if (!WritePostBuildEventTool(pConfig->GetPostBuildEventTool())) return false;
...
bool CProjectGenerator_PS3::WriteToXML() {
  ...
  m_XMLWriter.Write(CFmtStrMax("RootNamespace=\"%s\"",
