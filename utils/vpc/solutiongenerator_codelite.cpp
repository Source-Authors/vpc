 ...
 public:
  ...
  virtual void GenerateSolutionFile(
    ...
    if (!V_GetFileExtension(pSolutionFilename)) {
      V_strncpy(szSolutionFileBaseName, pSolutionFilename,
                sizeof(szSolutionFileBaseName));
      V_snprintf(szTmpSolutionFilename, sizeof(szTmpSolutionFilename),
                 "%s.workspace", pSolutionFilename);
      pSolutionFilename = szTmpSolutionFilename;
    } else {
      V_StripExtension(pSolutionFilename, szSolutionFileBaseName,
                       sizeof(szSolutionFileBaseName));
    }
    ...
    for (intp i = 0; i < projects.Count(); i++) {
      ...
      V_StripExtension(pCurProject->m_ProjectFilename.String(),
                       szProjectFileBaseName, sizeof(szProjectFileBaseName));
  ...
  void WriteBuildOrderProject(const char *pszSolutionFileBaseName,
                              CUtlVector<CDependency_Project *> &projects) {
    // write out a project with no files to encode the build order
    // (dependencies)
    char szProjectFileName[MAX_PATH];
    V_snprintf(szProjectFileName, sizeof(szProjectFileName), "%s.project",
               pszSolutionFileBaseName);
    ...
    Write("<CodeLite_Project Name=\"all\" InternalType=\"\">\n");
    {
      ...
      Write("<Settings Type=\"Static Library\">\n");
      {
        ...
        Write(
            ...
            "DebuggerType=\"GNU gdb debugger\" Type=\"Executable\" "
  ...
  void TraverseFrom(CUtlVector<CDependency_Project *> &projects,
    ...
    while (visitor.Advance()) {
      ...
      // ]->m_ProjectName.String(), visitor.CurrentNode(), projects[
      // visitor.CurrentNode() ]->m_ProjectName.String() );
      TraverseFrom(projects, dependencyGraph, visitedList,
                   visitor.CurrentNode());
    }
