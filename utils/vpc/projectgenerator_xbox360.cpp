...
CProjectGenerator_Xbox360::CProjectGenerator_Xbox360() {
  ...
  m_pVCProjGenerator->SetupGeneratorDefinition(this, "xbox360.def",
                                               s_Xbox360PropertyNames);
}
...
bool CProjectGenerator_Xbox360::WriteConfiguration(
  ...
  if (!pConfig->m_bIsFileConfig &&
      pConfig->m_PropertyStates.m_Properties.Count()) {
  ...
  if (!WriteNULLTool("VCResourceCompilerTool", pConfig)) return false;

  if (!WriteTool("VCPreLinkEventTool", pConfig->GetPreLinkEventTool()))
    return false;

  if (!WriteTool("VCX360LinkerTool", pConfig->GetLinkerTool())) return false;

  if (!WriteTool("VCLibrarianTool", pConfig->GetLibrarianTool())) return false;

  if (!WriteNULLTool("VCALinkTool", pConfig)) return false;

  if (!WriteTool("VCX360ImageTool", pConfig->GetXboxImageTool())) return false;
...
bool CProjectGenerator_Xbox360::WriteNULLTool(
...
bool CProjectGenerator_Xbox360::WriteTool(const char *pToolName,
...
bool CProjectGenerator_Xbox360::WriteProperty(
  ...
  switch (pPropertyState->m_pToolProperty->m_nType) {
    ...
    default:
      g_pVPC->VPCError(
          "CProjectGenerator_Xbox360: WriteProperty, %s - not implemented",
          pOutputName);
  }
