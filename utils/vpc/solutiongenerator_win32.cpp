 ...
 public:
  ...
  virtual void GenerateSolutionFile(
    ...
    if (!V_GetFileExtension(pSolutionFilename)) {
      V_snprintf(szTmpSolutionFilename, sizeof(szTmpSolutionFilename), "%s.sln",
                 pSolutionFilename);
    ...
    for (int k = 1; k < __argc; ++k) fprintf(fp, "%s ", __argv[k]);
    fprintf(fp, "\n");
    fprintf(fp, "#\n");
    fprintf(fp, "#\n");
    ...
    for (intp i = 0; i < projects.Count(); i++) {
      ...
      if (!V_MakeRelativePath(pFullProjectFilename, g_pVPC->GetSourcePath(),
  ...
  void WriteSolutionItems(FILE *fp) {
    ...
    if (V_IsAbsolutePath(g_pVPC->GetSolutionItemsFilename()))
      V_strncpy(szFullSolutionItemsPath, g_pVPC->GetSolutionItemsFilename(),
                sizeof(szFullSolutionItemsPath));
    else
      V_ComposeFileName(
          g_pVPC->GetStartDirectory(), g_pVPC->GetSolutionItemsFilename(),
    ...
    while (g_pVPC->GetScript().GetData()) {
      ...
      if (V_IsAbsolutePath(pToken))
        ...
        V_ComposeFileName(g_pVPC->GetStartDirectory(), pToken, szFullPath,
                          sizeof(szFullPath));
