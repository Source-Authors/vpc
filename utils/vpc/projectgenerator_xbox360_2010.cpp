...
CProjectGenerator_Xbox360_2010::CProjectGenerator_Xbox360_2010() {
  ...
  m_pVCProjGenerator->SetupGeneratorDefinition(this, "xbox360_2010.def",
                                               s_Xbox360PropertyNames_2010);
}
...
bool CProjectGenerator_Xbox360_2010::WritePropertyGroupTool(
...
bool CProjectGenerator_Xbox360_2010::WriteFile(CProjectFile *pFile,
...
bool CProjectGenerator_Xbox360_2010::WriteFolder(CProjectFolder *pFolder,
                                                 const char *pFileTypeName,
                                                 int nDepth) {
  if (!nDepth) {
...
bool CProjectGenerator_Xbox360_2010::WriteConfiguration(
...
bool CProjectGenerator_Xbox360_2010::WriteTools(
    CProjectConfiguration *pConfig) {
  m_XMLWriter.PushNode(
      "ItemDefinitionGroup",
      CFmtStr("Condition=\"'$(Configuration)|$(Platform)'=='%s|Xbox 360'\"",
              pConfig->m_Name.Get()));
...
bool CProjectGenerator_Xbox360_2010::WritePrimaryXML(
  ...
  for (int i = 0; i < configurationNames.Count(); i++) {
    ...
    if (m_pVCProjGenerator->GetRootConfiguration(configurationNames[i].Get(),
      ...
      if (!WritePropertyGroupTool(pConfiguration->GetXboxImageTool(),
                                  pConfiguration))
        return false;

      if (!WritePropertyGroupTool(pConfiguration->GetXboxDeploymentTool(),
                                  pConfiguration))
        return false;
    }
  }
...
bool CProjectGenerator_Xbox360_2010::WriteFolderToSecondaryXML(
...
bool CProjectGenerator_Xbox360_2010::WriteFileToSecondaryXML(
...
bool CProjectGenerator_Xbox360_2010::WriteFolderContentsToSecondaryXML(
...
bool CProjectGenerator_Xbox360_2010::WriteSecondaryXML(
    const char *pOutputFilename) {
...
bool CProjectGenerator_Xbox360_2010::WriteTool(const char *pToolName,
...
bool CProjectGenerator_Xbox360_2010::WriteProperty(
  ...
  switch (pPropertyState->m_pToolProperty->m_nType) {
    ...
    default:
      g_pVPC->VPCError(
          "CProjectGenerator_Xbox360_2010: WriteProperty, %s - not "
          "implemented",
          pOutputName);
  }
