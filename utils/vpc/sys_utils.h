...
#ifndef VPC_SYS_UTILS_H_
#define VPC_SYS_UTILS_H_
...
class CSimplePointerStack {
 public:
  inline CSimplePointerStack() { m_nCount = 0; }

  inline void Purge() {
    for (int i = 0; i < m_nCount; i++) m_Values[i] = (NullType)NULL;
    m_nCount = 0;
  }
...
class CXMLWriter {
 public:
  ...
  void WriteLineNode(const char *pName, const char *pExtra,
                     const char *pString);
...
inline bool IsCFileExtension(const char *pExtension) {
  ...
  return !V_stricmp(pExtension, "cpp") || !V_stricmp(pExtension, "cxx") ||
         !V_stricmp(pExtension, "cc") || !V_stricmp(pExtension, "c");
}
...
inline bool IsHFileExtension(const char *pExtension) {
  ...
  return !V_stricmp(pExtension, "hpp") || !V_stricmp(pExtension, "hxx") ||
