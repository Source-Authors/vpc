  ...
  switch (version.Family) {
...

    {
      ...
      hr = WriteMSR((dw_counter == 0) ? 0x186 : 0x187,
                    ...
                    uint64((u1_event | (b_user ? (1 << 16) : 0) |
                            (b_kernel ? (1 << 17) : 0) | (1 << 22) | (1 << 18) |
                            (u1_mask << 8))));
    } break;
...
HRESULT PME::ReadMSR(uint32 dw_reg, int64* pi64_value) {
  ...
  hr = DeviceIoControl(hFile,                  // Handle to device
...
double PME::GetCPUClockSpeedSlow(void) {
  ...
  // return clock speed
  //  optionally here you could round to known clocks, like speeds that are
  //  multimples of 100, 133, 166, etc.
  m_CPUClockSpeed =
      ((stop_tsc - start_tsc) * 1000.0) / (double)(stop_ms - start_ms);
