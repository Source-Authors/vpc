...
class CCommandLine : public ICommandLine {
 public:
  ...
  virtual const char *CheckParm(const char *psz,
                                const char **ppszValue = 0) const;
 ...
 private:
  ...
  void LoadParametersFromFile(const char *&pSrc, char *&pDst, intp maxDestLen,
                              bool bInQuotes);
...
//-----------------------------------------------------------------------------
void CCommandLine::LoadParametersFromFile(const char *&pSrc, char *&pDst,
                                          intp maxDestLen, bool bInQuotes) {
  // Suck out the file name
  char szFileName[MAX_PATH];
  char *pOut;
  char *pDestStart = pDst;

  if (maxDestLen < 3) return;
  ...
  if (bInQuotes) terminatingChar = '\"';

  while (*pSrc && *pSrc != terminatingChar) {
    *pOut++ = *pSrc++;
    if ((pOut - szFileName) >= (MAX_PATH - 1)) break;
  }
...
void CCommandLine::CreateCmdLine(const char *commandline) {
  ...
  while (*pSrc) {
    ...
    if (*pSrc == '@') {
      if (pSrc == commandline || (!bInQuotes && V_isspace(pSrc[-1])) ||
          (bInQuotes && pSrc == pInQuotesStart)) {
        LoadParametersFromFile(pSrc, pDst, sizeof(szFull) - (pDst - szFull),
                               bInQuotes);
        if (bInQuotes) {
          // Back up over the opening quote which has already been copied to
          // pDst. Otherwise we end up with an orphaned single quote which
          // causes later parsing problems.
          --pDst;
          Assert(*pDst == '\"');
        }
...
void CCommandLine::RemoveParm(const char *pszParm) {
  ...
  while (*p) {
    ...
    if (found > m_pszCmdLine && found[-1] == '\"') bHadQuote = true;
    ...
    if (pnextparam &&
        (static_cast<size_t>(pnextparam - found) > strlen(pszParm))) {
      p = pnextparam;
      continue;
    }
  ...
  while (1) {
    ...
    if (len == 0 || m_pszCmdLine[len - 1] != ' ') break;
...
// Output : const char
//-----------------------------------------------------------------------------
const char *CCommandLine::GetCmdLine(void) const { return m_pszCmdLine; }
...
const char *CCommandLine::GetParm(int nIndex) const {
  ...
  if ((nIndex < 0) || (nIndex >= m_nParmCount)) return "";
...
//-----------------------------------------------------------------------------
const char *CCommandLine::ParmValue(const char *psz,
                                    const char *pDefaultVal) const {
  int nIndex = FindParm(psz);
  if ((nIndex == 0) || (nIndex == m_nParmCount - 1)) return pDefaultVal;
...
int CCommandLine::ParmValue(const char *psz, int nDefaultVal) const {
  int nIndex = FindParm(psz);
  if ((nIndex == 0) || (nIndex == m_nParmCount - 1)) return nDefaultVal;
...
float CCommandLine::ParmValue(const char *psz, float flDefaultVal) const {
  int nIndex = FindParm(psz);
  if ((nIndex == 0) || (nIndex == m_nParmCount - 1)) return flDefaultVal;
