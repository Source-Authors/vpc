...
bool Plat_IsInBenchmarkMode() { return g_bBenchmarkMode; }

void Plat_SetBenchmarkMode(bool bBenchmark) { g_bBenchmarkMode = bBenchmark; }
...
double Plat_FloatTime() {
  ...
  double fRawSeconds = (double)(CurrentTime.QuadPart - g_ClockStart.QuadPart) /
                       (double)(g_PerformanceFrequency.QuadPart);

  return fRawSeconds;
#else
  return cell::fios::FIOSAbstimeToMicroseconds(
             cell::fios::FIOSGetCurrentTime() - g_fiosLaunchTime) *
         1e-6;
#endif
}
...
uint32 Plat_MSTime() {
  ...
  return (uint32)((CurrentTime.QuadPart - g_ClockStart.QuadPart) /
                  g_MSPerformanceFrequency.QuadPart);
#elif defined(_PS3)
  return (uint32)cell::fios::FIOSAbstimeToMilliseconds(
      cell::fios::FIOSGetCurrentTime() - g_fiosLaunchTime);
#else
#error
#endif
...
void Plat_GetModuleFilename(char *pOut, int nMaxBytes) {
  ...
  if (GetLastError() != ERROR_SUCCESS)
    Error("Plat_GetModuleFilename: The buffer given is too small (%d bytes).",
          nMaxBytes);
#elif PLATFORM_X360
...
#elif defined(_PS3)
  // We do not use this path to exit on PS3 (naturally), rather we want a clear
  // crash:
  int *x = NULL;
  *x = 1;
#else
...
void Plat_DefaultAllocErrorFn(unsigned long) {}
