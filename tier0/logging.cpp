...
// The index of the logging state used by the current thread.  This defaults to
// 0 across all threads, which indicates that the global listener set should be
// used (CLoggingSystem::m_nGlobalStateIndex).
//
// NOTE:
// Because our linux TLS implementation does not support embedding a thread
// local integer in a class, the logging system must use a global thread-local
// integer. This means that we can only have one instance of CLoggingSystem,
// although we could support additional instances if we are willing to lose
// support for thread-local spew handling. There is no other reason why this
// class must be a singleton, except for the fact that there's no reason to have
...
CLoggingSystem *GetGlobalLoggingSystem() {
  return (g_pGlobalLoggingSystem == NULL) ? GetGlobalLoggingSystem_Internal()
                                          : g_pGlobalLoggingSystem;
}
...
LoggingChannelID_t CLoggingSystem::RegisterLoggingChannel(
    const char *pChannelName, RegisterTagsFunc registerTagsFunc, int flags,
  ...
  if (m_nChannelCount >= MAX_LOGGING_CHANNEL_COUNT) {
    ...
    // Channels can be multiply defined, in which case return the ID of the
    // existing channel.
    for (int i = 0; i < m_nChannelCount; ++i) {
      if (V_tier0_stricmp(m_RegisteredChannels[i].m_Name, pChannelName) == 0) {
        // OK to call the tag registration callback; duplicates will be culled
        // away. This allows multiple people to register a logging channel, and
        // the union of all tags will be registered.
        if (registerTagsFunc != NULL) {
          registerTagsFunc();
        }

        // If a logging channel is registered multiple times, only one of the
        // registrations should specify flags/severity/color.
        if (m_RegisteredChannels[i].m_Flags == 0 &&
            m_RegisteredChannels[i].m_MinimumSeverity == LS_MESSAGE &&
            m_RegisteredChannels[i].m_SpewColor == UNSPECIFIED_LOGGING_COLOR) {
          ...
          AssertMsg(flags == 0 || flags == m_RegisteredChannels[i].m_Flags,
                    "Non-zero or mismatched flags specified in logging channel "
                    "re-registration!");
          AssertMsg(severity == LS_MESSAGE ||
                        severity == m_RegisteredChannels[i].m_MinimumSeverity,
                    "Non-default or mismatched severity specified in logging "
                    "channel re-registration!");
          AssertMsg(spewColor == UNSPECIFIED_LOGGING_COLOR ||
                        spewColor == m_RegisteredChannels[i].m_SpewColor,
                    "Non-default or mismatched color specified in logging "
                    "channel re-registration!");
        }
    ...
    m_RegisteredChannels[m_nChannelCount].m_Flags =
    ...
    strncpy(m_RegisteredChannels[m_nChannelCount].m_Name, pChannelName,
            MAX_LOGGING_IDENTIFIER_LENGTH);
...
void CLoggingSystem::SetChannelSpewLevel(LoggingChannelID_t channelID,
                                         LoggingSeverity_t minimumSeverity) {
  GetChannel(channelID)->SetSpewLevel(minimumSeverity);
}

void CLoggingSystem::SetChannelSpewLevelByName(
...
void CLoggingSystem::SetChannelSpewLevelByTag(
...
void CLoggingSystem::PopLoggingState(bool bThreadLocal) {
  ...
  // Shouldn't be less than 0 (implies error during Push()) or 0 (implies that
  ...
  if (bThreadLocal) {
    g_nThreadLocalStateIndex =
        m_LoggingStates[nCurrentState].m_nPreviousStackEntry;
  } else {
...
LoggingResponse_t CLoggingSystem::LogDirect(LoggingChannelID_t channelID,
  ...
  if (!IsValidChannelID(channelID)) return LR_CONTINUE;
  ...
  context.m_Color = (color == UNSPECIFIED_LOGGING_COLOR)
                        ? m_RegisteredChannels[channelID].m_SpewColor
                        : color;
  ...
  switch (response) {
    case LR_DEBUGGER:
      // Asserts put the debug break in the macro itself so the code breaks at
      // the failure point.
...
CLoggingSystem::LoggingChannel_t *CLoggingSystem::GetChannel(
    LoggingChannelID_t channelID) {
  Assert(IsValidChannelID(channelID));
  return &m_RegisteredChannels[channelID];
}

const CLoggingSystem::LoggingChannel_t *CLoggingSystem::GetChannel(
    LoggingChannelID_t channelID) const {
  Assert(IsValidChannelID(channelID));
  return &m_RegisteredChannels[channelID];
}
...
CLoggingSystem::LoggingState_t *CLoggingSystem::GetCurrentState() {
  ...
  if (nState != 0) {
    ...
    Assert(m_nGlobalStateIndex >= 0 &&
           m_nGlobalStateIndex < MAX_LOGGING_STATE_COUNT);
...
CLoggingSystem::LoggingTag_t *CLoggingSystem::AllocTag(const char *pTagName) {
  ...
  Assert(m_nTagNamePoolIndex + nTagLength + 1 <=
         MAX_LOGGING_TAG_CHARACTER_COUNT);
...
LoggingChannelID_t LoggingSystem_RegisterLoggingChannel(
    const char *pName, RegisterTagsFunc registerTagsFunc, int flags,
    LoggingSeverity_t severity, Color color) {
  return GetGlobalLoggingSystem()->RegisterLoggingChannel(
      pName, registerTagsFunc, flags, severity, color);
}
...
void LoggingSystem_SetLoggingResponsePolicy(
...
LoggingChannelID_t LoggingSystem_GetFirstChannelID() {
  return (GetGlobalLoggingSystem()->GetChannelCount() > 0)
             ? 0
             : INVALID_LOGGING_CHANNEL_ID;
}

LoggingChannelID_t LoggingSystem_GetNextChannelID(
    LoggingChannelID_t channelID) {
  int nChannelCount = GetGlobalLoggingSystem()->GetChannelCount();
  int nNextChannel = channelID + 1;
  return (nNextChannel < nChannelCount) ? nNextChannel
                                        : INVALID_LOGGING_CHANNEL_ID;
}

const CLoggingSystem::LoggingChannel_t *LoggingSystem_GetChannel(
    LoggingChannelID_t channelIndex) {
...
bool LoggingSystem_IsChannelEnabled(LoggingChannelID_t channelID,
                                    LoggingSeverity_t severity) {
...
void LoggingSystem_SetChannelSpewLevelByName(
    const char *pName, LoggingSeverity_t minimumSeverity) {
  GetGlobalLoggingSystem()->SetChannelSpewLevelByName(pName, minimumSeverity);
}

void LoggingSystem_SetChannelSpewLevelByTag(const char *pTag,
                                            LoggingSeverity_t minimumSeverity) {
...
LoggingChannelFlags_t LoggingSystem_GetChannelFlags(
    LoggingChannelID_t channelID) {
  return GetGlobalLoggingSystem()->GetChannelFlags(channelID);
}

void LoggingSystem_SetChannelFlags(LoggingChannelID_t channelID,
                                   LoggingChannelFlags_t flags) {
...
LoggingResponse_t LoggingSystem_Log(
  ...
  Tier0Internal_vsntprintf(formattedMessage, MAX_LOGGING_MESSAGE_LENGTH,
                           pMessageFormat, args);
  va_end(args);

  return GetGlobalLoggingSystem()->LogDirect(
...
LoggingResponse_t LoggingSystem_Log(
  ...
  return GetGlobalLoggingSystem()->LogDirect(channelID, severity, spewColor,
                                             formattedMessage);
}

LoggingResponse_t LoggingSystem_LogDirect(LoggingChannelID_t channelID,
                                          LoggingSeverity_t severity,
                                          Color spewColor,
                                          const char *pMessage) {
  if (!GetGlobalLoggingSystem()->IsChannelEnabled(channelID, severity))
...
LoggingResponse_t LoggingSystem_LogAssert(
  ...
  return GetGlobalLoggingSystem()->LogDirect(
      LOG_ASSERT, LS_ASSERT, UNSPECIFIED_LOGGING_COLOR, formattedMessage);
}
