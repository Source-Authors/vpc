...
#if (defined(USE_STACK_TRACES)) && \
    !(defined(TIER0_FPO_DISABLED) || defined(_DEBUG))
...
#if defined(SORT_STACK_TRACE_DESCRIPTION_DUMPS)  // Assuming StackDescribe is
                                                 // called iteratively on a
                                                 // sorted set of stacks (as in
                                                 // DumpStackStats()). We can
                                                 // save work by skipping
                                                 // unchanged parts at the
  ...
  for (k = 0; k < nMaxAddresses; ++k) {
    ...
    if (bUseExistingString && (k < STACK_TRACE_LENGTH)) {
      if (ppAddresses[k] == LastCallStack[k]) {
        ...
        for (int clearEntries = k; clearEntries < STACK_TRACE_LENGTH;
             ++clearEntries)  // wipe out unused entries
        {
...
#endif
    {
      ...
      if (GetFileAndLineFromAddress(ppAddresses[k], szTemp, MAX_PATH, iLine,
                                    &iLineDisplacement)) {
        char const *pchFileName = szTemp + strlen(szTemp);
        for (size_t numSlashesAllowed = 2; pchFileName > szTemp;
             --pchFileName) {
          if (*pchFileName == '\\') {
        ...
        sprintf(pchBuffer, iLineDisplacement ? "%s:%d+0x%I32X" : "%s:%d",
                pchFileName, iLine, iLineDisplacement);
      } else if (GetSymbolNameFromAddress(ppAddresses[k], szTemp, MAX_PATH,
                                          &iSymbolDisplacement)) {
        sprintf(pchBuffer,
                (iSymbolDisplacement > 0 && !(iSymbolDisplacement >> 63))
                    ? "%s+0x%llX"
                    : "%s",
                szTemp, iSymbolDisplacement);
      } else {
...
DbgMemHeader_t **GetRecentFrees() {
  static DbgMemHeader_t **g_pRecentFrees =
      (DbgMemHeader_t **)
#ifdef _PS3
...
void LMDReportInvalidBlock(DbgMemHeader_t *pHeader, const char *pszMessage) {
  ...
  if (pHeader) {
    sprintf(szMsg, "HEAP IS CORRUPT: %s (block 0x%zx, %zu bytes)\n", pszMessage,
            (size_t)(((byte *)pHeader) + sizeof(DbgMemHeader_t)),
            pHeader->nLogicalSize);
  } else {
...
void LMDValidateBlock(DbgMemHeader_t *pHeader, bool bFreeList) {
  if (memcmp(&pHeader->sentinal,
             bFreeList ? &g_HeadSentinelFree : &g_HeadSentinelAllocated,
             sizeof(Sentinal_t)) != 0) {
    LMDReportInvalidBlock(pHeader, "Head sentinel corrupt");
  }
  if (memcmp(((Sentinal_t *)((((byte *)pHeader) + sizeof(DbgMemHeader_t) +
                              pHeader->nLogicalSize))),
             &g_TailSentinel, sizeof(Sentinal_t)) != 0) {
...
#if defined(USE_STACK_TRACES)
#define GetAllocationStatIndex_Internal(pMem) \
  (((DbgMemHeader_t *)pMem - 1)->nStatIndex)
#endif
...
inline void *InternalMalloc(size_t nSize, const char *pFileName, int nLine) {
  ...
  pAllocedMem =
      malloc_zone_malloc(malloc_default_zone(),
                         nSize + sizeof(DbgMemHeader_t) + sizeof(Sentinal_t));
  DbgMemHeader_t *pInternalMem = GetCrtDbgMemHeader(pAllocedMem);
#elif defined(_PS3)
  pAllocedMem = (g_pMemOverrideRawCrtFns->pfn_malloc)(
      nSize + sizeof(DbgMemHeader_t) + sizeof(Sentinal_t));
  ...
  *((Sentinal_t *)(((byte *)pInternalMem) + sizeof(DbgMemHeader_t) + nSize)) =
      g_TailSentinel;
...
inline void *InternalMallocAligned(size_t nSize, size_t align,
  ...
  size_t nSizeRequired =
      nSize + numWastedAlignPages * align + sizeof(Sentinal_t);
  pAllocedMem = (g_pMemOverrideRawCrtFns->pfn_memalign)(align, nSizeRequired);
  DbgMemHeader_t *pInternalMem =
      GetCrtDbgMemHeader(((char *)pAllocedMem) + numWastedAlignPages * align);
  *((void **)pInternalMem->m_Reserved2) = pAllocedMem;
#else
  pAllocedMem = malloc(nSize + sizeof(DbgMemHeader_t) + sizeof(Sentinal_t));
  DbgMemHeader_t *pInternalMem = (DbgMemHeader_t *)pAllocedMem;
...
inline void *InternalRealloc(void *pMem, size_t nNewSize, const char *pFileName,
                             int nLine) {
...
#ifdef OSX
  pNewAllocedMem = (DbgMemHeader_t *)malloc_zone_realloc(
      malloc_default_zone(), pMem,
      nNewSize + sizeof(DbgMemHeader_t) + sizeof(Sentinal_t));
  DbgMemHeader_t *pInternalMem = GetCrtDbgMemHeader(pNewAllocedMem);
#elif defined(_PS3)
  DbgMemHeader_t *pInternalMem = GetCrtDbgMemHeader(pMem);
  pNewAllocedMem = (DbgMemHeader_t *)(g_pMemOverrideRawCrtFns->pfn_realloc)(
      *((void **)pInternalMem->m_Reserved2),
      nNewSize + sizeof(DbgMemHeader_t) + sizeof(Sentinal_t));
  pInternalMem = (DbgMemHeader_t *)pNewAllocedMem;
  *((void **)pInternalMem->m_Reserved2) = pNewAllocedMem;
#else
  DbgMemHeader_t *pInternalMem = GetCrtDbgMemHeader(pMem);
  pNewAllocedMem = (DbgMemHeader_t *)realloc(
      pInternalMem, nNewSize + sizeof(DbgMemHeader_t) + sizeof(Sentinal_t));
  pInternalMem = (DbgMemHeader_t *)pNewAllocedMem;
#endif
  ...
  *((int *)pInternalMem->m_Reserved) = 0xf00df00d;
  ...
  pInternalMem->sentinal = g_HeadSentinelAllocated;
  *((Sentinal_t *)(((byte *)pInternalMem) + sizeof(DbgMemHeader_t) +
                   nNewSize)) = g_TailSentinel;
  LMDValidateBlock(pInternalMem, false);
...
#ifdef MEMALLOC_SUPPORTS_ALIGNED_ALLOCATIONS
inline void *InternalReallocAligned(void *pMem, size_t nNewSize, size_t align,
                                    const char *pFileName, int nLine) {
  if (!pMem) return InternalMallocAligned(nNewSize, align, pFileName, nLine);
  ...
  if (align * numWastedAlignPages < sizeof(DbgMemHeader_t))
    ++numWastedAlignPages;
  size_t nSizeRequired =
      nNewSize + numWastedAlignPages * align + sizeof(Sentinal_t);

  DbgMemHeader_t *pInternalMem = GetCrtDbgMemHeader(pMem);
  pNewAllocedMem =
      (DbgMemHeader_t *)(g_pMemOverrideRawCrtFns->pfn_reallocalign)(
          *((void **)pInternalMem->m_Reserved2), nSizeRequired, align);
  pInternalMem = GetCrtDbgMemHeader(((char *)pNewAllocedMem) +
                                    numWastedAlignPages * align);
  *((void **)pInternalMem->m_Reserved2) = pNewAllocedMem;
#else
  DbgMemHeader_t *pInternalMem = GetCrtDbgMemHeader(pMem);
  pNewAllocedMem = (DbgMemHeader_t *)realloc(
...
inline void InternalFree(void *pMem) {
  ...
  if (memcmp(&pInternalMem->sentinal, &g_HeadSentinelAllocated,
             sizeof(Sentinal_t)) != 0) {
  ...
  for (uint32 k = iNextSlot - 1, iteration = 0; iteration < 10;
       ++iteration, --k) {
...
class CDbgMemAlloc : public IMemAlloc {
 public:
  ...
  virtual void *AllocAlign(size_t nSize, size_t align, const char *pFileName,
                           int nLine);
  ...
  // Returns the size of a particular allocation (NOTE: may be larger than the
  // size requested!)
  ...
  virtual int CrtDbgReport(int nRptType, const char *szFile, int nLine,
 ...
 private:
  ...
  typedef std::map<
      MemInfoKey_FileLine_t, MemInfo_t, std::less<MemInfoKey_FileLine_t>,
      CNoRecurseAllocator<std::pair<const MemInfoKey_FileLine_t, MemInfo_t>>>
      StatMap_FileLine_t;
  typedef StatMap_FileLine_t::iterator StatMapIter_FileLine_t;
  typedef StatMap_FileLine_t::value_type StatMapEntry_FileLine_t;

  typedef std::set<const char *, CStringLess, CNoRecurseAllocator<const char *>>
      Filenames_t;
 ...
 private:
...
#if defined(USE_STACK_TRACES)
  int GetCallStackForIndex(unsigned int index, void **pCallStackOut,
                           int iMaxEntriesOut);
  friend int GetAllocationCallStack(void *mem, void **pCallStackOut,
                                    int iMaxEntriesOut);
#endif
  ...
  virtual void RegisterDeallocation(const char *pFileName, int nLine,
                                    ...
                                    unsigned nTime);
#if defined(USE_STACK_TRACES)
  void RegisterAllocation(unsigned int nStatIndex, size_t nLogicalSize,
                          size_t nActualSize, unsigned nTime);
  void RegisterDeallocation(unsigned int nStatIndex, size_t nLogicalSize,
                            size_t nActualSize, unsigned nTime);
#endif
  void RegisterAllocation(MemInfo_t &info, size_t nLogicalSize,
                          size_t nActualSize, unsigned nTime);
  void RegisterDeallocation(MemInfo_t &info, size_t nLogicalSize,
                            size_t nActualSize, unsigned nTime);
  ...
  // Stat output
  void DumpMemInfo(const char *pAllocationName, int line,
                   const MemInfo_t &info);
  void DumpFileStats();
#if defined(USE_STACK_TRACES)
  void DumpMemInfo(void *const CallStack[STACK_TRACE_LENGTH],
                   const MemInfo_t &info);
...
#if defined(USE_STACK_TRACES)
  typedef CCallStackStatsGatherer<
      MemInfo_t, STACK_TRACE_LENGTH, GetCallStack_Fast,
      CCallStackStatsGatherer_StatMutexPool<128>, CNoRecurseAllocator>
      CallStackStatsType_t;
...
DEFINE_STATSTRUCTFIELD(m_nCurrentSize, BasicStatStructFieldDesc,
                       (BSSFT_SIZE_T, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD(m_nPeakSize, BasicStatStructFieldDesc,
                       (BSSFT_SIZE_T, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD(m_nTotalSize, BasicStatStructFieldDesc,
                       (BSSFT_SIZE_T, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD(m_nOverheadSize, BasicStatStructFieldDesc,
                       (BSSFT_SIZE_T, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD(m_nPeakOverheadSize, BasicStatStructFieldDesc,
                       (BSSFT_SIZE_T, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD(m_nCurrentCount, BasicStatStructFieldDesc,
                       (BSSFT_INT, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD(m_nPeakCount, BasicStatStructFieldDesc,
                       (BSSFT_INT, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD(m_nTotalCount, BasicStatStructFieldDesc,
                       (BSSFT_INT, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD(m_nSumTargetRange, BasicStatStructFieldDesc,
                       (BSSFT_INT, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD(m_nCurTargetRange, BasicStatStructFieldDesc,
                       (BSSFT_INT, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD(m_nMaxTargetRange, BasicStatStructFieldDesc,
                       (BSSFT_INT, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD_ARRAYENTRY(m_pCount, BYTE_COUNT_16,
                                  BasicStatStructFieldDesc,
                                  (BSSFT_INT, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD_ARRAYENTRY(m_pCount, BYTE_COUNT_32,
                                  BasicStatStructFieldDesc,
                                  (BSSFT_INT, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD_ARRAYENTRY(m_pCount, BYTE_COUNT_128,
                                  BasicStatStructFieldDesc,
                                  (BSSFT_INT, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD_ARRAYENTRY(m_pCount, BYTE_COUNT_2048,
                                  BasicStatStructFieldDesc,
                                  (BSSFT_INT, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD_ARRAYENTRY(m_pCount, BYTE_COUNT_GREATER,
                                  BasicStatStructFieldDesc,
                                  (BSSFT_INT, BSSFCM_ADD))
DEFINE_STATSTRUCTFIELD(m_nTime, BasicStatStructFieldDesc,
                       (BSSFT_INT64, BSSFCM_ADD))
...
#define IfDbgInfoIsReady()                       \
  if (TLSGlobals *IfDbgInfoIsReady_pTlsGlobals = \
          g_pfnElfGetTlsGlobals ? g_pfnElfGetTlsGlobals() : NULL)
#else
#define IfDbgInfoIsReady() \
  if (TLSGlobals *IfDbgInfoIsReady_pTlsGlobals = GetTLSGlobals())
#endif
#define g_DbgInfoStack \
  ((DbgInfoStack_t *&)IfDbgInfoIsReady_pTlsGlobals->pMallocDbgInfoStack)
#define g_nDbgInfoStackDepth \
  (IfDbgInfoIsReady_pTlsGlobals->nMallocDbgInfoStackDepth)
#else
...
struct CDbgMemAlloc_GetRawCrtMemOverrideFuncs_Early {
  CDbgMemAlloc_GetRawCrtMemOverrideFuncs_Early() {
    ...
    g_pMemOverrideRawCrtFns =
        reinterpret_cast<MemOverrideRawCrtFunctions_t *>(iResult);
  }
...
const char *CDbgMemAlloc::s_pCountHeader[CDbgMemAlloc::NUM_BYTE_COUNT_BUCKETS] =
    {"<=16 byte allocations", "17-32 byte allocations",
     "33-128 byte allocations", "129-2048 byte allocations",
...
void CDbgMemAlloc::SaveDebugInfo(void *pvDebugInfo) {
  IfDbgInfoIsReady() {
    ...
    *pnStackDepth = g_nDbgInfoStackDepth;
    memcpy(pnStackDepth + 1, &g_DbgInfoStack[0],
           sizeof(DbgInfoStack_t) * DBG_INFO_STACK_DEPTH);
  }
}
...
void CDbgMemAlloc::RestoreDebugInfo(const void *pvDebugInfo) {
  IfDbgInfoIsReady() {
    ...
    memcpy(&g_DbgInfoStack[0], pnStackDepth + 1,
           sizeof(DbgInfoStack_t) * DBG_INFO_STACK_DEPTH);
  }
}

void CDbgMemAlloc::InitDebugInfo(void *pvDebugInfo, const char *pchRootFileName,
                                 int nLine) {
  int32 *pnStackDepth = (int32 *)pvDebugInfo;
...
const char *CDbgMemAlloc::FindOrCreateFilename(const char *pFileName) {
...
#if defined(USE_STACK_TRACES_DETAILED)
  {
    ...
    int numStackAddrRetrieved = GetCallStack_Fast(
        arrStackAddresses, 10,
        2);  // Skip this function, and either CDbgMemAlloc::Alloc() or
             // CDbgMemAlloc::Realloc()
...
#if defined(USE_STACK_TRACES)
int CDbgMemAlloc::GetCallStackForIndex(unsigned int index, void **pCallStackOut,
                                       int iMaxEntriesOut) {
  if (iMaxEntriesOut > STACK_TRACE_LENGTH) iMaxEntriesOut = STACK_TRACE_LENGTH;

  CallStackStatsType_t::StackReference stackRef =
      m_CallStackStats.GetCallStackForIndex(index);

  memcpy(pCallStackOut, stackRef, iMaxEntriesOut * sizeof(void *));
  for (int i = 0; i != iMaxEntriesOut; ++i) {
    if (pCallStackOut[i] == NULL) return i;
  }
...
// Updates stats
//-----------------------------------------------------------------------------
void CDbgMemAlloc::RegisterAllocation(const char *pFileName, int nLine,
                                      size_t nLogicalSize, size_t nActualSize,
                                      unsigned nTime) {
  HEAP_LOCK();
...
void CDbgMemAlloc::RegisterDeallocation(const char *pFileName, int nLine,
                                        size_t nLogicalSize, size_t nActualSize,
                                        unsigned nTime) {
  HEAP_LOCK();
...
#if defined(USE_STACK_TRACES)
void CDbgMemAlloc::RegisterAllocation(unsigned int nStatIndex,
                                      size_t nLogicalSize, size_t nActualSize,
                                      unsigned nTime) {
  HEAP_LOCK();
  RegisterAllocation(m_GlobalInfo, nLogicalSize, nActualSize, nTime);
  CCallStackStatsGatherer_StructAccessor_AutoLock<MemInfo_t> entryAccessor =
      m_CallStackStats.GetEntry(nStatIndex);
  RegisterAllocation(*entryAccessor.GetStruct(), nLogicalSize, nActualSize,
                     nTime);
}

void CDbgMemAlloc::RegisterDeallocation(unsigned int nStatIndex,
                                        size_t nLogicalSize, size_t nActualSize,
                                        unsigned nTime) {
  HEAP_LOCK();
  RegisterDeallocation(m_GlobalInfo, nLogicalSize, nActualSize, nTime);
  CCallStackStatsGatherer_StructAccessor_AutoLock<MemInfo_t> entryAccessor =
      m_CallStackStats.GetEntry(nStatIndex);
  RegisterDeallocation(*entryAccessor.GetStruct(), nLogicalSize, nActualSize,
                       nTime);
}
#endif
...
void CDbgMemAlloc::RegisterAllocation(MemInfo_t &info, size_t nLogicalSize,
  ...
  if (nLogicalSize > g_TargetCountRangeMin &&
      nLogicalSize <= g_TargetCountRangeMax) {
...
#if defined(USE_STACK_TRACES)
  unsigned int iStatEntryIndex = m_CallStackStats.GetEntryIndex(
      CCallStackStorage(m_CallStackStats.StackFunction, 1));
#endif
...
#if defined(USE_STACK_TRACES)
  RegisterAllocation(GetAllocationStatIndex_Internal(pMem),
...
#if defined(USE_STACK_TRACES)
  unsigned int iStatEntryIndex =
      m_CallStackStats.GetEntryIndexForCurrentCallStack(1);
#endif
...
void *CDbgMemAlloc::Realloc(void *pMem, size_t nSize, const char *pFileName,
                            int nLine) {
  HEAP_LOCK();
  ...
  if (pMem != 0) {
#if defined(USE_STACK_TRACES)
    RegisterDeallocation(GetAllocationStatIndex_Internal(pMem),
...
#if defined(USE_STACK_TRACES)
  RegisterDeallocation(oldStatIndex, nOldLogicalSize, nOldSize,
                       m_Timer.GetDuration().GetMicroseconds());
#else
...
int CDbgMemAlloc::CrtIsValidPointer(const void *pMem, unsigned int size,
                                    int access) {
#ifdef POSIX
  return 0;
#else
...
int CDbgMemAlloc::CrtDbgReport(int nRptType, const char *szFile, int nLine,
...
// Stat output
//-----------------------------------------------------------------------------
void CDbgMemAlloc::DumpMemInfo(const char *pAllocationName, int line,
                               const MemInfo_t &info) {
  m_OutputFunc(
      "%s, line %i\t%.1f\t%.1f\t%.1f\t%.1f\t%.1f\t%d\t%d\t%d\t%d\t%d\t%d\t%d",
      pAllocationName, line, info.m_nCurrentSize / 1024.0f,
      info.m_nPeakSize / 1024.0f, info.m_nTotalSize / 1024.0f,
      info.m_nOverheadSize / 1024.0f, info.m_nPeakOverheadSize / 1024.0f,
      (int)(info.m_nTime / 1000), info.m_nCurrentCount, info.m_nPeakCount,
      info.m_nTotalCount, info.m_nSumTargetRange, info.m_nCurTargetRange,
...
void CDbgMemAlloc::DumpStatsFileBase(char const *pchFileBase) {
  ...
  if (m_OutputFunc == DefaultHeapReportFunc) {
    ...
    //"%s%s_%2.2d%2.2d_%2.2d%2.2d%2.2d_%d.txt", pPath, s_szStatsMapName,
    // systemTime.wMonth, systemTime.wDay, systemTime.wHour, systemTime.wMinute,
    // systemTime.wSecond, s_FileCount );
    _snprintf(szFileName, sizeof(szFileName), "%s%s_%d.txt", pPath,
              s_szStatsMapName, s_FileCount);
#else
    _snprintf(szFileName, sizeof(szFileName), "%s%s_%d.txt", pPath, pchFileBase,
              s_FileCount);
...

  {
    ...
    m_OutputFunc(
        ...
        "Size(k)\tTime(ms)\tCurrent Count\tPeak Count\tTotal "
        "Count\tTNum\tTCur\tTMax");
...
#ifdef _PS3
    {
      ...
      sys_prx_get_module_list_t prxList = {sizeof(sys_prx_get_module_list_t),
                                           ARRAYSIZE(prxIDs), 0, prxIDs, NULL};
    ...
    if (IsGameConsole()) {
      // Add a line showing total system memory usage from the OS (if this is
      // more than
      // "||Totals||", then there is unknown memory usage that we need to track
      // down):
...
#ifdef _MEMTEST
    {
      ...
      int nGPUMemSize, nGPUMemFree, nTextureSize, nRTSize, nVBSize, nIBSize,
          nUnknown;
      if (7 == sscanf(s_szStatsComment, "%d %d %d %d %d %d %d", &nGPUMemSize,
                      &nGPUMemFree, &nTextureSize, &nRTSize, &nVBSize, &nIBSize,
                      &nUnknown)) {
...
void CDbgMemAlloc::GlobalMemoryStatus(size_t *pUsedMemory,
                                      size_t *pFreeMemory) {
  ...
  *pUsedMemory = (stat.dwTotalPhys - 32 * 1024 * 1024) - *pFreeMemory;
  ...
  // (since it NEVER returns pages back to the OS after hitting a
  // high-watermark)
  static malloc_managed_size mms;
  (g_pMemOverrideRawCrtFns->pfn_malloc_stats)(&mms);
...
void CDbgMemAlloc::DumpCallStackFlow(char const *pchFileBase) {
  ...
  GetLocalTime(&systemTime);
  _snprintf(szFileName, sizeof(szFileName),
            "%s%s_%2.2d%2.2d_%2.2d%2.2d%2.2d_%d.csf", pPath, s_szStatsMapName,
            systemTime.wMonth, systemTime.wDay, systemTime.wHour,
            systemTime.wMinute, systemTime.wSecond, s_FileCount);
#else
  _snprintf(szFileName, sizeof(szFileName), "%s%s%d.vcsf", pPath, pchFileBase,
            s_FileCount);
#endif
...
static inline void set_glibc_hooks(void) {
  ...
  __memalign_hook =
      (void *(*)(size_t, size_t, const void *))glibc_memalign_hook;
...
void (*__malloc_initialize_hook)(void)
    __attribute__((visibility("default"))) = override_init_hook;
...
static void *override_realloc_hook(struct _malloc_zone_t *zone, void *ptr,
                                   size_t s) {
  void *retval;
...
static void override_free_hook(struct _malloc_zone_t *zone, void *ptr) {
  // sometime they pass in a null pointer from higher level calls, just ignore
  // it
  if (!ptr) return;
...
/*
 ...
 These are func's we could optionally override right now on OSX but don't need
 to
 ...
 static void *override_calloc_hook(struct _malloc_zone_t *zone, size_t
 num_items, size_t size )
 {
...
static inline void save_osx_hooks(void) {
  ...
  // need to osx_size_hook = (void *)malloc_zone->size; osx_calloc_hook = (void
  // *)malloc_zone->calloc; osx_valloc_hook = (void *)malloc_zone->valloc;
...
static inline void set_osx_hooks(void) {
  ...
  malloc_zone->realloc =
      (void *(*)(_malloc_zone_t *, void *, size_t))osx_realloc_hook;
  malloc_zone->free = (void (*)(_malloc_zone_t *, void *))osx_free_hook;

  // These are func's we could optionally override right now on OSX but don't
  // need to

  // malloc_zone->size = (size_t (*)(_malloc_zone_t*, const void
  // *))osx_size_hook;
  // malloc_zone->calloc = (void* (*)(_malloc_zone_t*, size_t,
  // size_t))osx_calloc_hook; malloc_zone->valloc = (void* (*)(_malloc_zone_t*,
  // size_t))osx_valloc_hook; malloc_zone->destroy = (void
  // (*)(_malloc_zone_t*))osx_destroy_hook;
}
