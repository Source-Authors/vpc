...
#ifndef USE_DLMALLOC
#ifdef _PS3
#define malloc_internal(region, bytes) \
  (g_pMemOverrideRawCrtFns->pfn_malloc)(bytes)
#define malloc_aligned_internal(region, bytes, align) \
  (g_pMemOverrideRawCrtFns->pfn_memalign)(align, bytes)
...
void *g_AllocRegions[] = {
    ...
    create_mspace(100 * 1024 * 1024, 1),
        ...
        // @TODO: per DLL regions didn't work out very well. flux of usage left
        // too much overhead. need to try lifetime-based management [6/9/2009
        // tom]
...
#else
// NOTE: this split is designed to force the 'large block' heap to ONLY perform
// virtual allocs (see
...
#define malloc_internal(region, bytes) \
  mspace_malloc(SelectRegion(region, bytes), bytes)
#define malloc_aligned_internal(region, bytes, align) \
  mspace_memalign(SelectRegion(region, bytes), align, bytes)
FORCEINLINE void *realloc_aligned_internal(void *mem, size_t bytes,
                                           size_t align) {
  // TODO: implement realloc_aligned inside dlmalloc (requires splitting
  // realloc's existing
  //       'grow in-place' code into a new function, then call that w/
  //       alloc_align/copy/free on failure)
  byte *newMem = (byte *)dlrealloc(mem, bytes);
  if (((size_t)newMem & (align - 1)) == 0) return newMem;
...
inline void heapstats_internal(FILE *pFile) {
  ...
  for (size_t i = 0; i < std::size(g_AllocRegions); i++) {
    ...
    _snprintf(
        ...
        "     %zu:footprint_max -%10zu (maximum total space used by the "
        "mspace)\n"
        "     %zu:arena         -%10zu (non-mmapped space allocated from "
        "system)\n"
        ...
        i, i ? "medium-block" : "large-block", i, footPrint, i, maxFootPrint, i,
        info.arena, i, info.ordblks, i, info.hblkhd, i, info.usmblks, i,
        info.uordblks, i, info.fordblks, i, info.keepcost);
    if (pFile)
      fprintf(pFile, "%s", buf);
    else
      Msg("%s", buf);
...
#if _MSC_VER < 1400 && defined(MSVC) && !defined(_STATIC_LINKED) && \
    (defined(_DEBUG) || defined(USE_MEM_DEBUG))
void *operator new(unsigned int nSize, int nBlockUse, const char *pFileName,
                   int nLine) {
...
CSmallBlockPool<CStdMemAlloc::CFixedAllocator<MBYTES_PRIMARY_SBH, true>>::
    SharedData_t CSmallBlockPool<
        CStdMemAlloc::CFixedAllocator<MBYTES_PRIMARY_SBH, true>>::gm_SharedData
        CONSTRUCT_EARLY;
#ifdef MEMALLOC_USE_SECONDARY_SBH
CSmallBlockPool<CStdMemAlloc::CFixedAllocator<MBYTES_SECONDARY_SBH, false>>::
    SharedData_t CSmallBlockPool<CStdMemAlloc::CFixedAllocator<
        MBYTES_SECONDARY_SBH, false>>::gm_SharedData CONSTRUCT_EARLY;
#endif
#ifndef MEMALLOC_NO_FALLBACK
CSmallBlockPool<CStdMemAlloc::CVirtualAllocator>::SharedData_t CSmallBlockPool<
    CStdMemAlloc::CVirtualAllocator>::gm_SharedData CONSTRUCT_EARLY;
#endif
...
void CSmallBlockPool<CAllocator>::ValidateFreelist(SharedData_t *pSharedData) {
  ...
  TSLNodeBase_t *pNode =
      (TSLNodeBase_t *)(m_FreeList.AccessUnprotected()->Next.Next);
#else
  TSLNodeBase_t *pNode =
      (TSLNodeBase_t *)(m_FreeList.AccessUnprotected()->value.Next);
#endif
    ...
    if (!pResult) {
      ...
      while (1) {
        ...
        if (pResult) {
          ...
          if ((((uintp)(pNextAlloc)-1) % BYTES_PAGE) + nBlockSize >
              BYTES_PAGE) {
          ...
          if (m_pNextAlloc.AssignIf(pResult, pNextAlloc)) {
            iPage =
                (size_t)((byte *)pResult - pSharedData->m_pBase) / BYTES_PAGE;
            break;
          }
        } else if (m_CommitMutex.TryLock()) {
          if (!m_pNextAlloc) {
            PageStatus_t *pAllocatedPageStatus =
                (PageStatus_t *)pSharedData->m_FreePages.Pop();
            if (pAllocatedPageStatus) {
              ...
              while (1) {
                ...
                if (ThreadInterlockedAssignPointerIf(
                        (void **)&pSharedData->m_pNextBlock,
                        (void *)(pBlock + BYTES_PAGE), (void *)pBlock)) {
                  iPage = (size_t)((byte *)pBlock - pSharedData->m_pBase) /
                          BYTES_PAGE;
...
int CSmallBlockPool<CAllocator>::PageSort(const void *p1, const void *p2) {
  SharedData_t *pSharedData = GetSharedData();
  return pSharedData->m_PageStatus[*((int *)p1)].m_SortList.Count() -
         pSharedData->m_PageStatus[*((int *)p2)].m_SortList.Count();
}

template <typename CAllocator>
bool CSmallBlockPool<CAllocator>::RemovePagesFromFreeList(byte **pPages,
                                                          int nPages,
                                                          bool bSortList) {
  // Since we don't use the depth of the tslist, and sequence is only used for
  // push, we can remove in-place
  int i;
  ...
  if (!bSortList || m_nCommittedPages - nPages == 1) {
    ...
    for (i = 0; i < nSortPages; i++) {
      while (
          (pNode = pSharedData->m_PageStatus[sortPages[i]].m_SortList.Pop()) !=
          NULL) {
...
size_t CSmallBlockPool<CAllocator>::Compact(bool bIncremental) {
  ...
  if (m_pFirstPage) {
    PageStatus_t **pReleasedPages =
        (PageStatus_t **)stackalloc(m_nCommittedPages * sizeof(PageStatus_t *));
    PageStatus_t **pReleasedPagesPrevs =
        (PageStatus_t **)stackalloc(m_nCommittedPages * sizeof(PageStatus_t *));
    ...
    while (pPage) {
      if (pPage->m_nAllocated == 0) {
        ...
        pPageBases[nPages] =
            pSharedData->m_pBase +
            (pPage - &pSharedData->m_PageStatus[0]) * BYTES_PAGE;
    ...
    if (nPages) {
      ...
      if (!RemovePagesFromFreeList(pPageBases, nPages, !bIncremental) &&
          !bWarnedCorruption) {
      ...
      for (i = nPages - 1; i >= 0; --i) {
        if (pReleasedPagesPrevs[i]) {
          pReleasedPagesPrevs[i]->m_pNextPageInPool =
              pReleasedPages[i]->m_pNextPageInPool;
        } else {
      ...
      if (!pSharedData->m_Allocator.IsVirtual()) {
        ...
        int nMinReserve = (bIncremental) ? CAllocator::MIN_RESERVE_PAGES * 8
                                         : CAllocator::MIN_RESERVE_PAGES;
        ...
        if (nPages) {
          // Detach the list, push the decommitted page on, iterate up to
          // previous decommits, but them on, then push the committed pages on
          ...
          while (pCur) {
            if (((PageStatus_t *)pCur)->m_nAllocated == NOT_COMMITTED) {
              if (pTemp) {
                ...
                pNodes = NULL;  // The list only has decommitted pages, don't go
                                // circular
              }
  ...
  if (bIsCorrupt) {
    Warning(
        "***** HEAP IS CORRUPT (free compromised for block size %d,in %s heap, "
        "possible write after free *****)\n",
        m_nBlockSize,
        (pSharedData->m_Allocator.IsVirtual()) ? "virtual" : "physical");
  }
  return nFreed;
}
...
bool CSmallBlockPool<CAllocator>::Validate() {
  ...
  byte **pPageBases = (byte **)stackalloc(m_nCommittedPages * sizeof(byte *));
  unsigned *pageCounts =
      (unsigned *)stackalloc(m_nCommittedPages * sizeof(unsigned));
  ...
  if (!m_pFirstPage) {
    ...
    PageStatus_t *pPage = m_pFirstPage;
    while (pPage) {
      pPageBases[nPages] = pSharedData->m_pBase +
                           (pPage - &pSharedData->m_PageStatus[0]) * BYTES_PAGE;
      ...
      if (m_pNextAlloc >= pPageBases[nPages] &&
          m_pNextAlloc < pPageBases[nPages] + BYTES_PAGE) {
        freeNotInFreeList = pageCounts[nPages] =
            ((pPageBases[nPages] + BYTES_PAGE) - m_pNextAlloc) / m_nBlockSize;
      }
  ...
  // Validate block counts
  {
    ...
    while (pNode) {
      for (i = 0; i < nPages; i++) {
        if ((byte *)pNode >= pPageBases[i] &&
            (byte *)pNode < pPageBases[i] + BYTES_PAGE) {
          pageCounts[i]++;
          break;
        }
      }
...
void *CSmallBlockHeap<CAllocator>::Realloc(void *p, size_t nBytes) {
  ...
  if (!pNewBlock) {
    pNewBlock =
        MemAlloc_Alloc(nBytes);  // Call back out so blocks can move from the
                                 // secondary to the primary pools
  }
...
void CSmallBlockHeap<CAllocator>::Free(void *p) {
  ...
  if (pPool) {
    ...
    // we probably didn't hook some allocation and now we're freeing it or the
    // heap has been trashed!
    DebuggerBreakIfDebugging();
  }
}
...
template <typename CAllocator>
void CSmallBlockHeap<CAllocator>::Usage(size_t &bytesCommitted,
...
void CSmallBlockHeap<CAllocator>::DumpStats(const char *pszTag, FILE *pFile) {
  ...
  if (pFile) {
    for (int i = 0; i < NUM_POOLS; i++) {
      ...
      fprintf(pFile,
              "Pool %2i: (size: %4u) blocks: allocated:%5i free:%5i "
              "committed:%5i (committed size:%4u kb)\n",
    ...
    for (int i = 0; i < NUM_POOLS; i++) {
      Msg("Pool %2i: (size: %4u) blocks: allocated:%5i free:%5i committed:%5i "
          "(committed size:%4u kb)\n",
          i, m_Pools[i].GetBlockSize(), m_Pools[i].CountAllocatedBlocks(),
          m_Pools[i].CountFreeBlocks(), m_Pools[i].CountCommittedBlocks(),
...
template <typename CAllocator>
CSmallBlockPool<CAllocator> *CSmallBlockHeap<CAllocator>::FindPool(
    size_t nBytes) {
...
bool g_bUsingLMD = (Plat_GetCommandLineA())
                       ? (strstr(Plat_GetCommandLineA(), "-uselmd") != NULL)
                       : false;
...
AllocHeader_t **g_pRecentFrees =
    (AllocHeader_t **)calloc(g_nRecentFrees, sizeof(AllocHeader_t *));
int g_iNextFreeSlot;
...
void LMDReportInvalidBlock(AllocHeader_t *pHeader, const char *pszMessage) {
  ...
  if (pHeader) {
    sprintf(szMsg, "HEAP IS CORRUPT: %s (block 0x%x, size %d, alignment %d)\n",
            pszMessage, (size_t)LMDFromHeader(pHeader), pHeader->nBytes,
            pHeader->align);
  } else {
...
void LMDValidateBlock(AllocHeader_t *pHeader, bool bFreeList) {
  ...
  if (memcmp(((Sentinal_t *)(LMDFromHeader(pHeader) + pHeader->nBytes)),
...
size_t LMDComputeHeaderSize(size_t align = 0) {
  if (!align) return sizeof(AllocHeader_t);
...
void *LMDNoteAlloc(void *p, size_t nBytes, size_t align = 0,
  ...
  // Some SBH clients rely on allocations > 16 bytes being 16-byte aligned, so
  // we mustn't break that assumption:
...
void *LMDNoteFree(void *p) {
  ...
  if (pToReturn && (pToReturn->align)) {
    // For aligned allocations, the actual system allocation starts *before* the
    // LMD header:
    size_t headerPadding =
        LMDComputeHeaderSize(pToReturn->align) - sizeof(AllocHeader_t);
...
#ifdef MEMALLOC_SUPPORTS_ALIGNED_ALLOCATIONS
INTERNAL_INLINE void *CStdMemAlloc::InternalAllocAligned(int region,
                                                         size_t nSize,
                                                         size_t align) {
...
#ifdef MEMALLOC_SUPPORTS_ALIGNED_ALLOCATIONS
INTERNAL_INLINE void *CStdMemAlloc::InternalReallocAligned(void *pMem,
                                                           size_t nSize,
                                                           size_t align) {
  if (!pMem) {
...
void *CStdMemAlloc::AllocAlign(size_t nSize, size_t align,
  ...
  return LMDNoteAlloc(
      CStdMemAlloc::InternalAllocAligned(DEF_REGION, nAdjustedSize, align),
      nSize, align, pFileName, nLine);
}
#endif  // MEMALLOC_SUPPORTS_ALIGNED_ALLOCATIONS

void *CStdMemAlloc::Realloc(void *pMem, size_t nSize, const char *pFileName,
                            int nLine) {
  if (UsingLMD()) return LMDRealloc(pMem, nSize, 0, pFileName, nLine);
  return CStdMemAlloc::InternalRealloc(pMem, nSize);
}
...
void *CStdMemAlloc::ReallocAlign(void *pMem, size_t nSize, size_t align,
                                 const char *pFileName, int nLine) {
  if (UsingLMD()) return LMDRealloc(pMem, nSize, align, pFileName, nLine);
  return CStdMemAlloc::InternalReallocAligned(pMem, nSize, align);
}
#endif  // MEMALLOC_SUPPORTS_ALIGNED_ALLOCATIONS
...
void *CStdMemAlloc::RegionAlloc(int region, size_t nSize) {
  size_t nAdjustedSize = LMDAdjustSize(nSize);
  return LMDNoteAlloc(CStdMemAlloc::InternalAlloc(region, nAdjustedSize),
                      nSize);
}
...
void CStdMemAlloc::DumpStatsFileBase(char const *pchFileBase) {
  ...
  if (pFile)
    ...
    Msg("\nFallback SBH:\n");
  m_FallbackSBH.DumpStats("Fallback SBH",
                          pFile);  // Dump statistics to small block heap
#endif                             // MEMALLOC_NO_FALLBACK
#endif                             // MEM_SBH_ENABLED
  ...
  Msg("PS3 malloc_stats: %u / %u / %u \n", mms.current_inuse_size,
      mms.current_system_size, mms.max_system_size);
#endif  // _PS3
...
void CStdMemAlloc::GlobalMemoryStatus(size_t *pUsedMemory,
                                      size_t *pFreeMemory) {
  ...
  //  - the first region is supposed to service large allocations via virtual
  //  allocation, and to grow as
  //    needed (until all physical pages are used), so true 'out of memory'
  //    failures should occur there.
  //  - other regions (the 2-256kb 'medium block heap', or per-DLL heaps, and
  //  the Small Block Heap)
  //    are sized to a pre-determined high watermark, and not intended to grow.
  //    Free memory within those regions is not available for large allocations,
  //    so adding that to the 'free memory' yields confusing data which does not
  //    correspond well with out-of-memory failures.
  ...
  *pFreeMemory += dlMallocFree;
  // Adjust free mem to reflect a retail box, even if we're using a devkit with
  // extra memory
  ...
  *pUsedMemory = (stat.dwTotalPhys - 32 * 1024 * 1024) - *pFreeMemory;
  ...
  // NOTE: we use dlmalloc instead of the system heap, so we do NOT count the
  // system heap's free space!
...
int CStdMemAlloc::GetGenericMemoryStats(GenericMemoryStat_t **ppMemoryStats) {
...
#if MEM_SBH_ENABLED
  {
    ...
    SBHCommitted += commitTmp;
    SBHAllocated += allocTmp;
#ifdef MEMALLOC_USE_SECONDARY_SBH
    ...
    static size_t SBHMaxCommitted = 0;
    SBHMaxCommitted = MAX(SBHMaxCommitted, SBHCommitted);
...
#if !defined(MEMALLOC_REGIONS) && defined(MEMALLOC_SEGMENT_MIXED)
  {
    ...
    size_t nMBHCurUsed =
        infoMBH.uordblks;  // nMBH_WRONG_MaxUsed = infoMBH.usmblks; // TODO:
                           // figure out why dlmalloc mis-reports MBH max usage
                           // (it just returns the footprint)
    static size_t nMBHMaxUsed = 0;
    nMBHMaxUsed = MAX(nMBHMaxUsed, nMBHCurUsed);
    ...
    size_t nLBHCurUsed = mspace_footprint(g_AllocRegions[0]),
           nLBHMaxUsed = mspace_max_footprint(g_AllocRegions[0]),
           nLBHArenaSize = infoLBH.arena, nLBHFree = infoLBH.fordblks;
    AddGenericMemoryStat("LBH_cur", (int)nLBHCurUsed);
    AddGenericMemoryStat("LBH_max", (int)nLBHMaxUsed);
    // LBH arena used+free (these are non-virtual allocations - there should be
    // none, since we only allocate 256KB+ items in the LBH)
    // TODO: I currently see the arena grow to 320KB due to a larger allocation
    // being realloced down... if this gets worse, add an 'ALWAYS use VMM' flag
    // to the mspace.
...
#else   // (!MEMALLOC_REGIONS && MEMALLOC_SEGMENT_MIXED)
  {
    ...
    AddGenericMemoryStat("mspace_size",
                         (int)mspace_footprint(g_AllocRegions[0]));
  }
...
#ifdef _PS3
  {
    ...
    extern void VirtualMemoryManager_GetStats(
        size_t & nReserved, size_t & nReservedMax, size_t & nCommitted,
        size_t & nCommittedMax);
    VirtualMemoryManager_GetStats(nReserved, nReservedMax, nCommitted,
                                  nCommittedMax);
    ...
    // Estimate memory committed by memory stacks (these account for all VMM
    ...
    // On PS3, we can more accurately determine 'phys_free_min', since we know
    // nCommittedMax (otherwise nPhysFreeMin is only updated intermittently;
  ...
  // Total/free/min-free physical pages
  {
    ...
    MEMORYSTATUS stat;
    ::GlobalMemoryStatus(&stat);
    size_t nPhysTotal = stat.dwTotalPhys,
           nPhysFree = stat.dwAvailPhys - ExtraDevkitMemory();
#elif defined(_PS3)
    static sys_memory_info stat;
    sys_memory_get_user_memory_size(&stat);
    size_t nPhysTotal = stat.total_user_memory,
           nPhysFree = stat.available_user_memory - ExtraDevkitMemory();
#endif  // _X360/_PS3
...
void CStdMemAlloc::CompactHeap() {
  ...
  if (nBytesRecovered && IsGameConsole()) {
    Msg("Compact freed %d bytes from virtual heap (up to 256k still "
        "committed)\n",
        nBytesRecovered);
  }
#endif  // MEMALLOC_NO_FALLBACK
  ...
  if (IsX360()) {
    ...
    ExecuteOnce({
      ...
      _snprintf(buffer, sizeof(buffer),
                "***** Memory pool overflow, attempted allocation size: %u "
