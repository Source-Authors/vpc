...
CVProfNode *CVProfNode::GetSubNode(const tchar *pszName, int detailLevel,
    ...
    if ((m_iBitFlags & kCPUTrace) != 0) {
      ...
      switch (g_VProfCurrentProfile.GetCPUTraceMode()) {
        ...
        case CVProfile::kAllNodesInFrame_RecordingMultiFrame:
          ...
          if (!XTraceStartRecording(
                  g_VProfCurrentProfile.GetCPUTraceFilename())) {
    ...
    if ((m_iBitFlags & kCPUTrace) != 0) {
      ...
      switch (g_VProfCurrentProfile.GetCPUTraceMode()) {
        ...
        case CVProfile::kAllNodesInFrame_RecordingMultiFrame: {
          // one-off recording. stop now.
          if (XTraceStopRecording()) {
            if (g_VProfCurrentProfile.GetCPUTraceMode() ==
                CVProfile::kAllNodesInFrame_RecordingMultiFrame) {
              Msg("%.3f msec in %s\n", m_CurFrameTime.GetMillisecondsF(),
                  g_VProfCurrentProfile.GetCPUTraceFilename());
            } else {
              Msg("CPU trace finished.\n");
            }
...
#if 0  // This doesn't want to work on the xbox360-- MoveFile not available or
       // file still being put down to disk?
				char suffix[ 32 ];
...
void CVProfile::SumTimes(CVProfNode *pNode, int budgetGroupID) {
  ...
  if (GetRoot() != pNode) {
    if (g_pStartNode && pNode->GetTotalCalls() > 0 &&
      ...
      iter = g_TimeSumsMap.find(
          pNode->GetName());  // intenionally using address of string rather
                              // than string compare (toml 01-27-03)
  ...
  if (bSetStartNode) g_pStartNode = NULL;
}
...
// recursively set l2/lhs recording state for a node and all children AND
// SIBLINGS
...
const char *CVProfile::GetCPUTraceFilename() {
  ...
  switch (m_iCPUTraceEnabled) {
    ...
    case kAllNodesInFrame_Recording:
      _snprintf(retBuf, sizeof(retBuf), "e:\\%.128s%.4d.pix2",
                m_CPUTraceFilename, m_iSuccessiveTraceIndex);
      break;

    case kAllNodesInFrame_WaitingForMarkMultiFrame:
    case kAllNodesInFrame_RecordingMultiFrame:
      _snprintf(retBuf, sizeof(retBuf), "e:\\%.128s_%.4d_%.4d.pix2",
                m_CPUTraceFilename, m_nFrameCount, m_iSuccessiveTraceIndex);
      break;
...
CVProfNode *CVProfile::CPUTraceEnableForNode(const tchar *pszNodeName) {
  // disable whatever may be enabled already (we can only trace one node at a
  // time)
...
void CVProfile::DumpNodes(CVProfNode *pNode, int indent,
  ...
  if (fIsRoot || pNode == g_pStartNode) {
    if (bAverageAndCountOnly) {
      ...
      Msg(_T("       Sum (ms)            Avg Time/Frame (ms)     Avg ")
          _T("Time/Call (ms)\n"));
      Msg(_T("[ func+child      func ]  [ func+child   func ]  [ func+child   ")
          _T("func ]    Count   Peak\n"));
      Msg(_T("  ---------- ---------      ---------- ------      ---------- ")
          _T("------   -------- ------\n"));
    }
  ...
  if (!fIsRoot) {
    map<CVProfNode *, double>::iterator iterTimeLessChildren =
        g_TimesLessChildren.find(pNode);
    ...
    if (bAverageAndCountOnly) {
      Msg(_T("  %10.3f %9.2f      %8u"),
          (pNode->GetTotalCalls() > 0)
              ...
              : 0,
          (pNode->GetTotalCalls() > 0) ? dNodeTime / (double)NumFramesSampled()
                                       : 0,
          pNode->GetTotalCalls());
    } else {
      Msg(_T("  %10.3f %9.2f      %10.3f %6.2f      %10.3f %6.2f   %8u %6.2f"),
          ...
          (pNode->GetTotalCalls() > 0) ? dNodeTime / (double)NumFramesSampled()
                                       : 0,
          (pNode->GetTotalCalls() > 0)
              ? pNode->GetTotalTime() / (double)pNode->GetTotalCalls()
              : 0,
          (pNode->GetTotalCalls() > 0)
              ? dNodeTime / (double)pNode->GetTotalCalls()
              : 0,
...
static void CalcBudgetGroupTimes_Recursive(CVProfNode *pNode,
                                           unsigned int *groupTimes,
                                           int numGroups, float flScale) {
  int groupID;
  CVProfNode *nodePtr;
...
static void CalcBudgetGroupL2CacheMisses_Recursive(CVProfNode *pNode,
                                                   unsigned int *groupTimes,
                                                   int numGroups,
                                                   float flScale) {
  int groupID;
  ...
  nodePtr = pNode->GetSibling();
  if (nodePtr) {
    CalcBudgetGroupL2CacheMisses_Recursive(nodePtr, groupTimes, numGroups,
                                           flScale);
  }
...
static void CalcBudgetGroupLHS_Recursive(CVProfNode *pNode,
                                         unsigned int *groupTimes,
                                         int numGroups, float flScale) {
  int groupID;
  CVProfNode *nodePtr;
...
void CVProfile::VXConsoleReportScale(VXConsoleReportMode_t mode,
                                     float flScale) {
...
void CVProfile::VXProfileStart() {
  ...
  if (m_UpdateMode &
      (VPROF_UPDATE_TEXTURE_GLOBAL | VPROF_UPDATE_TEXTURE_PERFRAME)) {
    // update texture profiling
    numGroups = 0;
    counterGroup = (m_UpdateMode & VPROF_UPDATE_TEXTURE_GLOBAL)
    ...
    if (pNode && pNode->GetChild()) {
      switch (m_ReportMode) {
        ...
        case VXCONSOLE_REPORT_TIME:
          CalcBudgetGroupTimes_Recursive(pNode->GetChild(), groupData,
                                         numGroups,
                                         m_pReportScale[VXCONSOLE_REPORT_TIME]);
          break;

        case VXCONSOLE_REPORT_L2CACHE_MISSES:
          CalcBudgetGroupL2CacheMisses_Recursive(
              pNode->GetChild(), groupData, numGroups,
              m_pReportScale[VXCONSOLE_REPORT_L2CACHE_MISSES]);
          break;

        case VXCONSOLE_REPORT_LOAD_HIT_STORE:
          CalcBudgetGroupLHS_Recursive(
              pNode->GetChild(), groupData, numGroups,
              m_pReportScale[VXCONSOLE_REPORT_LOAD_HIT_STORE]);
          break;
      }
    }
...
static void VXBuildNodeList_r(CVProfNode *pNode, xVProfNodeItem_t *pNodeList,
                              int *pNumNodes) {
  if (!pNode) {
    return;
  }
  ...
  pNodeList[*pNumNodes].pBudgetGroupName =
      g_VProfCurrentProfile.GetBudgetGroupName(pNode->GetBudgetGroupID());
  int r, g, b, a;
  g_VProfCurrentProfile.GetBudgetGroupColor(pNode->GetBudgetGroupID(), r, g, b,
                                            a);
...
void CVProfile::VXSendNodes(void) {
  ...
  xVProfNodeItem_t *pNodeList = (xVProfNodeItem_t *)stackalloc(
      MAX_VPROF_NODES_IN_LIST * sizeof(xVProfNodeItem_t));
  int numNodes = 0;
...
//-------------------------------------
static void DumpSorted(const tchar *pszHeading, double totalTime,
  ...
  Msg(_T("%s\n"), pszHeading);
  Msg(_T("  Scope                                                      Calls ")
      _T("Calls/Frame  Time+Child    Pct        Time    Pct   Avg/Frame    ")
      _T("Avg/Call Avg-NoChild        Peak\n"));
  Msg(_T("  ---------------------------------------------------- ----------- ")
      _T("----------- ----------- ------ ----------- ------ ----------- ")
      _T("----------- ----------- -----------\n"));
  for (i = 0; i < sortedSums.size() && i < (unsigned)maxLen; i++) {
    double avg = (sortedSums[i].calls)
                     ? sortedSums[i].time / (double)sortedSums[i].calls
                     : 0.0;
    double avgLessChildren =
        (sortedSums[i].calls)
            ? sortedSums[i].timeLessChildren / (double)sortedSums[i].calls
            : 0.0;

    Msg(_T("  ")
        _T("%52.52s%12d%12.3f%12.3f%7.2f%12.3f%7.2f%12.3f%12.3f%12.3f%12.3f\n"),
        sortedSums[i].pszProfileScope, sortedSums[i].calls,
        (float)sortedSums[i].calls / (float)g_TotalFrames, sortedSums[i].time,
        ...
        sortedSums[i].time / (float)g_TotalFrames, avg, avgLessChildren,
        sortedSums[i].peak);
  }
}
...
static void DumpPMC(CVProfNode *pNode, bool &bPrintHeader, uint64 L2thresh = 1,
                    uint64 LHSthresh = 1) {
  if (!pNode) return;

  uint64 l2 = pNode->GetL2CacheMisses();
  uint64 lhs = pNode->GetLoadHitStores();
  if (l2 > L2thresh && lhs > LHSthresh) {
    // met threshold.
    if (bPrintHeader) {
      // print header
      Msg(_T("-- 360 PMC information --\n"));
      Msg(_T("Scope                                                  L2/call  ")
          _T("L2/frame  LHS/call LHS/frame\n"));
      Msg(_T("---------------------------------------------------- --------- ")
          _T("--------- --------- ---------\n"));
    ...
    Msg(_T("%52.52s %9.2f %9.2f %9.2f %9.2f\n"), pNode->GetName(), l2 / calls,
        l2 / frames, lhs / calls, lhs / frames);
  }
...
void CVProfile::OutputReport(int type, const tchar *pszStartNode,
                             int budgetGroupID) {
  ...
  if (NumFramesSampled() == 0 || GetTotalTimeSampled() == 0)
    ...
    if (type & VPRT_LIST_BY_TIME) {
      DumpSorted(_T("-- Profile scopes sorted by time (including children) --"),
                 GetTotalTimeSampled(), TimeCompare, maxLen);
      Msg(_T("\n"));
    }
    if (type & VPRT_LIST_BY_TIME_LESS_CHILDREN) {
      DumpSorted(_T("-- Profile scopes sorted by time (without children) --"),
                 GetTotalTimeSampled(), TimeLessChildrenCompare, maxLen);
      Msg(_T("\n"));
    }
    if (type & VPRT_LIST_BY_AVG_TIME) {
      DumpSorted(
          _T("-- Profile scopes sorted by average time (including children) ")
          _T("--"),
    ...
    if (type & VPRT_LIST_BY_AVG_TIME_LESS_CHILDREN) {
      DumpSorted(
          _T("-- Profile scopes sorted by average time (without children) --"),
          GetTotalTimeSampled(), AverageTimeLessChildrenCompare, maxLen);
      Msg(_T("\n"));
    }
    if (type & VPRT_LIST_BY_PEAK_TIME) {
      DumpSorted(_T("-- Profile scopes sorted by peak --"),
                 GetTotalTimeSampled(), PeakCompare, maxLen);
      Msg(_T("\n"));
    }
    if (type & VPRT_LIST_BY_PEAK_OVER_AVERAGE) {
      DumpSorted(
          _T("-- Profile scopes sorted by peak over average (including ")
          _T("children) --"),
...
CVProfile::CVProfile()
  ...
  BudgetGroupNameToBudgetGroupID(VPROF_BUDGETGROUP_MODEL_RENDERING,
                                 BUDGETFLAG_CLIENT);
  BudgetGroupNameToBudgetGroupID(VPROF_BUDGETGROUP_MODEL_FAST_PATH_RENDERING,
                                 BUDGETFLAG_CLIENT);
...
int CVProfile::AddBudgetGroupName(const tchar *pBudgetGroupName,
                                  int budgetFlags) {
  MEM_ALLOC_CREDIT();
  ...
  if (m_nBudgetGroupNames + 1 > m_nBudgetGroupNamesAllocated) {
    ...
    for (int i = 0; i < m_nBudgetGroupNames; i++) pNew[i] = m_pBudgetGroups[i];
...
int *CVProfile::FindOrCreateCounter(const tchar *pName,
...
void CVProfile::LatchMultiFrame(int64 cycles) {
  if (cycles > m_WorstCycles) {
    strncpy(m_WorstTraceFilename, GetCPUTraceFilename(),
            sizeof(m_WorstTraceFilename));
