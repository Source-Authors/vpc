...
// register the providers if possible, and get the performance counter
// frequency.
class CETWRegister {
 public:
  CETWRegister() {
    ...
    if (pAdvapiDLL) {
      ...
      m_pEventRegister =
          (tEventRegister)GetProcAddress(pAdvapiDLL, "EventRegister");
      m_pEventWrite = (tEventWrite)GetProcAddress(pAdvapiDLL, "EventWrite");
      m_pEventUnregister =
          (tEventUnregister)GetProcAddress(pAdvapiDLL, "EventUnregister");

      // Register two ETW providers. If registration fails then the event
      // logging calls will fail. On XP these calls will do nothing. On Vista
      // and above, if these providers have been enabled by xperf or logman then
      // the VALVE_FRAMERATE_Context and VALVE_MAIN_Context globals will be
      // modified like this:
...
ULONG EVNTAPI EventRegister(LPCGUID ProviderId, PENABLECALLBACK EnableCallback,
  ...
  if (g_ETWRegister.m_pEventRegister)
    return g_ETWRegister.m_pEventRegister(ProviderId, EnableCallback,
                                          CallbackContext, RegHandle);
...
ULONG EVNTAPI EventWrite(REGHANDLE RegHandle,
  ...
  if (g_ETWRegister.m_pEventWrite)
    return g_ETWRegister.m_pEventWrite(RegHandle, EventDescriptor,
...
static float QPCToMS(int64 nDelta) {
  ...
  float flSeconds =
      (float)(nDelta / double(g_ETWRegister.m_frequency.QuadPart));
...
void ETWMark4F(const char *pMessage, float data1, float data2, float data3,
               float data4) {
...
void ETWMark4I(const char *pMessage, int data1, int data2, int data3,
               int data4) {
...
int ETWGetRenderFrameNumber() { return s_nRenderFrameCount; }

// Insert a render frame marker using the Valve-FrameRate provider.
// Automatically count the frame number and frame time. Since the frame count
// and elapsed time are tracked without paying attention to the bIsServerProcess
// flag the results will be 'unexpected' if bIsServerProcess changes value
// within a process.
...
// Insert a simulation frame marker using the Valve-FrameRate provider.
// Automatically count the frame number and frame time. Since the frame count
// and elapsed time are tracked without paying attention to the bIsServerProcess
// flag the results will be 'unexpected' if bIsServerProcess changes value
// within a process.
...
void ETWSendPacket(const char *pTo, int nWireSize, int nOutSequenceNR,
                   int nOutSequenceNrAck) {
  static int s_nCumulativeWireSize;
  s_nCumulativeWireSize += nWireSize;

  EventWriteSendPacket(pTo, nWireSize, nOutSequenceNR, nOutSequenceNrAck,
                       s_nCumulativeWireSize);
}

void ETWThrottled() { EventWriteThrottled(); }

void ETWReadPacket(const char *pFrom, int nWireSize, int nInSequenceNR,
                   int nOutSequenceNRAck) {
  static int s_nCumulativeWireSize;
  s_nCumulativeWireSize += nWireSize;

  EventWriteReadPacket(pFrom, nWireSize, nInSequenceNR, nOutSequenceNRAck,
                       s_nCumulativeWireSize);
}
