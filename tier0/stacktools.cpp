...
// where we'll find our PDB's for win32. Translation will not work until this
// has been called once (even if with NULL)
...
int TranslateStackInfo(const void *const *pCallStack, int iCallStackCount,
                       tchar *szOutput, int iOutBufferSize,
                       const tchar *szEntrySeparator,
                       TranslateStackInfo_StyleFlags_t style) {
  if (iOutBufferSize > 0) *szOutput = '\0';
...
bool GetFileAndLineFromAddress(const void *pAddress, tchar *pFileNameOut,
                               int iMaxFileNameLength, uint32 &iLineNumberOut,
                               uint32 *pDisplacementOut) {
  if (iMaxFileNameLength > 0) *pFileNameOut = '\0';

  return false;
}

bool GetSymbolNameFromAddress(const void *pAddress, tchar *pSymbolNameOut,
                              int iMaxSymbolNameLength,
                              uint64 *pDisplacementOut) {
  if (iMaxSymbolNameLength > 0) *pSymbolNameOut = '\0';

  return false;
}

bool GetModuleNameFromAddress(const void *pAddress, tchar *pModuleNameOut,
                              int iMaxModuleNameLength) {
  if (iMaxModuleNameLength > 0) *pModuleNameOut = '\0';
...
class CStackTop_FriendFuncs : public CStackTop_Base {
 public:
  friend int AppendParentStackTrace(void **pReturnAddressesOut, int iArrayCount,
                                    int iAlreadyFilled);
  friend int GetCallStack_Fast(void **pReturnAddressesOut, int iArrayCount,
                               int iSkipCount);
};

inline int AppendParentStackTrace(void **pReturnAddressesOut, int iArrayCount,
                                  int iAlreadyFilled) {
  ...
  if (pTop != NULL) {
    ...
    if (pTop->m_iParentStackTraceLength != 0) {
      int iCopy =
          MIN(iArrayCount - iAlreadyFilled, pTop->m_iParentStackTraceLength);
      memcpy(pReturnAddressesOut + iAlreadyFilled, pTop->m_pParentStackTrace,
             iCopy * sizeof(void *));
...
inline bool ValidStackAddress(void *pAddress, const void *pNoLessThan,
                              const void *pNoGreaterThan) {
  if ((uint64)pAddress & 3) return false;
  if (pAddress < pNoLessThan)  // frame pointer traversal should always increase
                               // the pointer
    return false;
  if (pAddress > pNoGreaterThan)  // never traverse outside the stack (Oh
                                  // 0xCCCCCCCC, how I hate you)
    return false;

#if defined(WIN32) && !defined(_X360) && 1
  if (IsBadReadPtr(pAddress, (sizeof(void *) *
                              2)))  // safety net, but also throws an exception
                                    // (handled internally) to stop bad access
    return false;
#endif
...
int GetCallStack_Fast([[maybe_unused]] void **pReturnAddressesOut,
  ...
  /*
  With frame pointer omission disabled, this should be the pattern all the way
  up the stack [ebp+00]   Old ebp value [ebp+04]   Return address
  */

  void *pNoLessThan = pStackCrawlEBP;  // impossible for a valid stack to
...
typedef BOOL(WINAPI *PFN_SymSetSearchPath)(IN HANDLE hProcess,
                                           IN PSTR SearchPath);
typedef BOOL(WINAPI *PFN_SymInitialize)(IN HANDLE hProcess,
                                        IN PSTR UserSearchPath,
                                        IN BOOL fInvadeProcess);
typedef BOOL(WINAPI *PFN_SymCleanup)(IN HANDLE hProcess);
typedef BOOL(WINAPI *PFN_SymEnumerateModules64)(
    IN HANDLE hProcess, IN PSYM_ENUMMODULES_CALLBACK64 EnumModulesCallback,
    IN PVOID UserContext);
typedef BOOL(WINAPI *PFN_EnumerateLoadedModules64)(
    IN HANDLE hProcess,
    IN PENUMLOADED_MODULES_CALLBACK64 EnumLoadedModulesCallback,
    IN PVOID UserContext);
typedef DWORD64(WINAPI *PFN_SymLoadModule64)(IN HANDLE hProcess,
                                             IN HANDLE hFile, IN PSTR ImageName,
                                             IN PSTR ModuleName,
                                             IN DWORD64 BaseOfDll,
                                             IN DWORD SizeOfDll);
typedef BOOL(WINAPI *PFN_SymUnloadModule64)(IN HANDLE hProcess,
                                            IN DWORD64 BaseOfDll);
typedef BOOL(WINAPI *PFN_SymFromAddr)(IN HANDLE hProcess, IN DWORD64 Address,
                                      OUT PDWORD64 Displacement,
                                      IN OUT PSYMBOL_INFO Symbol);
typedef BOOL(WINAPI *PFN_SymGetLineFromAddr64)(IN HANDLE hProcess,
                                               IN DWORD64 qwAddr,
                                               OUT PDWORD pdwDisplacement,
                                               OUT PIMAGEHLP_LINE64 Line64);
typedef BOOL(WINAPI *PFN_SymGetModuleInfo64)(IN HANDLE hProcess,
                                             IN DWORD64 dwAddr,
                                             OUT PIMAGEHLP_MODULE64 ModuleInfo);
typedef BOOL(WINAPI *PFN_StackWalk64)(
...
typedef USHORT(WINAPI *PFN_CaptureStackBackTrace)(
    IN ULONG FramesToSkip, IN ULONG FramesToCapture, OUT PVOID *BackTrace,
...
class CHelperFunctionsLoader {
 public:
  ...
  ~CHelperFunctionsLoader(void) {
    ...
    if (m_hDbgHelpDll != NULL) ::FreeLibrary(m_hDbgHelpDll);

#if defined(USE_CAPTURESTACKBACKTRACE)
    if (m_hNTDllDll != NULL) ::FreeLibrary(m_hNTDllDll);
#endif
...
#if _MSC_VER >= 1600
  static BOOL CALLBACK LoadSymbolsCallback(PCSTR ModuleName, DWORD64 ModuleBase,
                                           ULONG ModuleSize, PVOID UserContext)
#else
  static BOOL CALLBACK LoadSymbolsCallback(PSTR ModuleName, DWORD64 ModuleBase,
                                           ULONG ModuleSize, PVOID UserContext)
#endif
  {
    const CHelperFunctionsLoader *pThis =
        ((CHelperFunctionsLoader *)UserContext);
    // SymLoadModule64( IN HANDLE hProcess, IN HANDLE hFile, IN PSTR ImageName,
    // IN PSTR ModuleName, IN DWORD64 BaseOfDll, IN DWORD SizeOfDll );
    pThis->m_pSymLoadModule64(pThis->m_hProcess, NULL, (PSTR)ModuleName,
                              (PSTR)ModuleName, ModuleBase, ModuleSize);
    return TRUE;
  }
  ...
  void TryLoadingNewSymbols(void) {
    ...
    if (m_bIsInitialized) {
      // m_pSymEnumerateModules64( m_hProcess, UnloadSymbolsCallback, this );
      // //unloaded modules we've already loaded
      m_pEnumerateLoadedModules64(m_hProcess, LoadSymbolsCallback,
                                  this);  // load everything
      m_bShouldReloadSymbols = false;
    }
  }

  void SetStackTranslationSymbolSearchPath(
      const char *szSemicolonSeparatedList) {
    AUTO_LOCK_FM(m_Mutex);
    ...
    // re-init search paths. Or if we haven't yet loaded dbghelp.dll, this will
    // go to the dummy function and do nothing
  ...
  bool GetSymbolNameFromAddress(const void *pAddress, tchar *pSymbolNameOut,
    ...
    if (m_pSymFromAddr(m_hProcess, (DWORD64)pAddress, &dwDisplacement,
                       (PSYMBOL_INFO)genericbuffer)) {
      strncpy(pSymbolNameOut, ((PSYMBOL_INFO)genericbuffer)->Name,
              iMaxSymbolNameLength);
      if (pDisplacementOut != NULL) *pDisplacementOut = dwDisplacement;
      return true;
    }
  ...
  bool GetFileAndLineFromAddress(const void *pAddress, tchar *pFileNameOut,
    ...
    if (m_pSymGetLineFromAddr64(m_hProcess, (DWORD64)pAddress, &dwDisplacement,
                                &imageHelpLine64)) {
  ...
  bool TranslatePointer(const void *const pAddress, tchar *pTranslationOut,
                        int iTranslationBufferLength,
                        TranslateStackInfo_StyleFlags_t style) {
    // AUTO_LOCK( m_Mutex );

    if (pTranslationOut == NULL) return false;
    ...
    *pWrite = '\0';
    ...
    if (style & TSISTYLEFLAG_MODULENAME) {
      if (!this->GetModuleNameFromAddress(pAddress, pWrite,
      ...
      if (iTranslationBufferLength < 2) return false;  // need more buffer
    ...
    // use symbol name to test if the rest is going to work. So grab it whether
    // they want it or not
    if (!this->GetSymbolNameFromAddress(pAddress, pWrite,
                                        iTranslationBufferLength, NULL)) {
      int nBytesWritten =
          _snprintf(pWrite, iTranslationBufferLength, "0x%p", pAddress);
      if (nBytesWritten < 0) {
        *pWrite = '\0';  // if we can't write all of the line/lineandoffset,
                         // don't write any at all
        return false;
      }
      return true;
    ...
    if (style & (TSISTYLEFLAG_FULLPATH | TSISTYLEFLAG_SHORTPATH |
                 TSISTYLEFLAG_LINE | TSISTYLEFLAG_LINEANDOFFSET)) {
      if (pWrite !=
          pTranslationOut)  // if we've written anything yet, separate the
                            // printed data from the file name and line
      {
        if (iTranslationBufferLength < 6) return false;  // need more buffer
      ...
      if (this->GetFileAndLineFromAddress(pAddress, szFileName, MAX_PATH, iLine,
                                          &iDisplacement)) {
        if (style & TSISTYLEFLAG_FULLPATH) {
          iLength = (int)strlen(szFileName);
          if (iTranslationBufferLength < iLength + 1) return false;
        ...
        if (style & (TSISTYLEFLAG_LINE | TSISTYLEFLAG_LINEANDOFFSET)) {
          int nBytesWritten = _snprintf(
              pWrite, iTranslationBufferLength,
              ((style & TSISTYLEFLAG_LINEANDOFFSET) && (iDisplacement != 0))
                  ? "(%d) + %d bytes"
                  : "(%d)",
              iLine, iDisplacement);
          if (nBytesWritten < 0) {
  ...
  void EnsureReady(void) {
    if (m_bIsInitialized) {
      if (m_bShouldReloadSymbols) TryLoadingNewSymbols();
    ...
    // .lib, and that we can easily change it to using our own dll when this
    // code is used on win98/ME/2K machines
    m_hDbgHelpDll = ::LoadLibrary("DbgHelp.dll");
    if (!m_hDbgHelpDll) {
      // it's possible it's just way too early to initialize (as shown with
      // attempts at using these tools in the memory allocator)
      if (m_szPDBSearchPath ==
          NULL)  // not a rock solid check, but pretty good compromise between
                 // endless failing initialization and general failure due to
                 // trying too early
        m_bIsInitialized = false;
    ...
    m_pSymInitialize =
        (PFN_SymInitialize)::GetProcAddress(m_hDbgHelpDll, "SymInitialize");
    ...
    m_pSymCleanup =
        (PFN_SymCleanup)::GetProcAddress(m_hDbgHelpDll, "SymCleanup");
    if (m_pSymCleanup == NULL) m_pSymCleanup = SymCleanup_DummyFn;

    m_pSymGetOptions =
        (PFN_SymGetOptions)::GetProcAddress(m_hDbgHelpDll, "SymGetOptions");
    if (m_pSymGetOptions == NULL) m_pSymGetOptions = SymGetOptions_DummyFn;

    m_pSymSetOptions =
        (PFN_SymSetOptions)::GetProcAddress(m_hDbgHelpDll, "SymSetOptions");
    if (m_pSymSetOptions == NULL) m_pSymSetOptions = SymSetOptions_DummyFn;

    m_pSymSetSearchPath = (PFN_SymSetSearchPath)::GetProcAddress(
        m_hDbgHelpDll, "SymSetSearchPath");
    if (m_pSymSetSearchPath == NULL)
      m_pSymSetSearchPath = SymSetSearchPath_DummyFn;

    m_pSymEnumerateModules64 = (PFN_SymEnumerateModules64)::GetProcAddress(
        m_hDbgHelpDll, "SymEnumerateModules64");
    if (m_pSymEnumerateModules64 == NULL)
      m_pSymEnumerateModules64 = SymEnumerateModules64_DummyFn;

    m_pEnumerateLoadedModules64 =
        (PFN_EnumerateLoadedModules64)::GetProcAddress(
            m_hDbgHelpDll, "EnumerateLoadedModules64");
    if (m_pEnumerateLoadedModules64 == NULL)
      m_pEnumerateLoadedModules64 = EnumerateLoadedModules64_DummyFn;

    m_pSymLoadModule64 =
        (PFN_SymLoadModule64)::GetProcAddress(m_hDbgHelpDll, "SymLoadModule64");
    if (m_pSymLoadModule64 == NULL)
      m_pSymLoadModule64 = SymLoadModule64_DummyFn;

    m_pSymUnloadModule64 = (PFN_SymUnloadModule64)::GetProcAddress(
        m_hDbgHelpDll, "SymUnloadModule64");
    if (m_pSymUnloadModule64 == NULL)
      m_pSymUnloadModule64 = SymUnloadModule64_DummyFn;

    m_pSymFromAddr =
        (PFN_SymFromAddr)::GetProcAddress(m_hDbgHelpDll, "SymFromAddr");
    if (m_pSymFromAddr == NULL) m_pSymFromAddr = SymFromAddr_DummyFn;

    m_pSymGetLineFromAddr64 = (PFN_SymGetLineFromAddr64)::GetProcAddress(
        m_hDbgHelpDll, "SymGetLineFromAddr64");
    if (m_pSymGetLineFromAddr64 == NULL)
      m_pSymGetLineFromAddr64 = SymGetLineFromAddr64_DummyFn;

    m_pSymGetModuleInfo64 = (PFN_SymGetModuleInfo64)::GetProcAddress(
        m_hDbgHelpDll, "SymGetModuleInfo64");
    if (m_pSymGetModuleInfo64 == NULL)
      m_pSymGetModuleInfo64 = SymGetModuleInfo64_DummyFn;

#if defined(USE_STACKWALK64)
    m_pStackWalk64 =
        (PFN_StackWalk64)::GetProcAddress(m_hDbgHelpDll, "StackWalk64");
    if (m_pStackWalk64 == NULL) m_pStackWalk64 = StackWalk64_DummyFn;
#endif

#if defined(USE_CAPTURESTACKBACKTRACE)
    m_hNTDllDll = ::LoadLibrary("ntdll.dll");

    m_pCaptureStackBackTrace = (PFN_CaptureStackBackTrace)::GetProcAddress(
        m_hNTDllDll, "RtlCaptureStackBackTrace");
    if (m_pCaptureStackBackTrace == NULL)
      m_pCaptureStackBackTrace = CaptureStackBackTrace_DummyFn;
#endif

    m_pSymSetOptions(
        m_pSymGetOptions() | SYMOPT_DEFERRED_LOADS |  // load on demand
...
#if defined(USE_STACKWALK64)  // most reliable method thanks to boatloads of
                              // windows helper functions. Also the slowest.
int CrawlStack_StackWalk64(CONTEXT *pExceptionContext,
                           void **pReturnAddressesOut, int iArrayCount,
                           int iSkipCount) {
...
#error Unknown CPU arch.
  ...
  int i;
  for (i = 0; i != iSkipCount; ++i)  // skip entries that the requesting
                                     // function thinks are uninformative
  {
    if (!s_HelperFunctions.m_pStackWalk64(
            STACKWALK64_MACHINETYPE, s_HelperFunctions.m_hProcess, hThread,
            &sfFrame, &currentContext, NULL, NULL, NULL, NULL) ||
...
void GetCallStackReturnAddresses_Exception(
    void **CallStackReturnAddresses, int *pRetCount, int iSkipCount,
    _EXCEPTION_POINTERS *pExceptionInfo) {
  int iCount = CrawlStack_StackWalk64(
      pExceptionInfo->ContextRecord, CallStackReturnAddresses, *pRetCount,
...
#if defined(USE_CAPTURESTACKBACKTRACE)
  if (s_HelperFunctions.m_pCaptureStackBackTrace !=
      CaptureStackBackTrace_DummyFn) {
    // docs state a total limit of 63 back traces between skipped and stored
    int iRetVal = s_HelperFunctions.m_pCaptureStackBackTrace(
        iSkipCount, MIN(iArrayCount, 63 - iSkipCount), pReturnAddressesOut,
        NULL);
  ...
  if (s_HelperFunctions.m_pStackWalk64 != StackWalk64_DummyFn) {
    ...
    __try {
      ::RaiseException(0, EXCEPTION_NONCONTINUABLE, 0, NULL);
                    ...
                    pReturnAddressesOut, &iInOutArrayCount, iSkipCount,
                    GetExceptionInformation()),
                EXCEPTION_EXECUTE_HANDLER) {
      return AppendParentStackTrace(pReturnAddressesOut, iArrayCount,
                                    iInOutArrayCount);
    }
  }
#endif
...
void SetStackTranslationSymbolSearchPath(const char *szSemicolonSeparatedList) {
  s_HelperFunctions.SetStackTranslationSymbolSearchPath(
...
int TranslateStackInfo(const void *const *pCallStack, int iCallStackCount,
  ...
  if (szEntrySeparator == NULL) szEntrySeparator = _T("");

  int iSeparatorSize = (int)strlen(szEntrySeparator);

  for (int i = 0; i < iCallStackCount; ++i) {
    if (!s_HelperFunctions.TranslatePointer(pCallStack[i], szOutput,
                                            iOutBufferSize, style)) {
      return i;
    }
  ...
  szOutput -= iSeparatorSize;
  if (szOutput >= szStartOutput) *szOutput = '\0';
...
bool GetFileAndLineFromAddress(const void *pAddress, tchar *pFileNameOut,
  ...
  return s_HelperFunctions.GetFileAndLineFromAddress(
      pAddress, pFileNameOut, iMaxFileNameLength, iLineNumberOut,
      pDisplacementOut);
}
...
bool GetSymbolNameFromAddress(const void *pAddress, tchar *pSymbolNameOut,
                              ...
                              uint64 *pDisplacementOut) {
  s_HelperFunctions.EnsureReady();
  return s_HelperFunctions.GetSymbolNameFromAddress(
      pAddress, pSymbolNameOut, iMaxSymbolNameLength, pDisplacementOut);
}

bool GetModuleNameFromAddress(const void *pAddress, tchar *pModuleNameOut,
                              int iMaxModuleNameLength) {
  s_HelperFunctions.EnsureReady();
  return s_HelperFunctions.GetModuleNameFromAddress(pAddress, pModuleNameOut,
...
class C360StackTranslationHelper {
 public:
  C360StackTranslationHelper(void) { m_bInitialized = true; }
 ...
 public:
  inline StackAddressInfo_t *CreateEntry(const FullStackInfo_t &info) {
    std::pair<AddressInfoMapIter_t, bool> retval = m_AddressInfoMap.insert(
        AddressInfoMapEntry_t(info.pAddress, StackAddressInfo_t()));
    ...
    // module
    {
      ...
      StringSet_t::const_iterator iter =
          m_ModuleNameSet.find(info.szModuleName);
  ...
  inline StackAddressInfo_t *FindInfoEntry(const void *pAddress) {
    ...
    if (Iter != m_AddressInfoMap.end()) return &Iter->second;

    return NULL;
  }

  inline int RetrieveStackInfo(const void *const *pAddresses,
                               FullStackInfo_t *pReturnedStructs,
                               int iAddressCount) {
    ...
    pMessageWrite[0] =
        XBX_DBG_BNH_STACKTRANSLATOR;  // have this message handled by stack
                                      // translator handler
    ...
    *(FullStackInfo_t **)pMessageWrite =
        (FullStackInfo_t *)BigDWord((DWORD)pReturnedStructs);
  ...
  inline StackAddressInfo_t *CreateEntry(const void *pAddress) {
    ...
    ReturnedData.szFileName[0] =
        '\0';  // strncpy( ReturnedData.szFileName, "FileUninitialized", sizeof(
               // ReturnedData.szFileName ) );
    ReturnedData.szModuleName[0] =
        '\0';  // strncpy( ReturnedData.szModuleName, "ModuleUninitialized",
               // sizeof( ReturnedData.szModuleName ) );
    ReturnedData.szSymbol[0] =
        '\0';  // strncpy( ReturnedData.szSymbol, "SymbolUninitialized", sizeof(
               // ReturnedData.szSymbol ) );
  ...
  inline void LoadStackInformation(void *const *pAddresses, int iAddressCount) {
    ...
    int iNeedLoading = 0;
    void **pNeedLoading = (void **)stackalloc(
        sizeof(const void *) *
        iAddressCount);  // addresses we need to ask VXConsole about
    ...
    if (iNeedLoading != 0) {
      // ask VXConsole for information about the addresses we're clueless about
      FullStackInfo_t *pReturnedStructs =
          (FullStackInfo_t *)stackalloc(sizeof(FullStackInfo_t) * iNeedLoading);
      ...
      int iTranslated =
          RetrieveStackInfo(pNeedLoading, pReturnedStructs, iNeedLoading);
  ...
  inline bool GetFileAndLineFromAddress(const void *pAddress,
                                        tchar *pFileNameOut,
                                        int iMaxFileNameLength,
                                        uint32 &iLineNumberOut,
                                        uint32 *pDisplacementOut) {
  ...
  inline bool GetSymbolNameFromAddress(const void *pAddress,
                                       tchar *pSymbolNameOut,
                                       int iMaxSymbolNameLength,
                                       uint64 *pDisplacementOut) {
    StackAddressInfo_t *pInfo = FindOrCreateEntry(pAddress);
    if (pInfo && (pInfo->szSymbol[0] != '\0')) {
      ...
      if (pDisplacementOut) *pDisplacementOut = pInfo->iSymbolOffset;
  ...
  inline bool GetModuleNameFromAddress(const void *pAddress,
...
#pragma pack(pop)
  ...
  typedef std::map<const void *, StackAddressInfo_t, std::less<const void *>>
      AddressInfoMap_t;
...
int GetCallStack(void **pReturnAddressesOut, int iArrayCount, int iSkipCount) {
  ...
  // DmCaptureStackBackTrace() has no skip functionality, so we need to grab
  // everything and skip within that list
  void **pAllResults =
      (void **)stackalloc(sizeof(void *) * (iArrayCount + iSkipCount));
  if (DmCaptureStackBackTrace(iArrayCount + iSkipCount, pAllResults) ==
      XBDM_NOERR) {
    for (int i = 0; i != iSkipCount; ++i) {
      if (*pAllResults ==
          NULL)  // DmCaptureStackBackTrace() NULL terminates the list instead
                 // of telling us how many were returned
        return AppendParentStackTrace(pReturnAddressesOut, iArrayCount, 0);

      ++pAllResults;  // move the pointer forward so the second loop indices
                      // match up
    }
...
int TranslateStackInfo(const void *const *pCallStack, int iCallStackCount,
  ...
  if (iCallStackCount == 0) {
    if (iOutBufferSize > 1) *szOutput = '\0';

    return 0;
  }

  if (szEntrySeparator == NULL) szEntrySeparator = "";

  int iSeparatorLength = strlen(szEntrySeparator) + 1;
  int iDataSize = (sizeof(void *) * iCallStackCount) +
                  (iSeparatorLength * sizeof(tchar)) + 1;  // 1 for style flags

  // 360 is incapable of translation on it's own. Encode the stack for
  // translation in VXConsole Encoded section is as such ":CSDECODE[encoded
  // binary]"
  int iEncodedSize = -EncodeBinaryToString(NULL, iDataSize, NULL,
                                           0);  // get needed buffer size
  const tchar cControlPrefix[] = XBX_CALLSTACKDECODEPREFIX;
  const size_t cControlLength = (sizeof(cControlPrefix) / sizeof(tchar)) -
                                1;  //-1 to remove null terminator
      ...
      (iEncodedSize + (int)cControlLength + 2))  //+2 for ']' and null term
  {
    ...
    memcpy(pData + 1 + (iSeparatorLength * sizeof(tchar)), pCallStack,
           iCallStackCount * sizeof(void *));

    memcpy(szOutput, XBX_CALLSTACKDECODEPREFIX, cControlLength * sizeof(tchar));
    int iLength =
        cControlLength +
        EncodeBinaryToString(pData, iDataSize, &szOutput[cControlLength],
                             (iOutBufferSize - cControlLength));
...
bool GetFileAndLineFromAddress(const void *pAddress, tchar *pFileNameOut,
                               ...
                               uint32 *pDisplacementOut) {
  AUTO_LOCK_FM(s_360StackTranslator.m_hMutex);
  return s_360StackTranslator.GetFileAndLineFromAddress(
      pAddress, pFileNameOut, iMaxFileNameLength, iLineNumberOut,
      pDisplacementOut);
}
...
bool GetSymbolNameFromAddress(const void *pAddress, tchar *pSymbolNameOut,
                              ...
                              uint64 *pDisplacementOut) {
  AUTO_LOCK_FM(s_360StackTranslator.m_hMutex);
  return s_360StackTranslator.GetSymbolNameFromAddress(
      pAddress, pSymbolNameOut, iMaxSymbolNameLength, pDisplacementOut);
}
...
bool GetModuleNameFromAddress(const void *pAddress, tchar *pModuleNameOut,
                              int iMaxModuleNameLength) {
  AUTO_LOCK_FM(s_360StackTranslator.m_hMutex);
  return s_360StackTranslator.GetModuleNameFromAddress(pAddress, pModuleNameOut,
                                                       iMaxModuleNameLength);
}
...
CStackTop_CopyParentStack::CStackTop_CopyParentStack(
    void *const *pParentStackTrace, int iParentStackTraceLength) {
  ...
  if ((pParentStackTrace != NULL) && (iParentStackTraceLength > 0)) {
    while ((iParentStackTraceLength > 0) &&
           (pParentStackTrace[iParentStackTraceLength - 1] == NULL)) {
      --iParentStackTraceLength;
    }

    if (iParentStackTraceLength > 0) {
      m_pParentStackTrace = new void *[iParentStackTraceLength];
      memcpy((void **)m_pParentStackTrace, pParentStackTrace,
             sizeof(void *) * iParentStackTraceLength);
    }
  }
...
int EncodeBinaryToString(const void *pToEncode, int iDataLength,
  ...
  iEncodedSize +=
      (iEncodedSize + 6) / 7;  // Have 1 control byte for every 7 actual bytes
  iEncodedSize += sizeof(uint32) + 1;  // data size at the beginning of the blob
                                       // and null terminator at the end

  if ((iEncodedSize > iEncodeBufferSize) || (pEncodeOut == NULL) ||
      (pToEncode == NULL))
  ...
  while (pEncodeRead < pEncodeStop) {
    if (iControl == 0) {
      ...
      *pEncodeWrite =
          *pEncodeRead | 0x80;  // encoded data always has the MSB bit set
                                // (cheap avoidance of null terminators)
      *pEncodeWriteLastControlByte |=
          (((*pEncodeRead) & 0x80) ^ 0x80) >>
          iControl;  // We use the control byte to XOR the MSB back to original
                     // values on decode
      ++pEncodeRead;
    }
...
int DecodeBinaryFromString(const char *pString, void *pDestBuffer,
  ...
  if ((pDecodeRead[0] < 0x80) || (pDecodeRead[1] < 0x80) ||
      (pDecodeRead[2] < 0x80) || (pDecodeRead[3] < 0x80)) {
    if (ppParseFinishOut != NULL) *ppParseFinishOut = (char *)pString;
  ...
  iTextLength +=
      (iTextLength + 6) / 7;  // Have 1 control byte for every 7 actual bytes
  ...
  if (iDestBufferSize < iDecodedSize) {
    if (ppParseFinishOut != NULL) *ppParseFinishOut = (char *)pDecodeRead;
