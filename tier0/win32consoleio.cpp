...
bool SetupWin32ConsoleIO() {
  ...
  if (GetFileType(GetStdHandle(STD_OUTPUT_HANDLE)) == FILE_TYPE_UNKNOWN) {
    ...
    pAttachConsole_t pAttachConsole(
        (BOOL(_stdcall *)(DWORD))GetProcAddress(hInst, "AttachConsole"));
    ...
    *stdout = *_fdopen(
        _open_osfhandle(reinterpret_cast<intp>(GetStdHandle(STD_OUTPUT_HANDLE)),
                        _O_TEXT),
        "w");
    setvbuf(stdout, NULL, _IONBF, 0);

    *stdin = *_fdopen(
        _open_osfhandle(reinterpret_cast<intp>(GetStdHandle(STD_INPUT_HANDLE)),
                        _O_TEXT),
        "r");
    setvbuf(stdin, NULL, _IONBF, 0);

    *stderr = *_fdopen(
        _open_osfhandle(reinterpret_cast<intp>(GetStdHandle(STD_ERROR_HANDLE)),
                        _O_TEXT),
        "w");
    setvbuf(stdout, NULL, _IONBF, 0);
...
//-----------------------------------------------------------------------------
uint16 SetWin32ConsoleColor(Win32ConsoleColorContext_t *pContext, int nRed,
                            int nGreen, int nBlue, int nIntensity) {
#if PLATFORM_WINDOWS_PC
...
//-----------------------------------------------------------------------------
// Restore's the active foreground console color, without distributing the
// current background color.
//-----------------------------------------------------------------------------
void RestoreWin32ConsoleColor(Win32ConsoleColorContext_t *pContext,
                              uint16 prevColor) {
#if PLATFORM_WINDOWS_PC
  SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                          prevColor | pContext->m_BackgroundFlags);
