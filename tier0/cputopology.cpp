 ...
 public:
  ...
  /*virtual*/ DWORD_PTR CoreAffinityMask(DWORD coreIdx) const {
    ...
    if (1 == coreIdx) {
      ...
      GetProcessAffinityMask(GetCurrentProcess(), &coreAffinity,
                             &dwSystemAffinity);
    }
    return coreAffinity;
  }
...
class GlpiImpl : public ICpuTopology {
 public:
  ...
  /*virtual*/ DWORD NumberOfProcessCores() const {
    ...
    GetProcessAffinityMask(GetCurrentProcess(), &dwProcessAffinity,
                           &dwSystemAffinity);
  ...
  /*virtual*/ DWORD NumberOfSystemCores() const {
    DWORD nCores = 0;
    for (DWORD i = 0; i < m_nItems; ++i) {
      if (RelationProcessorCore == m_pSlpi[i].Relationship) ++nCores;
    }
    return nCores;
  }
...
//       Each APIC ID is composed of a PACKAGE_ID, CORE_ID and SMT_ID that
//       describe the relationship of a logical processor within the processor
...
class ApicExtractor {
 public:
  ...
  //       the sub id accessors (SmtId(), CoreId(), PackageId(),
  //       PackageCoreId()) as this information is required to effectively
  //       decode an APIC ID into its sub parts.
  //-----------------------------------------------------------------------------
  void SetPackageTopology(DWORD nLogProcsPerPkg, DWORD nCoresPerPkg) {
    ...
    // fix for Phenom x3 and similar CPUs - it reports 3 logical processors per
    // package, and 4 cores per package so one core is probably just disabled
    // for yield, but it causes a bug in GetMaskWidth that propagates
...
class Cpuid {
 public:
  ...
  //-----------------------------------------------------------------------------
  // Name: Cpuid::Call
  // Desc: Calls the CPUID instruction with the specified function.  Returns
  // TRUE
  ...
  //       support different functions.  This method is automatically called
  //       from the Call() method, so you don't need to call it beforehand.
  //-----------------------------------------------------------------------------
...
//       interface.  This is a ConcreteImplementor class in the traditional
//       Bridge Pattern.
//---------------------------------------------------------------------------------
class CpuidImpl : public ICpuTopology {
 public:
  // CpuidFnMasks are used when extracting bit-encoded information retrieved
    ...
    if (cpu.Edx() & HTT) {
      ...
      if (Cpuid::IsVendor(GenuineIntel)) {
        ...
        if (cpu.Call(Cpuid::Ext, 8)) {
          ...
          // and not the actual number of current cores, which is how the msb
          // width of the CORE_ID bit field has been traditionally determined.
          // If the ApicIdCoreIdSize_Amd value is zero, then you use the
          // traditional method to determine the CORE_ID msb width.
  ...
  /*virtual*/ DWORD_PTR CoreAffinityMask(DWORD coreIdx) const {
    ...
    if (coreIdx < nPkgCoreIds) {
      for (DWORD i = 0; i < m_nItems; ++i) {
        if (m_apicExtractor.PackageCoreId(m_apicIds[i]) ==
            pkgCoreIds[coreIdx]) {
  ...
  static BOOL IsSupported() {
    BOOL bSupported =
        Cpuid::IsVendor(GenuineIntel) || Cpuid::IsVendor(AuthenticAMD);
    ...
    if (bSupported) {
      ...
      bSupported = GetProcessAffinityMask(hProcess, &dwProcessAffinity,
                                          &dwSystemAffinity);
      if (bSupported) {
        if (dwProcessAffinity != dwSystemAffinity) {
        ...
        if (bSupported && (dwSystemAffinity > 1)) {
          ...
          DWORD_PTR dwThreadAffinity =
              SetThreadAffinityMask(hThread, dwProcessAffinity);
          if (dwThreadAffinity) {
 ...
 private:
  ...
  //       nPkgCore is an in/out parm that will reflect the total number of
  //       items in pkgCoreIds array.  It will be incrememted if a unique
  //       package/core id is found and added.
  //-----------------------------------------------------------------------------
  void AddUniquePkgCoreId_(DWORD idx, BYTE* pkgCoreIds,
                           DWORD& nPkgCoreIds) const {
    _ASSERT(idx < m_nItems);
    _ASSERT(NULL != pkgCoreIds);

    DWORD j;
    for (j = 0; j < nPkgCoreIds; ++j) {
      if (pkgCoreIds[j] == m_apicExtractor.PackageCoreId(m_apicIds[idx])) break;
    }
    if (j == nPkgCoreIds) {
...
// Desc: Constructs a cpu topology object.  If bForce is FALSE, then a GlpiImpl
// object
//       is first attempted, then CpuidImpl, then finally DefaultImpl.  If
//       bForce is TRUE, then GlpiImpl is never attempted.
//-------------------------------------------------------------------------------------
