...
// 360 uses a 48MB primary (physical) SBH and 10MB secondary (virtual) SBH, with
// no fallback
...
// PS3 uses just a 32MB SBH - this was enough to avoid overflow when Portal 2
// shipped. NOTE: when Steam uses the game's tier0 allocator (see memalloc.h),
// we increase the size
//       of the SBH and MBH (see memstd.cpp) to accommodate those extra
//       allocations.
...
#define MEMSTD_COMPILE_TIME_ASSERT(pred) \
  switch (0) {                           \
    case 0:                              \
    case pred:;                          \
  }
...
class CStdMemAlloc : public IMemAlloc {
 public:
  ...
  virtual void OutOfMemory(size_t nBytesAttempted = 0) {
    SetCRTAllocFailed(nBytesAttempted);
  }
...
#ifdef _WIN32
      return (byte *)VirtualAlloc(NULL, TOTAL_BYTES, VA_RESERVE_FLAGS,
                                  PAGE_NOACCESS);
#elif defined(_PS3)
      Error("");
      return NULL;
...
#ifdef _WIN32
      return (VirtualAlloc(pPage, BYTES_PAGE, VA_COMMIT_FLAGS,
                           PAGE_READWRITE) != NULL);
#elif defined(_PS3)
      return false;
#else
  ...
  class CFixedAllocator {
   public:
    ...
    byte *AllocatePoolMemory() {
...
#ifdef _X360
      if (bPhysical)
        return (byte *)XPhysicalAlloc(TOTAL_BYTES, MAXULONG_PTR, 4096,
                                      PAGE_READWRITE | MEM_16MB_PAGES);
#endif
      return (byte *)VirtualAlloc(NULL, TOTAL_BYTES, VA_COMMIT_FLAGS,
                                  PAGE_READWRITE);
#elif defined(_PS3)
      ...
      IVirtualMemorySection *pSection =
          VirtualMemoryManager_AllocateVirtualMemorySection(TOTAL_BYTES);
      if (!pSection)
        Error(
            "CFixedAllocator::AllocatePoolMemory() failed in "
            "VirtualMemoryManager_AllocateVirtualMemorySection\n");
      if (!pSection->CommitPages(pSection->GetBaseAddress(), TOTAL_BYTES))
        Error(
            "CFixedAllocator::AllocatePoolMemory() failed in "
            "IVirtualMemorySection::CommitPages\n");
...
#error
  ...
  typedef CSmallBlockHeap<CFixedAllocator<MBYTES_PRIMARY_SBH, true>>
      CFixedSmallBlockHeap;
#ifdef MEMALLOC_USE_SECONDARY_SBH
  typedef CSmallBlockHeap<CFixedAllocator<MBYTES_SECONDARY_SBH, false>>
      CFixedVirtualSmallBlockHeap;  // @TODO: move back into above heap if
                                    // number stays at 16 [7/15/2009 tom]
#endif
