...
#include "winlite.h"  // Currently needed for IsBadReadPtr and IsBadWritePtr
...
static bool s_bCallStacksWithAllWarnings =
    false;  // if true, attach a call stack to every SPEW_WARNING message.
            // Warning()/DevWarning()/...
static int s_iWarningMaxCallStackLength = 5;
#define AutomaticWarningCallStackLength() \
  (s_bCallStacksWithAllWarnings ? s_iWarningMaxCallStackLength : 0)
...
static bool s_bCallStacksWithAllErrors =
    false;  // if true, attach a call stack to every SPEW_ERROR message. Mostly
            // just Error()
static int s_iErrorMaxCallStackLength =
    20;  // default to higher output with an error since we're quitting anyways
#define AutomaticErrorCallStackLength() \
  (s_bCallStacksWithAllErrors ? s_iErrorMaxCallStackLength : 0)
...
void _ExitOnFatalAssert(const tchar *pFile, int line) {
  Log_Msg(LOG_ASSERT,
          _T("Fatal assert failed: %s, line %d.  Application exiting.\n"),
          pFile, line);
...
void AppendCallStackToLogMessage(tchar *formattedMessage, int iMessageLength,
                                 int iAppendCallStackLength) {
#if defined(ENABLE_RUNTIME_STACK_TRANSLATION)
#if defined(TCHAR_IS_CHAR)  // I'm horrible with unicode and I don't plan on
                            // testing this with wide characters just yet
  if (iAppendCallStackLength > 0) {
    int iExistingMessageLength = (int)strlen(
        formattedMessage);  // no V_strlen in tier 0, plus we're only compiling
                            // this for windows and 360. Seems safe
    ...
    // append preface
    int iAppendedLength =
        _snprintf(formattedMessage, iMessageLength, _T("\nCall Stack:\n\t"));

    void **CallStackBuffer =
        (void **)stackalloc(iAppendCallStackLength * sizeof(void *));
    int iCount = GetCallStack(CallStackBuffer, iAppendCallStackLength, 2);
    if (TranslateStackInfo(CallStackBuffer, iCount,
                           formattedMessage + iAppendedLength,
                           iMessageLength - iAppendedLength, _T("\n\t")) == 0) {
      // failure
      formattedMessage[0] =
          '\0';  // this is pointing at where we wrote "\nCall Stack:\n\t"
    } else {
      iAppendedLength += (int)strlen(
          formattedMessage +
          iAppendedLength);  // no V_strlen in tier 0, plus we're only compiling
                             // this for windows and 360. Seems safe
...
#define Log_LegacyHelperColor_Stack(Channel, Severity, Color, MessageFormat, \
                                    AppendCallStackLength)                   \
  do {                                                                       \
    ...
    if (pLoggingSystem->IsChannelEnabled(Channel, Severity)) {               \
      ...
      Tier0Internal_vsntprintf(formattedMessage, MAX_LOGGING_MESSAGE_LENGTH, \
                               MessageFormat, args);                         \
      va_end(args);                                                          \
      AppendCallStackToLogMessage(formattedMessage,                          \
...
#define Log_LegacyHelperColor(Channel, Severity, Color, MessageFormat) \
  Log_LegacyHelperColor_Stack(Channel, Severity, Color, MessageFormat, 0)

#define Log_LegacyHelper_Stack(Channel, Severity, MessageFormat,        \
                               AppendCallStackLength)                   \
  Log_LegacyHelperColor_Stack(Channel, Severity,                        \
                              pLoggingSystem->GetChannelColor(Channel), \
                              MessageFormat, AppendCallStackLength)
#define Log_LegacyHelper(Channel, Severity, MessageFormat)        \
  Log_LegacyHelperColor(Channel, Severity,                        \
                        pLoggingSystem->GetChannelColor(Channel), \
                        MessageFormat)
...
void Warning(const tchar *pMsgFormat, ...) {
  Log_LegacyHelper_Stack(LOG_GENERAL, LS_WARNING, pMsgFormat,
                         AutomaticWarningCallStackLength());
}
...
void Error(const tchar *pMsgFormat, ...) {
  Log_LegacyHelper_Stack(LOG_GENERAL, LS_ERROR, pMsgFormat,
                         AutomaticErrorCallStackLength());
}
...
void DevMsg(int level, const tchar *pMsgFormat, ...) {
  LoggingChannelID_t channel =
      level >= 2 ? LOG_DEVELOPER_VERBOSE : LOG_DEVELOPER;
...
// If we don't have a function from math.h, then it doesn't link certain
// floating-point functions in and printfs with %f cause runtime errors in the C
// libraries.
...
void COM_TimestampedLog(char const *fmt, ...) {
  ...
  if (!s_bChecked) {
    ...
    s_bShouldLogToConsole =
        (CommandLine()->ParmValue("-profile", 0.0f) != 0.0f) ? true : false;

    s_bShouldLogToETW =
        (CommandLine()->CheckParm("-etwprofile")) ? true : false;
...
void CHardwareBreakPoint::SetBits(DWORD_PTR &dw, int lowBit, int bits,
                                  int newValue) {
...
DWORD WINAPI CHardwareBreakPoint::ThreadProc(LPVOID lpParameter) {
  ...
  if (h->m_eOperation == CHardwareBreakPoint::BRK_UNSET) {
    ...
    ct.Dr6 = 0;
    int st = 0;
    if (h->m_eType == BREAKPOINT_EXECUTE) st = 0;
    if (h->m_eType == BREAKPOINT_READWRITE) st = 3;
    if (h->m_eType == BREAKPOINT_WRITE) st = 1;

    int le = 0;
    if (h->m_eSize == BREAKPOINT_SIZE_1) le = 0;
    if (h->m_eSize == BREAKPOINT_SIZE_2) le = 1;
    if (h->m_eSize == BREAKPOINT_SIZE_4) le = 3;
    if (h->m_eSize == BREAKPOINT_SIZE_8) le = 2;
...
HardwareBreakpointHandle_t SetHardwareBreakpoint(EHardwareBreakpointType eType,
