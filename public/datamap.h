...
typedef enum _fieldtypes {
  ...
  FIELD_POSITION_VECTOR,  // A world coordinate (these are fixed up across level
  ...
  FIELD_MATRIX3X4_WORLDSPACE,  // matrix3x4_t that maps some local space to
                               // world space (translation is fixed up on level
                               // transitions)

  FIELD_INTERVAL,  // a start and range floating point interval ( e.g., 3.2->3.6
                   // == 3.2 and 0.4 )
  FIELD_MODELINDEX,     // a model index
  FIELD_MATERIALINDEX,  // a material index (using the material precache string
                        // table)
...
// NOTE:  Must match interval.h definition
...
#define _FIELD(name, fieldtype, count, flags, mapname, tolerance)              \
  {                                                                            \
    fieldtype, #name, offsetof(classNameTypedef, name), count, flags, mapname, \
        NULL, NULL, NULL, sizeof(((classNameTypedef *)0)->name), NULL, 0,      \
        tolerance                                                              \
  }
...
#define DEFINE_FIELD_NOT_SAVED(name, fieldtype) \
...
#define DEFINE_ARRAY_NOT_SAVED(name, fieldtype, count) \
...
#define DEFINE_CUSTOM_FIELD(name, datafuncs)                                  \
  {                                                                           \
    FIELD_CUSTOM, #name, offsetof(classNameTypedef, name), 1, FTYPEDESC_SAVE, \
        NULL, datafuncs, NULL                                                 \
  }
#define DEFINE_CUSTOM_KEYFIELD(name, datafuncs, mapname)         \
  {                                                              \
    FIELD_CUSTOM, #name, offsetof(classNameTypedef, name), 1,    \
...
#define DEFINE_EMBEDDED(name)                                   \
  {                                                             \
    FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name), 1, \
...
#define DEFINE_EMBEDDED_ARRAY(name, count)                          \
  {                                                                 \
    FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name), count, \
        FTYPEDESC_SAVE, NULL, NULL, NULL,                           \
        &(((classNameTypedef *)0)->name->m_DataMap),                \
...
#define DEFINE_EMBEDDED_AUTO_ARRAY(name)                                    \
  {                                                                         \
    FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name),                \
        SIZE_OF_ARRAY(((classNameTypedef *)0)->name), FTYPEDESC_SAVE, NULL, \
        NULL, NULL, &(((classNameTypedef *)0)->name->m_DataMap),            \
        sizeof(((classNameTypedef *)0)->name[0]), NULL, 0, 0.0f             \
  }

#ifndef NO_ENTITY_PREDICTION

// FTYPEDESC_KEY tells the prediction copy system to report the full nameof the
// field when reporting errors
#define DEFINE_PRED_TYPEDESCRIPTION(name, fieldtype)            \
  {                                                             \
    FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name), 1, \
        FTYPEDESC_SAVE | FTYPEDESC_KEY, NULL, NULL, NULL,       \
        &fieldtype::m_PredMap                                   \
  }
...
#define DEFINE_PRED_TYPEDESCRIPTION_PTR(name, fieldtype)                  \
  {                                                                       \
    FIELD_EMBEDDED, #name, offsetof(classNameTypedef, name), 1,           \
        FTYPEDESC_SAVE | FTYPEDESC_PTR | FTYPEDESC_KEY, NULL, NULL, NULL, \
        &fieldtype::m_PredMap                                             \
  }
...
#define DEFINE_INPUT(name, fieldtype, inputname)                           \
  {                                                                        \
    fieldtype, #name, offsetof(classNameTypedef, name), 1,                 \
...
#define DEFINE_INPUTFUNC(fieldtype, inputname, inputfunc)             \
  {                                                                   \
    fieldtype, #inputfunc, NULL, 1, FTYPEDESC_INPUT, inputname, NULL, \
        static_cast<inputfunc_t>(&classNameTypedef::inputfunc)        \
  }
...
#define DEFINE_FUNCTION_RAW(function, func_type)                \
  {                                                             \
    FIELD_VOID, nameHolder.GenerateName(#function), NULL, 1,    \
...
// This field can be requested and written to by string name at load
// time
#define FTYPEDESC_KEY 0x0004
// This field can be written to by string name at run time, and a
// function called
#define FTYPEDESC_INPUT 0x0008
// This field propagates its value to all targets whenever it changes
...
// The field is an override for one in a base class (only used by
...
// The field is local to the client or server only (not referenced by
// prediction code and not replicated by networking)
#define FTYPEDESC_PRIVATE 0x0200
// The field is part of the prediction typedescription, but doesn't
...
// Only show this data if the player is on the same team as the local
// player
...
// This is a FIELD_FLOAT and should only be checked to be within 0.001
// of the networked info
...
//
using inputfunc_t = void (CBaseEntity::*)(inputdata_t &data);
...
struct typedescription_t {
  ...
  // For raw fields (including children of embedded stuff) this is the flattened
  // offset
...
#define BEGIN_DATADESC_GUTS(className)                                        \
  ...
  namespace className##_DataDescInit {                                        \
    datamap_t *g_DataMapHolder = DataMapInit<className>(                      \
        (className *)                                                         \
            NULL); /* This can/will be used for some clean up duties later */ \
  }                                                                           \
                                                                              \
  template <>                                                                 \
  datamap_t *DataMapInit<className>(className *) {                            \
    ...
    static typedescription_t dataDesc[] = {                                   \
      {FIELD_VOID, 0, 0, 0, 0, 0, 0, 0,                                       \
       0}, /* so you can define "empty" tables */

#define BEGIN_DATADESC_GUTS_NAMESPACE(className, nameSpace)                   \
  template <typename T>                                                       \
  datamap_t *nameSpace::DataMapInit(T *);                                     \
  template <>                                                                 \
  datamap_t *nameSpace::DataMapInit<className>(className *);                  \
  namespace className##_DataDescInit {                                        \
    datamap_t *g_DataMapHolder = nameSpace::DataMapInit(                      \
        (className *)                                                         \
            NULL); /* This can/will be used for some clean up duties later */ \
  }                                                                           \
                                                                              \
