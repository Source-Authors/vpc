...
//
// Functions for UCS/UTF/Unicode string operations. These functions are in
// vstdlib instead of tier1, because on PS/3 they need to load and initialize a
// system module, which is more frugal to do from a single place rather than
// multiple times in different PRX'es. The functions themselves aren't supposed
// to be called frequently enough for the DLL/PRX boundary marshalling, if any,
// to have any measureable impact on performance.

#ifndef VPC_VSTDLIB_VSTRTOOLS_H_
#define VPC_VSTDLIB_VSTRTOOLS_H_
...
VSTRTOOLS_INTERFACE int V_UnicodeToUCS2(const wchar_t *pUnicode,
                                        int cubSrcInBytes, char *pUCS2,
                                        int cubDestSizeInBytes);
VSTRTOOLS_INTERFACE int V_UTF8ToUCS2(const char *pUTF8, int cubSrcInBytes,
                                     ucs2 *pUCS2, int cubDestSizeInBytes);

// copy at most n bytes into destination, will not corrupt utf-8 multi-byte
// sequences
...
#endif  // VPC_VSTDLIB_VSTRTOOLS_H_