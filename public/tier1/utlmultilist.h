...
#ifndef VPC_TIER1_UTLMULTILIST_H_
#define VPC_TIER1_UTLMULTILIST_H_
...
// A lovely index-based linked list! T is the class type, I is the index type,
// which usually should be an unsigned short or smaller.  This list can contain
// multiple lists.
...
class CUtlMultiList {
 ...
 protected:
  ...
  I m_FirstFree;
  I m_TotalElements;
  int m_MaxElementIndex;  // The number allocated (use int so we can catch
                          // overflow)
  ...
  void ResetDbgInfo() {
    ...
    if (m_Memory.NumAllocated() > 0) {
      if (!m_pElementList) {
        ...
        m_pElementList =
            (I*)realloc(m_pElementList, m_Memory.NumAllocated() * sizeof(I));
      }
    }
#endif
  }
...
CUtlMultiList<T, I>::~CUtlMultiList() {
  RemoveAll();
  if (m_pElementList) free(m_pElementList);
}
...
{
  // Since I is not necessarily the type returned by M (int), we need to check
  // that M returns indices which are representable by I. A common case is 'I
  // === unsigned short', in which case case CUtlMemory will have 'InvalidIndex
  // == (int)-1' (which casts to 65535 in I), and will happily return elements
  // at index 65535 and above.
...
I CUtlMultiList<T, I>::Alloc() {
  ...
  if (m_FirstFree == InvalidIndex()) {
    // We can overflow before the utlmemory overflows, since we have have I !=
    // int
    if (!IndexInRange(m_MaxElementIndex)) {
      ExecuteNTimes(
          10, Warning("CUtlMultiList overflow! (exhausted index range)\n"));
      return InvalidIndex();
    }
...
//-----------------------------------------------------------------------------
template <class T, class I>
inline bool CUtlMultiList<T, I>::IsElementInList(ListHandle_t list,
                                                 I elem) const {
  if (!m_pElementList) return true;
...
void CUtlMultiList<T, I>::Remove(ListHandle_t list, I elem) {
  if (IsInList(elem)) Unlink(list, elem);
  Free(elem);
}
...
void CUtlMultiList<T, I>::RemoveAll() {
  ...
  for (int i = (int)m_MaxElementIndex; --i >= 0;) {
    ...
    if (IsValidIndex((I)i)) Destruct(&Element((I)i));
  ...
  for (I list = m_List.Head(); list != m_List.InvalidIndex();
       list = m_List.Next(list)) {
...
#endif  // VPC_TIER1_UTLMULTILIST_H_
