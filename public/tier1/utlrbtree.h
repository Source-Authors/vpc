...
#ifndef VPC_TIER1_UTLRBTREE_H_
#define VPC_TIER1_UTLRBTREE_H_
...
#define FOR_EACH_UTLRBTREE(treeName, iteratorName) \
  for (int iteratorName = treeName.FirstInorder(); \
...
class CUtlRBTree {
 public:
  ...
  // in size at each increment. LessFunc_t is required, but may be set after the
  // constructor using SetLessFunc() below
  ...
  // NOTE: CopyFrom is fast but dangerous! It just memcpy's all nodes - it does
  // NOT run copy constructors, so
  //       it is not a true deep copy (i.e 'T' must be POD for this to work -
  //       e.g CUtlString will not work).
  ...
  // it's not generally safe to iterate from index 0 to MaxElement()-1 (you
  // could do this as a potential iteration optimization, IF CUtlMemory is the
  // allocator, and IF IsValidIndex() is tested for each element...
  //  but this should be implemented inside the CUtlRBTree iteration API, if
  //  anywhere)
  I MaxElement() const;
  ...
  // NOTE: the returned 'index' will be valid as long as the element remains in
  // the tree
...
inline void CUtlRBTree<T, I, L, M>::CopyFrom(
  ...
  memcpy(m_Elements.Base(), other.m_Elements.Base(),
         other.m_Elements.Count() * sizeof(UtlRBTreeNode_t<T, I>));
