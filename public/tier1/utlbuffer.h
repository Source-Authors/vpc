...
#ifndef VPC_TIER1_UTLBUFFER_H_
#define VPC_TIER1_UTLBUFFER_H_
...
class CUtlCharConversion {
 public:
  ...
  CUtlCharConversion(char nEscapeChar, const char *pDelimiter, intp nCount,
...
#define END_CHAR_CONVERSION(_name, _delimiter, _escapeChar)                   \
  ...
  ;                                                                           \
  CUtlCharConversion _name(_escapeChar, _delimiter,                           \
...
template <class A>
inline const char *GetFmtStr(int nRadix = 10, bool bPrint = true) {
  Assert(0);
  return "";
}
...
inline FMSTRRETTYPE GetFmtStr<double>([[maybe_unused]] int nRadix,
  ...
  return bPrint ? "%.15lf" : "%lf";
   ...
   // defaults to FLT_DIG=6

//-----------------------------------------------------------------------------
// Command parsing..
//-----------------------------------------------------------------------------
class CUtlBuffer {
  // Brian has on his todo list to revisit this as there are issues in some
  // cases with CUtlVector using operator = instead of copy construtor in
  // InsertMultiple, etc. The unsafe case is something like this:
  ...
  //	CUtlBuffer &operator=( const CUtlBuffer & );//  { Assert( 0 ); return
  //*this; }
 ...
 public:
  ...
  enum BufferFlags_t {
    ...
    EXTERNAL_GROWABLE = 0x2,  // This is used w/ external buffers and causes the
                              // utlbuf to switch to reallocatable memory if an
                              // overflow happens when Putting.
    CONTAINS_CRLF =
        0x4,  // For text buffers only, does this contain \n or \n\r?
    READ_ONLY =
        0x8,  // For external buffers; prevents null termination from happening.
    AUTO_TABS_DISABLED = 0x10,  // Used to disable/enable push/pop tabs
  };

  // Overflow functions when a get or put overflows
  typedef bool (CUtlBuffer::*UtlBufferOverflowFunc_t)(intp nSize);
  ...
  CUtlBuffer(const void *pBuffer, intp size, bool crap);
  ...
  void *AccessForDirectRead(intp nBytes);
  ...
  FORCEINLINE void ActivateByteSwappingIfBigEndian(void) {
    if ((IsX360() || IsPS3())) ActivateByteSwapping(true);
  }
  ...
  double GetDouble();
  void *GetPtr();
  void GetString(char *pString, intp nMaxChars = 0);
  ...
  template <typename T>
  void GetObjects(T *dest, intp count = 1);
  ...
  template <typename T>
  void PutObjects(T *src, intp count = 1);
 ...
 protected:
  ...
  // almost always called immediately after modifying m_Put and this lets it
  // stay in a register
  void AddNullTermination(intp nPut);
...
class CUtlInplaceBuffer : public CUtlBuffer {
 public:
  CUtlInplaceBuffer(intp growSize = 0, intp initSize = 0, int nFlags = 0);
 ...
 public:
  ...
  //
  // Note:	the returned length of the line is at least one character
  // because the
  //			trailing newline characters are also included as part of
  // the line.
  //
  // Note:	the pointer returned points into the local memory of this
  // buffer, in
  //			case the buffer gets relocated or destroyed the pointer
  // becomes invalid.
  //
  // e.g.:	-------------
  //
  //			char *pszLine;
  //			int nLineLen;
  //			while ( pUtlInplaceBuffer->InplaceGetLinePtr( &pszLine,
  //&nLineLen ) )
  //			{
  //				...
  //			}
  ...
  //			-------------
  //
  // @param	ppszInBufferPtr		on return points into this buffer at
  // start of line
  // @param	pnLineLength		on return holds num bytes accessible via
  // (*ppszInBufferPtr)
  //
  // @returns	true				if line was successfully read
  //			false				when EOF is reached or
  // error occurs
  //
  bool InplaceGetLinePtr(/* out */ char **ppszInBufferPtr,
                         /* out */ intp *pnLineLength);

  //
  // Determines the line length, advances the "get" pointer offset by the line
  // length, replaces the newline character with null-terminator and returns the
  // initial pointer to now null-terminated line.
  //
  ...
  //
  //			while ( char *pszLine =
  // pUtlInplaceBuffer->InplaceGetLinePtr()
  //)
  //			{
  //				...
  //			}
...
// Where am I reading?
//-----------------------------------------------------------------------------
inline intp CUtlBuffer::TellGet() const { return m_Get; }
...
inline void *CUtlBuffer::GetPtr() {
  void *p;
  // LEGACY WARNING: in text mode, PutPtr writes 32 bit pointers in hex, while
  // GetPtr reads 32 or 64 bit pointers in decimal
#ifndef X64BITS
...
// Where am I writing?
//-----------------------------------------------------------------------------
inline intp CUtlBuffer::TellPut() const { return m_Put; }

//-----------------------------------------------------------------------------
// What's the most I've ever written?
//-----------------------------------------------------------------------------
inline intp CUtlBuffer::TellMaxPut() const { return m_nMaxPut; }
...
inline void CUtlBuffer::PutShort(short s) { PutType(s); }

inline void CUtlBuffer::PutUnsignedShort(unsigned short s) { PutType(s); }

inline void CUtlBuffer::PutInt(int i) { PutType(i); }

inline void CUtlBuffer::PutInt64(int64 i) { PutType(i); }

inline void CUtlBuffer::PutUnsignedInt(unsigned int u) { PutType(u); }

inline void CUtlBuffer::PutFloat(float f) { PutType(f); }

inline void CUtlBuffer::PutDouble(double d) { PutType(d); }
...
inline intp CUtlBuffer::Size() const { return m_Memory.NumAllocated(); }
...
//
//-----------------------------------------------------------------------------
inline void *CUtlBuffer::AccessForDirectRead(intp nBytes) {
