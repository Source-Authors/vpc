...
#ifndef VPC_TIER1_UTLMAP_H_
#define VPC_TIER1_UTLMAP_H_
...
#define FOR_EACH_MAP(mapName, iteratorName)                            \
  for (auto iteratorName = (mapName).FirstInorder();                    \
...
void DeepCopyMap(const CUtlMap<K, T, I> &pmapIn,
  ...
  out_pmapOut->Purge();
  FOR_EACH_MAP_FAST(pmapIn, i) { out_pmapOut->Insert(i, pmapIn[i]); }
}

#endif  // VPC_TIER1_UTLMAP_H_
