...
#ifndef VPC_TIER1_UTLLINKEDLIST_H_
#define VPC_TIER1_UTLLINKEDLIST_H_
...
class CUtlFixedLinkedList
    : public CUtlLinkedList<T, intp, true, intp,
                            CUtlFixedMemory<UtlLinkedListElem_t<T, intp>>> {
 public:
  CUtlFixedLinkedList(int growSize = 0, int initSize = 0)
      : CUtlLinkedList<T, intp, true, intp,
                       CUtlFixedMemory<UtlLinkedListElem_t<T, intp>>>(
            growSize, initSize) {}

  bool IsValidIndex(intp i) const {
    if (!this->Memory().IsIdxValid(i)) return false;
    ...
    return (this->Memory()[i].m_Previous != i) ||
           (this->Memory()[i].m_Next == i);
  }
...
CUtlLinkedList<T, S, ML, I, M>::CUtlLinkedList(int growSize, int initSize)
  ...
  // Prevent signed non-intp datatypes
  static_assert(sizeof(S) == sizeof(intp) || (((S)-1) > 0));
  ConstructList();
  ResetDbgInfo();
}
...
{
  // Since S is not necessarily the type returned by M, we need to check that M
  // returns indices which are representable by S. A common case is 'S ===
...
I CUtlLinkedList<T, S, ML, I, M>::AllocInternal(bool multilist) {
  ...
  if (m_FirstFree == InvalidIndex()) {
    ...
    if (!m_Memory.IsValidIterator(it)) {
      ...
      it = m_Memory.IsValidIterator(m_LastAlloc) ? m_Memory.Next(m_LastAlloc)
                                                 : m_Memory.First();
    ...
    if (!IndexInRange(m_Memory.GetIndex(it))) {
      ExecuteNTimes(
          10, Warning("CUtlLinkedList overflow! (exhausted index range)\n"));
      return InvalidIndex();
    }
...
void CUtlLinkedList<T, S, ML, I, M>::LinkBefore(I before, I elem) {
  ...
  S newElem_mPrevious;  // we need to hang on to this for the compairson against
                        // InvalidIndex() below; otherwise we get a a
...
#endif  // VPC_TIER1_UTLLINKEDLIST_H_
