...
#ifndef VPC_TIER1_MEMPOOL_H_
#define VPC_TIER1_MEMPOOL_H_
...
using MemoryPoolReportFunc_t = void (*)(char const *pMsg, ...);
...
class CUtlMemoryPool {
 public:
  ...
  enum MemoryPoolGrowType_t {
    ...
    // New blob size is numElements * (i+1)  (ie: the blocks it
...
class CClassMemoryPool : public CUtlMemoryPool {
 public:
  CClassMemoryPool(int numElements, int growMode = GROW_FAST,
                   int nAlignment = 0)
      : CUtlMemoryPool(sizeof(T), numElements, growMode, MEM_ALLOC_CLASSNAME(T),
                       nAlignment) {}
...
template <int ITEM_SIZE, int ALIGNMENT, int CHUNK_SIZE, class CAllocator,
          bool GROWMODE = false, int COMPACT_THRESHOLD = 4>
...
class CFixedBudgetMemoryPool {
 public:
  ...
  bool IsEmpty() {
#ifndef USE_MEM_DEBUG
    if (m_pBase && m_freeList.Count() != ITEM_COUNT) return false;
#endif
    return true;
  }

  enum { ITEM_SIZE = ALIGN_VALUE(PROVIDED_ITEM_SIZE, TSLIST_NODE_ALIGNMENT) };
...
#define BIND_TO_FIXED_BUDGET_POOL(poolName)                                    \
  ...
  inline void *operator new(size_t size, int nBlockUse, const char *pFileName, \
                            int nLine) {                                       \
...
template <int ITEM_SIZE, int ALIGNMENT, int CHUNK_SIZE, class CAllocator,
          bool GROWMODE, int COMPACT_THRESHOLD>
...
inline void *CAlignedMemPool<ITEM_SIZE, ALIGNMENT, CHUNK_SIZE, CAllocator,
                             GROWMODE, COMPACT_THRESHOLD>::Alloc() {
  AUTO_LOCK(m_mutex);
...
template <int ITEM_SIZE, int ALIGNMENT, int CHUNK_SIZE, class CAllocator,
          bool GROWMODE, int COMPACT_THRESHOLD>
inline void CAlignedMemPool<ITEM_SIZE, ALIGNMENT, CHUNK_SIZE, CAllocator,
                            GROWMODE, COMPACT_THRESHOLD>::Free(void *p) {
  AUTO_LOCK(m_mutex);
  ...
  if (m_nFree >= (CHUNK_SIZE / BLOCK_SIZE) * COMPACT_THRESHOLD) {
    ...
    double compactTime =
        (m_nFree >= (CHUNK_SIZE / BLOCK_SIZE) * COMPACT_THRESHOLD * 4) ? 15.0
                                                                       : 30.0;
...
template <int ITEM_SIZE, int ALIGNMENT, int CHUNK_SIZE, class CAllocator,
          bool GROWMODE, int COMPACT_THRESHOLD>
inline void CAlignedMemPool<ITEM_SIZE, ALIGNMENT, CHUNK_SIZE, CAllocator,
                            GROWMODE, COMPACT_THRESHOLD>::Compact() {
  FreeBlock_t *pCur = m_pFirstFree;
  FreeBlock_t *pPrev = NULL;
