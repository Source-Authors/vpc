...
#ifndef VPC_TIER1_FMTSTR_H_
#define VPC_TIER1_FMTSTR_H_
...
#define FmtStrVSNPrintf(szBuf, nBufSize, bQuietTruncation, ppszFormat,        \
  ...
  do {                                                                        \
    ...
    result = V_vsnprintfRet((szBuf), (nBufSize)-1, (*(ppszFormat)), arg_ptr,  \
                            &bTruncated);                                     \
    va_end(arg_ptr);                                                          \
                                                                              \
    (szBuf)[(nBufSize)-1] = 0;                                                \
...
class CFmtStrN {
 ...
 protected:
  virtual void InitQuietTruncation() { m_bQuietTruncation = QUIET_TRUNCATION; }
...
class CNumStr {
 public:
  ...
  inline void SetDouble(double f) {
    V_snprintf(m_szBuf, sizeof(m_szBuf), "%.18g", f);
  }
  ...
  inline void SetHexUint64(uint64 un64) {
    V_binarytohex((byte *)&un64, sizeof(un64), m_szBuf, sizeof(m_szBuf));
  }
 ...
 protected:
  char m_szBuf[28];  // long enough to hold 18 digits of precision, a decimal, a
                     // - sign, e+### suffix, and quotes
};
