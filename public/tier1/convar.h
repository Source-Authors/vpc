...
#ifndef VPC_TIER1_CONVAR_H_
#define VPC_TIER1_CONVAR_H_
...
// +host_thread_mode 0 +r_threaded_particles 0 +sv_parallel_packentities 0
...
class ConVar : public ConCommandBase, public IConVar {
 ...
 public:
  ...
  virtual bool IsRegistered(void) const;
  virtual const char *GetName(void) const;
  // Return name of command (usually == GetName(), except in case of
  // FCVAR_SS_ADDED vars
  ...
  // Any function that allocates/frees memory needs to be virtual or else you'll
  // have crashes
  //  from alloc/free across dll/exe boundaries.

  // These just call into the IConCommandBaseAccessor to check flags and set the
  // var (which ends up calling InternalSetValue).
  ...
  FORCEINLINE_CVAR CVValue_t &GetRawValue() { return m_Value; }
  FORCEINLINE_CVAR const CVValue_t &GetRawValue() const { return m_Value; }
 ...
 protected:
  // This either points to "this" or it points to the original declaration of a
  // ConVar. This allows ConVars to exist in separate modules, and they all use
  // the first one to be declared. m_pParent->m_pParent must equal m_pParent
  // (ie: m_pParent must be the root, or original, ConVar).
  ConVar *m_pParent;
...
FORCEINLINE_CVAR float ConVar::GetFloat(void) const {
  ...
  Assert(ThreadInMainThread() ||
         IsFlagSet(FCVAR_MATERIAL_THREAD_MASK | FCVAR_ACCESSIBLE_FROM_THREADS));
#endif
...
FORCEINLINE_CVAR Color ConVar::GetColor(void) const {
  ...
  unsigned char *pColorElement =
      ((unsigned char *)&m_pParent->m_Value.m_nValue);
  return Color(pColorElement[0], pColorElement[1], pColorElement[2],
               pColorElement[3]);
}
...
template <>
FORCEINLINE_CVAR float ConVar::Get<float>(void) const {
  return GetFloat();
}
template <>
FORCEINLINE_CVAR int ConVar::Get<int>(void) const {
  return GetInt();
}
template <>
FORCEINLINE_CVAR bool ConVar::Get<bool>(void) const {
  return GetBool();
}
template <>
FORCEINLINE_CVAR const char *ConVar::Get<const char *>(void) const {
  return GetString();
}
template <>
FORCEINLINE_CVAR float ConVar::Get<float>(float *p) const {
  return (*p = GetFloat());
}
template <>
FORCEINLINE_CVAR int ConVar::Get<int>(int *p) const {
  return (*p = GetInt());
}
template <>
FORCEINLINE_CVAR bool ConVar::Get<bool>(bool *p) const {
  return (*p = GetBool());
}
template <>
FORCEINLINE_CVAR const char *ConVar::Get<const char *>(char const **p) const {
  return (*p = GetString());
}
...
class CSplitScreenAddedConVar : public ConVar {
 ...
 public:
  CSplitScreenAddedConVar(int nSplitScreenSlot, const char *pName,
                          const ConVar *pBaseVar)
      : BaseClass(
            pName, pBaseVar->GetDefault(),
            // Keep basevar flags, except remove _SS and add _SS_ADDED instead
            (pBaseVar->GetFlags() & ~FCVAR_SS) | FCVAR_SS_ADDED,
            pBaseVar->GetHelpText(), pBaseVar->HasMin(),
            pBaseVar->GetMinValue(), pBaseVar->HasMax(),
...
FORCEINLINE_CVAR void CSplitScreenAddedConVar::SetSplitScreenPlayerSlot(
    int nSlot) {
...
FORCEINLINE_CVAR void SplitScreenConVarRef::SetValue(int nSlot,
                                                     const char *pValue) {
