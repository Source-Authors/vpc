...
#ifndef VPC_TIER1_STRTOOLS_H_
#define VPC_TIER1_STRTOOLS_H_
...
// there are some users of these via tier1 templates in used in tier0. but tier0
// can't depend on vstdlib which means in tier0 we always need the inlined ones
...
// To avoid cross-prx calls, making the V_* fucntions that don't do anything but
// debug checks and call through to the non V_* function go ahead and call the
// non-V_* functions directly.
...
#else
  // this is 11% faster on Core i7 than the previous, VC2005 compiler generates
  // a seemingly unbalanced search tree that's faster
  switch (c) {
    case ' ':
    case 9:
    case '\r':
...
// pathname. Returns false if it tries to ".." past the root directory in the
// drive (in which case it is an invalid path).
...
struct BinString_t {
  ...
  bool ToBin() { return V_StringToBin(m_string, &m_bin, sizeof(m_bin)); }
  void ToString() { V_BinToString(m_string, &m_bin, sizeof(m_bin)); }
  T m_bin;
  char m_string[sizeof(T) * 2 +
                2];  // 0-terminated string representing the binary data in hex
};
...
// can end a line, begin a line, or be broken up when repeated (eg: "...")
...
// passed; calls the above functions on each character
bool CanBreakAfter(const wchar_t *wsz);

}  // namespace AsianWordWrap

// We use this function to determine where it is permissible to break lines
// of text while wrapping them. On most platforms, the native iswspace()
// function returns FALSE for the "non-breaking space" characters 0x00a0 and
// 0x202f, and so we don't break on them. On the 360, however, iswspace returns
// TRUE for them. So, on that platform, we work around it by defining this
// wrapper which returns false for &nbsp; and calls through to the library
// function for everything else.
int isbreakablewspace(wchar_t ch);

#endif  // VPC_TIER1_STRTOOLS_H_
