...
#ifndef VPC_TIER0_BASETYPES_H_
#define VPC_TIER0_BASETYPES_H_

// This is a trick to get the DLL extension off the -D option on the command
// line.
...
// #define COMPILETIME_MAX and COMPILETIME_MIN for max/min in constant
// expressions
...
FORCEINLINE double fpmin(double a, double b) { return a > b ? b : a; }
FORCEINLINE float fpmin(float a, float b) { return a > b ? b : a; }

FORCEINLINE double fpmax(double a, double b) { return a >= b ? a : b; }
FORCEINLINE float fpmax(float a, float b) { return a >= b ? a : b; }
...
// __fsel(double fComparand, double fValGE, double fLT) == fComparand >= 0 ?
// fValGE : fLT this is much faster than if ( aFloat > 0 ) { x = .. } the XDK
// defines two intrinsics, one for floats and one for doubles -- it's the same
// opcode, but the __fself version tells the compiler not to do a wasteful
// unnecessary rounding op after each sel. #define fsel __fsel
#ifdef _X360
...
FORCEINLINE float fsel(float fComparand, float fValGE, float fLT) {
  return __fsels(fComparand, fValGE, fLT);
}
#endif

#if !defined(_X360)
FORCEINLINE float fpmin(float a, float b) { return fsel(a - b, b, a); }
FORCEINLINE double fpmin(double a, double b) { return fsel(a - b, b, a); }

FORCEINLINE float fpmax(float a, float b) { return fsel(a - b, a, b); }
FORCEINLINE double fpmax(double a, double b) { return fsel(a - b, a, b); }

// any mixed calls should promote to double
FORCEINLINE double fpmax(float a, double b) { return fpmax((double)a, b); }
// any mixed calls should promote to double
...
FORCEINLINE int imin(int x, int y) {
  ...
  int nMaxMask =
      nMaxSign >> 31;  // 0 if x greater than y, 0xffffffff if x smaller than y
...
FORCEINLINE int imax(int x, int y) {
  ...
  int nMinMask =
      nMinSign >> 31;  // 0 if x smaller than y, 0xffffffff if x greater than y
...
#ifdef _WIN32
typedef __int32
    vec_t_as_gpr;  // a general purpose register type equal in size to a vec_t
                   // (in case we have to avoid the fpu for some reason)
#endif
...
inline float FloatMakePositive(vec_t f) { return fabsf(f); }
#else
inline float FloatMakePositive(vec_t f) {
  return fabsf(f);  // was since 2002: BitsToFloat( FloatBits(f) & 0x7FFFFFFF );
                    // fixed in 2010
}
#endif
...
inline float FloatMakeNegative(vec_t f) {
  return -fabsf(f);  // was since 2002: BitsToFloat( FloatBits(f) | 0x80000000
                     // ); fixed in 2010
}
...
struct Rect3D_t {
  ...
  FORCEINLINE Rect3D_t(int nX, int nY, int nZ, int nWidth, int nHeight,
                       int nDepth) {
    x = nX;
    y = nY;
    z = nZ;
...
class CIntHandle16 : public CBaseIntHandle<unsigned short> {
 ...
 protected:
  inline CIntHandle16(HANDLE_TYPE val) { m_Handle = val; }
};
...
class CIntHandle32 : public CBaseIntHandle<uint32> {
 ...
 protected:
  inline CIntHandle32(HANDLE_TYPE val) { m_Handle = val; }
};

// NOTE: This macro is the same as windows uses; so don't change the guts of it
#define DECLARE_HANDLE_16BIT(name) \
  typedef CIntHandle16<struct name##__handle *> name;
#define DECLARE_HANDLE_32BIT(name) \
  typedef CIntHandle32<struct name##__handle *> name;
...
#define DECLARE_DERIVED_POINTER_HANDLE(_name, _basehandle) \
  struct _name##__ : public _basehandle##__ {};            \
  typedef struct _name##__ *_name
#define DECLARE_ALIASED_POINTER_HANDLE(_name, _alias) \
  typedef struct _alias##__ *name
...
// Max 2 player splitscreen in portal (don't merge this back), saves a bunch of
// memory [8/31/2010 tom]
