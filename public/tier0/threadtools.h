...
//
// Purpose: A collection of utility classes to simplify thread handling, and as
// much as possible contain portability problems.  Here avoiding including
// windows.h.

#ifndef VPC_TIER0_THREADTOOLS_H_
#define VPC_TIER0_THREADTOOLS_H_
...
#if (defined(PLATFORM_WINDOWS_PC) || defined(PLATFORM_X360)) && \
    !defined(STEAM) && !defined(_CERT)
// Thread parent stack trace linkage requires ALL executing binaries to disable
// frame pointer omission to operate speedily/successfully. (/Oy-)  "vpc /nofpo"
#define THREAD_PARENT_STACK_TRACE_SUPPORTED \
  1  // uncomment to support joining the root of a thread's stack trace to its
     // parent's at time of invocation. Must also set
     // ENABLE_THREAD_PARENT_STACK_TRACING in stacktools.h
#endif

#if defined(THREAD_PARENT_STACK_TRACE_SUPPORTED)
#include "tier0/stacktools.h"
#if defined(ENABLE_THREAD_PARENT_STACK_TRACING)  // stacktools.h opted in
#define THREAD_PARENT_STACK_TRACE_ENABLED \
  1  // both threadtools.h and stacktools.h have opted into the feature, enable
     // it
#endif
#endif
...
CreateSimpleThread(ThreadFunc_t, void *pParam, ThreadId_t *pID,
                   unsigned stackSize = 0x10000 /*64*/);
PLATFORM_INTERFACE ThreadHandle_t CreateSimpleThread(
    ThreadFunc_t, void *pParam, unsigned stackSize = 0x10000 /*64*/);
#else   //_PS3
...
PLATFORM_INTERFACE void ThreadSetDebugName(ThreadHandle_t hThread,
                                           const char *pszName);
inline void ThreadSetDebugName(const char *pszName) {
  ThreadSetDebugName(NULL, pszName);
}
...
inline int32 ThreadInterlockedExchange(int32 volatile *p, int32 value) {
  ...
  // Note: The LOCK instruction prefix is assumed on the XCHG instruction and
  // GCC gets very confused on the Mac when we use it.
  __asm __volatile("xchgl %2,(%1)"
                   : "=r"(nRet)
                   : "r"(p), "0"(value)
                   : "memory");
  return nRet;
...
inline int32 ThreadInterlockedCompareExchange(int32 volatile *p, int32 value,
                                              int32 comperand) {
  Assert((size_t)p % 4 == 0);
  return __sync_val_compare_and_swap(p, comperand, value);
}

inline bool ThreadInterlockedAssignIf(int32 volatile *p, int32 value,
                                      int32 comperand) {
  Assert((size_t)p % 4 == 0);
...
#elif defined(_PS3)
PLATFORM_INTERFACE inline int32 ThreadInterlockedIncrement(int32 volatile *ea) {
  return cellAtomicIncr32((uint32_t *)ea) + 1;
}
PLATFORM_INTERFACE inline int32 ThreadInterlockedDecrement(int32 volatile *ea) {
  return cellAtomicDecr32((uint32_t *)ea) - 1;
}
PLATFORM_INTERFACE inline int32 ThreadInterlockedExchange(int32 volatile *ea,
                                                          int32 value) {
  return cellAtomicStore32((uint32_t *)ea, value);
}
PLATFORM_INTERFACE inline int32 ThreadInterlockedExchangeAdd(int32 volatile *ea,
                                                             int32 value) {
  return cellAtomicAdd32((uint32_t *)ea, value);
}
PLATFORM_INTERFACE inline int32 ThreadInterlockedCompareExchange(
    int32 volatile *ea, int32 value, int32 comperand) {
  return cellAtomicCompareAndSwap32((uint32_t *)ea, comperand, value);
}
PLATFORM_INTERFACE inline bool ThreadInterlockedAssignIf(int32 volatile *ea,
                                                         int32 value,
                                                         int32 comperand) {
  return (cellAtomicCompareAndSwap32((uint32_t *)ea, comperand, value) ==
          (uint32_t)comperand);
...
PLATFORM_INTERFACE inline int64 ThreadInterlockedCompareExchange64(
    int64 volatile *pDest, int64 value, int64 comperand) {
  return cellAtomicCompareAndSwap64((uint64_t *)pDest, comperand, value);
}
PLATFORM_INTERFACE inline bool ThreadInterlockedAssignIf64(
    volatile int64 *pDest, int64 value, int64 comperand) {
  return (cellAtomicCompareAndSwap64((uint64_t *)pDest, comperand, value) ==
          (uint64_t)comperand);
}
...
FORCEINLINE int32 ThreadInterlockedIncrement(int32 volatile *pDest) {
  Assert((size_t)pDest % 4 == 0);
  return InterlockedIncrement(TO_INTERLOCK_PARAM(pDest));
}
FORCEINLINE int32 ThreadInterlockedDecrement(int32 volatile *pDest) {
  Assert((size_t)pDest % 4 == 0);
  return InterlockedDecrement(TO_INTERLOCK_PARAM(pDest));
}
FORCEINLINE int32 ThreadInterlockedExchange(int32 volatile *pDest,
                                            int32 value) {
  Assert((size_t)pDest % 4 == 0);
  return InterlockedExchange(TO_INTERLOCK_PARAM(pDest), value);
}
FORCEINLINE int32 ThreadInterlockedExchangeAdd(int32 volatile *pDest,
                                               int32 value) {
  Assert((size_t)pDest % 4 == 0);
  return InterlockedExchangeAdd(TO_INTERLOCK_PARAM(pDest), value);
}
FORCEINLINE int32 ThreadInterlockedCompareExchange(int32 volatile *pDest,
                                                   int32 value,
                                                   int32 comperand) {
  Assert((size_t)pDest % 4 == 0);
...
FORCEINLINE bool ThreadInterlockedAssignIf(int32 volatile *pDest, int32 value,
                                           int32 comperand) {
  Assert((size_t)pDest % 4 == 0);
  return (InterlockedCompareExchange(TO_INTERLOCK_PARAM(pDest), value,
                                     comperand) == comperand);
...
PLATFORM_INTERFACE int32 ThreadInterlockedExchange(int32 volatile *,
                                                   int32 value) NOINLINE;
PLATFORM_INTERFACE int32 ThreadInterlockedExchangeAdd(int32 volatile *,
                                                      int32 value) NOINLINE;
PLATFORM_INTERFACE int32 ThreadInterlockedCompareExchange(
    int32 volatile *, int32 value, int32 comperand) NOINLINE;
PLATFORM_INTERFACE bool ThreadInterlockedAssignIf(int32 volatile *, int32 value,
                                                  int32 comperand) NOINLINE;
#endif
...
inline void *ThreadInterlockedExchangePointer(void *volatile *p, void *value) {
  return (void *)((intp)ThreadInterlockedExchange(
...
inline bool ThreadInterlockedAssignPointerIf(void *volatile *p, void *value,
                                             void *comperand) {
  return (ThreadInterlockedCompareExchange(reinterpret_cast<intp volatile *>(p),
                                           reinterpret_cast<intp>(value),
                                           reinterpret_cast<intp>(comperand)) ==
          reinterpret_cast<intp>(comperand));
}
#else
...
inline unsigned ThreadInterlockedExchangeSubtract(int32 volatile *p,
                                                  int32 value) {
  return ThreadInterlockedExchangeAdd((int32 volatile *)p, -value);
}
...
inline unsigned ThreadInterlockedExchangeSubtract(uint32 volatile *p,
                                                  uint32 value) {
  return ThreadInterlockedExchangeAdd((int32 volatile *)p, value);
}

inline unsigned ThreadInterlockedIncrement(uint32 volatile *p) {
  return ThreadInterlockedIncrement((int32 volatile *)p);
}
inline unsigned ThreadInterlockedDecrement(uint32 volatile *p) {
  return ThreadInterlockedDecrement((int32 volatile *)p);
}
inline unsigned ThreadInterlockedExchange(uint32 volatile *p, uint32 value) {
  return ThreadInterlockedExchange((int32 volatile *)p, value);
}
inline unsigned ThreadInterlockedExchangeAdd(uint32 volatile *p, uint32 value) {
  return ThreadInterlockedExchangeAdd((int32 volatile *)p, value);
}
inline unsigned ThreadInterlockedCompareExchange(uint32 volatile *p,
                                                 uint32 value,
                                                 uint32 comperand) {
  return ThreadInterlockedCompareExchange((int32 volatile *)p, value,
                                          comperand);
}
inline bool ThreadInterlockedAssignIf(uint32 volatile *p, uint32 value,
                                      uint32 comperand) {
  return ThreadInterlockedAssignIf((int32 volatile *)p, value, comperand);
}

// inline int ThreadInterlockedExchangeSubtract( int volatile *p, int value )
// { return ThreadInterlockedExchangeAdd( (int32 volatile *)p, value ); } inline
// int ThreadInterlockedIncrement( int volatile *p )	{ return
// ThreadInterlockedIncrement( (int32 volatile *)p ); } inline int
// ThreadInterlockedDecrement( int volatile *p )	{ return
// ThreadInterlockedDecrement( (int32 volatile *)p ); } inline int
// ThreadInterlockedExchange( int volatile *p, int value )	{ return
// ThreadInterlockedExchange( (int32 volatile *)p, value ); } inline int
// ThreadInterlockedExchangeAdd( int volatile *p, int value )	{ return
// ThreadInterlockedExchangeAdd( (int32 volatile *)p, value ); } inline int
// ThreadInterlockedCompareExchange( int volatile *p, int value, int comperand )
// { return ThreadInterlockedCompareExchange( (int32 volatile *)p, value,
// comperand ); } inline bool ThreadInterlockedAssignIf( int volatile *p, int
// value, int comperand )	{ return ThreadInterlockedAssignIf( (int32
// volatile *)p, value, comperand ); }
...
#define CTHREADLOCALUSEERROR                                \
  PS3_ELF_EXPORTED_THREADLOCAL PS3_ELF_EXPORTED_THREADLOCAL \
...
// a (not so efficient) implementation of thread locals for compilers without
// full support (i.e. visual c). don't use this explicity - instead, use the
// CTHREADxxx macros above.
...
class CThreadLocalInt : public CThreadLocal<T> {
 public:
  ...
  inline CThreadLocalInt(const T &initialvalue) { this->Set(initialvalue); }
};
...
class CInterlockedIntT {
 public:
  ...
  bool AssignIf(T conditionValue, T newValue) {
    return ThreadInterlockedAssignIf((int32 *)&m_value, (int32)newValue,
  ...
  // Atomic add is like += except it returns the previous value as its return
  // value
  T AtomicAdd(T add) {
...
class CInterlockedPtr {
 public:
  CInterlockedPtr() : m_value(0) {
    COMPILE_TIME_ASSERT(
        sizeof(T *) ==
        sizeof(int32)); /* Will need to rework operator+= for 64 bit */
  }
  ...
  // value
  T *AtomicAdd(int add) {
    return (T *)ThreadInterlockedExchangeAdd((int32 *)&m_value,
                                             add * sizeof(T));
  }
...
class PLATFORM_CLASS CThreadMutex {
 public:
  ...
  void LockSilent();  // A Lock() operation which never spews.  Required by the
                      // logging system to prevent badness.
  void UnlockSilent();  // An Unlock() operation which never spews.  Required by
                        // the logging system to prevent badness.
...
#error
  ...
  // Debugging (always herge to allow mixed debug/release builds w/o changing
  // size)
 ...
 private:
  FORCEINLINE bool TryLockInline(const uint32 threadId) volatile {
    if (threadId != m_ownerID &&
        !ThreadInterlockedAssignIf((volatile int32 *)&m_ownerID,
                                   (int32)threadId, 0))
      return false;
...
class PLATFORM_CLASS CThreadSyncObject {
 public:
  ...
  // or THREADSYNC_TIMEOUT if the timeout was hit before the wait condition was
  // met.
  //
  ...
  //-----------------------------------------------------
  static uint32 WaitForMultiple(int nObjects, CThreadSyncObject **ppObjects,
                                bool bWaitAll, uint32 dwTimeout = TT_INFINITE);

  // This builds a list of pointers and calls straight through to the other
  // WaitForMultiple.
  static uint32 WaitForMultiple(int nObjects, CThreadSyncObject *ppObjects,
                                bool bWaitAll, uint32 dwTimeout = TT_INFINITE);
...
class PLATFORM_CLASS CThreadEvent : public CThreadSyncObject {
 public:
  ...
  CThreadEvent(const char *name, bool initialState = false,
               bool bManualReset = false);
  static NamedEventResult_t CheckNamedEvent(const char *name,
                                            uint32 dwTimeout = 0);
  ...
  static uint32 WaitForMultiple(int nObjects, CThreadEvent **ppObjects,
                                bool bWaitAll, uint32 dwTimeout = TT_INFINITE);
 ...
 protected:
#ifdef _PS3
  // These virtual functions need to be inline in order for the class to be
  // exported from tier0.prx
  virtual bool AddWaitingThread() {
    // This checks if the event is already signaled and if not creates a
    // semaphore which will be signaled when the event is finally signaled.
    bool result;
 ...
 private:
  ...
  CThreadEventWaitObject *LLLinkNode(CThreadEventWaitObject *list,
                                     CThreadEventWaitObject *node);
...
PLATFORM_INTERFACE int ThreadWaitForObjects(int nEvents, const HANDLE *pHandles,
                                            bool bWaitAll = true,
                                            unsigned timeout = TT_INFINITE);
inline int ThreadWaitForEvents(int nEvents, const CThreadEvent *pEvents,
                               bool bWaitAll = true,
                               unsigned timeout = TT_INFINITE) {
...
/* (commented out to reduce distraction in colorized editor, remove entirely
when new implementation settles) class ALIGN8 PLATFORM_CLASS CThreadSpinRWLock
{
public:
        CThreadSpinRWLock()	{ COMPILE_TIME_ASSERT( sizeof( LockInfo_t ) ==
sizeof( int64 ) ); Assert( (intp)this % 8 == 0 ); memset( this, 0, sizeof( *this
) ); }
        ...
        bool TryLockForWrite() const { return const_cast<CThreadSpinRWLock
*>(this)->TryLockForWrite(); } bool TryLockForRead() const { return
const_cast<CThreadSpinRWLock *>(this)->TryLockForRead(); } void LockForRead()
const { const_cast<CThreadSpinRWLock *>(this)->LockForRead(); } void
UnlockRead() const { const_cast<CThreadSpinRWLock *>(this)->UnlockRead(); } void
LockForWrite() const { const_cast<CThreadSpinRWLock *>(this)->LockForWrite(); }
        void UnlockWrite() const { const_cast<CThreadSpinRWLock
*>(this)->UnlockWrite(); }

private:
        // This structure is used as an atomic & exchangeable 64-bit value. It
would probably be better to just have one 64-bit value
        // and accessor functions that make/break it, but at this late stage of
development, I'm just wrapping it into union
        // Beware of endianness: on Xbox/PowerPC m_writerId is high-word of
m_i64; on PC, it's low-dword of m_i64 union LockInfo_t
        {
                struct
                {
                        uint32	m_writerId;
...
); bool TryLockForWrite( const uint32 threadId ); void SpinLockForWrite( const
uint32 threadId );
...
class PLATFORM_CLASS CThread {
 public:
  ...
  virtual bool Start(unsigned nBytesStack = 0,
                     ThreadPriorityEnum_t nPriority = PRIORITY_DEFAULT);
...
#undef Yield
 ...
 private:
  ...
  // EAPS3 Event to indicate that a thread has terminated. This helps with the
  // replacing of WaitForMultipleObjects
...
// The CThread implementation needs to be inlined for performance on the PS3 -
// It makes a difference of more than 1ms/frame Since the dependency checker
// isn't smart enough to take an #ifdef _PS3 into account, all platforms will
// inline it.
#ifdef _PS3
...
class PLATFORM_CLASS CWorkerThread : public CThread {
 ...
 protected:
  typedef uint32 (*WaitFunc_t)(uint32 nHandles, CThreadEvent **ppHandles,
...
#else
  DebuggerBreak();  // should not be called - not defined for this
                    // platform/compiler!!!
#endif
}
...
inline void CThreadMutex::LockSilent() { pthread_mutex_lock(&m_Mutex); }

//---------------------------------------------------------

inline void CThreadMutex::UnlockSilent() { pthread_mutex_unlock(&m_Mutex); }
...
FORCEINLINE bool CThreadSpinRWLock::TryLockForWrite() {
  ...
  if (!(curValue.m_i32 & 0x00010000) &&
      ThreadInterlockedAssignIf(&curValue.m_i32, 0x00010000, 0)) {
    ThreadMemoryBarrier();
...
FORCEINLINE bool CThreadSpinRWLock::TryLockForRead() {
  ...
  if (!(curValue.m_i32 & 0x00010000))  // !m_lockInfo.m_fWriting
  {
    ...
    if (ThreadInterlockedAssignIf(&m_lockInfo.m_i32, newValue.m_i32,
                                  oldValue.m_i32)) {
      ThreadMemoryBarrier();
...
void CThreadSpinRWLock::UnlockRead() {
  RWLAssert(m_writerId == 0 ||
            (m_writerId == ThreadGetCurrentId() && m_lockInfo.m_fWriting));
#ifdef REENTRANT_THREAD_SPIN_RW_LOCK
  if (!(m_lockInfo.m_i32 & 0x00010000))  // !m_lockInfo.m_fWriting
#endif
  {
...
#else
/* (commented out to reduce distraction in colorized editor, remove entirely
when new implementation settles) inline bool CThreadSpinRWLock::AssignIf( const
LockInfo_t &newValue, const LockInfo_t &comperand )
{
        // Note: using unions guarantees no aliasing bugs. Casting structures
through *(int64*)&
        //       may create hard-to-catch bugs because when you do that,
compiler doesn't know that the newly computed pointer
        //       is actually aliased with LockInfo_t structure. It's rarely a
problem in practice, but when it is, it's a royal pain to debug. return
ThreadInterlockedAssignIf64( &m_lockInfo.m_i64, newValue.m_i64, comperand.m_i64
);
}

FORCEINLINE bool CThreadSpinRWLock::TryLockForWrite( const uint32 threadId )
{
        // In order to grab a write lock, there can be no readers and no owners
of the write lock if ( m_lockInfo.m_nReaders > 0 || ( m_lockInfo.m_writerId &&
...
FORCEINLINE bool CThreadSpinRWLock::TryLockForRead()
{
        ...
        // In order to grab a write lock, the number of readers must not change
and no thread can own the write LockInfo_t oldValue; LockInfo_t newValue;

        if( IsX360() || IsPS3() )
        {
                // this is the code equivalent to original code (see below) that
doesn't cause LHS on Xbox360
                // WARNING: This code assumes BIG Endian CPU
                oldValue.m_i64 = uint32( m_lockInfo.m_nReaders );
                newValue.m_i64 = oldValue.m_i64 + 1; // NOTE: when we have -1
(or 0xFFFFFFFF) readers, this will result in non-equivalent code
        }
        else
        {
...
DWORD WaitForMultipleObjects(DWORD nCount, CThreadEvent **lppHandles,
                             BOOL bWaitAll, DWORD dwMilliseconds);
#endif  // _PS3
#endif  // VPC_TIER0_THREADTOOLS_H_
