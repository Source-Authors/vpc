...
#ifndef VPC_TIER0_DBG_H_
#define VPC_TIER0_DBG_H_
...
//   message along with the file and line, the third will display a generic
//   message and will also cause the function BadFunc to be executed, and the
//   last two will report an error if f is not equal to 5 (the last one asserts
...
#else
#define DbgFlagMacro_ExitOnFatalAssert(pFile, line) \
  _ExitOnFatalAssert(pFile, line)
#endif
...
PLATFORM_INTERFACE bool DoNewAssertDialog(const tchar *pFile, int line,
                                          const tchar *pExpression);

#if defined(DBGFLAG_STRINGS_STRIP)
#define DbgFlagMacro_DoNewAssertDialog(pFile, line, pExpression) \
  DoNewAssertDialog("", 0, "")
#else
#define DbgFlagMacro_DoNewAssertDialog(pFile, line, pExpression) \
  DoNewAssertDialog(pFile, line, pExpression)
#endif

/* Used to define macros, never use these directly. */

#ifdef _PREFAST_
// When doing /analyze builds define the assert macros to be __analysis_assume.
// This tells the compiler to assume that the condition is true, which helps to
// suppress many warnings. This define is done in debug and release builds, but
// debug builds should be preferred for static analysis because some asserts are
// compiled out in release. The unfortunate !! is necessary because otherwise
// /analyze is incapable of evaluating all of the logical expressions that the
// regular compiler can handle.
...
#define _AssertMsg(_exp, _msg, _executeExp, _bFatal)                      \
  do {                                                                    \
    if (!(_exp)) {                                                        \
      LoggingResponse_t ret =                                             \
          Log_Assert("%s (%d) : %s\n", __TFILE__, __LINE__, _msg);        \
      _executeExp;                                                        \
      if (ret == LR_DEBUGGER) {                                           \
        if (!ShouldUseNewAssertDialog() ||                                \
            DbgFlagMacro_DoNewAssertDialog(__TFILE__, __LINE__, _msg))    \
          DebuggerBreak();                                                \
        if (_bFatal) DbgFlagMacro_ExitOnFatalAssert(__TFILE__, __LINE__); \
      }                                                                   \
    }                                                                     \
  } while (0)
...
#define AssertFatalMsg1(_exp, _msg, a1) \
  AssertFatalMsg(_exp, (const tchar *)(CDbgFmtMsg(_msg, a1)))
#define AssertFatalMsg2(_exp, _msg, a1, a2) \
  AssertFatalMsg(_exp, (const tchar *)(CDbgFmtMsg(_msg, a1, a2)))
#define AssertFatalMsg3(_exp, _msg, a1, a2, a3) \
  AssertFatalMsg(_exp, (const tchar *)(CDbgFmtMsg(_msg, a1, a2, a3)))
#define AssertFatalMsg4(_exp, _msg, a1, a2, a3, a4) \
  AssertFatalMsg(_exp, (const tchar *)(CDbgFmtMsg(_msg, a1, a2, a3, a4)))
#define AssertFatalMsg5(_exp, _msg, a1, a2, a3, a4, a5) \
  AssertFatalMsg(_exp, (const tchar *)(CDbgFmtMsg(_msg, a1, a2, a3, a4, a5)))
#define AssertFatalMsg6(_exp, _msg, a1, a2, a3, a4, a5, a6) \
  AssertFatalMsg(_exp,                                      \
                 (const tchar *)(CDbgFmtMsg(_msg, a1, a2, a3, a4, a5, a6)))
#define AssertFatalMsg6(_exp, _msg, a1, a2, a3, a4, a5, a6) \
  AssertFatalMsg(_exp,                                      \
                 (const tchar *)(CDbgFmtMsg(_msg, a1, a2, a3, a4, a5, a6)))
#define AssertFatalMsg7(_exp, _msg, a1, a2, a3, a4, a5, a6, a7) \
  AssertFatalMsg(                                               \
      _exp, (const tchar *)(CDbgFmtMsg(_msg, a1, a2, a3, a4, a5, a6, a7)))
#define AssertFatalMsg8(_exp, _msg, a1, a2, a3, a4, a5, a6, a7, a8) \
  AssertFatalMsg(                                                   \
      _exp, (const tchar *)(CDbgFmtMsg(_msg, a1, a2, a3, a4, a5, a6, a7, a8)))
...
// lightweight assert macros: in theory, can be run in release without slowing
// it down
...
#if defined(_X360)
#define AssertAligned(PTR) \
  __twnei(intp(PTR) & 0xF, \
          0)  // trap if not equal to immediate value; unsigned comparison
...
#define AssertMsg1(_exp, _msg, a1) \
  AssertMsg_(_exp, (const tchar *)(CDbgFmtMsg(_T(_msg), a1)))
#define AssertMsg2(_exp, _msg, a1, a2) \
  AssertMsg_(_exp, (const tchar *)(CDbgFmtMsg(_T(_msg), a1, a2)))
#define AssertMsg3(_exp, _msg, a1, a2, a3) \
  AssertMsg_(_exp, (const tchar *)(CDbgFmtMsg(_T(_msg), a1, a2, a3)))
#define AssertMsg4(_exp, _msg, a1, a2, a3, a4) \
  AssertMsg_(_exp, (const tchar *)(CDbgFmtMsg(_T(_msg), a1, a2, a3, a4)))
#define AssertMsg5(_exp, _msg, a1, a2, a3, a4, a5) \
  AssertMsg_(_exp, (const tchar *)(CDbgFmtMsg(_T(_msg), a1, a2, a3, a4, a5)))
#define AssertMsg6(_exp, _msg, a1, a2, a3, a4, a5, a6) \
  AssertMsg_(_exp,                                     \
             (const tchar *)(CDbgFmtMsg(_T(_msg), a1, a2, a3, a4, a5, a6)))
#define AssertMsg7(_exp, _msg, a1, a2, a3, a4, a5, a6, a7) \
  AssertMsg_(                                              \
      _exp, (const tchar *)(CDbgFmtMsg(_T(_msg), a1, a2, a3, a4, a5, a6, a7)))
...
#define FILE_LINE_FUNCTION_STRING \
  __FILE__ "(" STRINGIFY(__LINE__) "):" __FUNCTION__ ":"
...
PLATFORM_INTERFACE void Error_SpewCallStack(int iMaxCallStackLength,
                                            const tchar *pMsg, ...)
    FMTFUNCTION(2, 3);
...
PLATFORM_INTERFACE void Warning_SpewCallStack(int iMaxCallStackLength,
                                              const tchar *pMsg, ...)
    FMTFUNCTION(2, 3);

#ifdef _PS3

PLATFORM_OVERLOAD void DevMsg(int level, const tchar *pMsg, ...)
    FMTFUNCTION(2, 3);
PLATFORM_OVERLOAD void DevWarning(int level, const tchar *pMsg, ...)
    FMTFUNCTION(2, 3);
...
PLATFORM_INTERFACE void ConColorMsg(const Color &clr, const tchar *pMsg, ...)
    FMTFUNCTION(2, 3);
PLATFORM_INTERFACE void ConMsg(const tchar *pMsg, ...) FMTFUNCTION(1, 2);

#else  // !_PS3

PLATFORM_INTERFACE void DevMsg(int level, const tchar *pMsg, ...)
    FMTFUNCTION(2, 3);
PLATFORM_INTERFACE void DevWarning(int level, const tchar *pMsg, ...)
    FMTFUNCTION(2, 3);
...
PLATFORM_OVERLOAD void ConColorMsg(const Color &clr, const tchar *pMsg, ...)
    FMTFUNCTION(2, 3);
...
PLATFORM_INTERFACE void COM_TimestampedLog(char const *fmt, ...)
    FMTFUNCTION(1, 2);
...
// @TODO: these callstack spew functions are currently disabled in the new
// logging system.  Need to add support for these if desired.
PLATFORM_INTERFACE void _Warning_AlwaysSpewCallStack_Enable(bool bEnable);
PLATFORM_INTERFACE void _Warning_AlwaysSpewCallStack_Length(
    int iMaxCallStackLength);

PLATFORM_INTERFACE void _Error_AlwaysSpewCallStack_Enable(bool bEnable);
PLATFORM_INTERFACE void _Error_AlwaysSpewCallStack_Length(
    int iMaxCallStackLength);
...
inline void AssertValidStringPtr(const tchar *ptr, int maxchar = 0xFFFFFF) {
  _AssertValidStringPtr(ptr, maxchar);
}
template <class T>
...
// Tier0 uses these for string functions since this abstraction is normally done
// in tier1.
#ifdef POSIX
...
// Code for programmatically setting/unsetting hardware breakpoints (there's
// probably a 360 and
#ifdef IS_WINDOWS_PC
...
PLATFORM_INTERFACE HardwareBreakpointHandle_t
SetHardwareBreakpoint(EHardwareBreakpointType eType,
                      EHardwareBreakpointSize eSize, const void *pvLocation);
PLATFORM_INTERFACE bool ClearHardwareBreakpoint(
    HardwareBreakpointHandle_t handle);

class CHardwareBreakPointScopeGuard {
 public:
  CHardwareBreakPointScopeGuard(
      const void *pvLocation, size_t nLocationSize,
      EHardwareBreakpointType eType = BREAKPOINT_WRITE) {
    ...
    switch (nLocationSize) {
      ...
      default:
        Warning(
            _T( "SetHardwareBreakpoint can only work with 1, 2, 4 or 8 byte data fields." ));
        break;
    }
  ...
  ~CHardwareBreakPointScopeGuard() { Release(); }
