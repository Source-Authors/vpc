...
// mod, tool) across all platforms to output information, warnings, errors, etc.
//
// This system supersedes the existing Msg(), Warning(), Error(), DevMsg(),
// ConMsg() etc. functions.  There are channels defined in the new system
// through which all old messages are routed; see LOG_GENERAL, LOG_CONSOLE,
// LOG_DEVELOPER, etc.
//
...
//
// A ChannelID is typically created by defining a logging channel with
// the log channel macros:
//
// DEFINE_LOGGING_CHANNEL_NO_TAGS(LOG_ChannelName, "ChannelName", [Flags],
// [MinimumSeverity], [Color]);
//
// or
//
// BEGIN_DEFINE_LOGGING_CHANNEL(LOG_ChannelName, "ChannelName",
// [Flags], [MinimumSeverity], [Color] ); ADD_LOGGING_CHANNEL_TAG( "Tag1" );
//              ADD_LOGGING_CHANNEL_TAG( "Tag2" );
//              END_DEFINE_LOGGING_CHANNEL();
//
// These macros create a global channel ID variable with the name specified by
// the first parameter (in this example, LOG_ChannelName). This channel ID can
// be used by various LoggingSystem_** functions to manipulate the channel
// settings.
//
// The optional [Flags] parameter is an OR'd together set of
// LoggingChannelFlags_t values (default: 0).// The optional [MinimumSeverity]
// parameter is the lowest threshold above which messages will be processed
// (inclusive).  The default is LS_MESSAGE, which results in all messages,
// warnings, and errors being logged.
//
// Variadic parameters to the Log_** functions will be ignored if a channel is
// not enabled for a given severity (for performance reasons).
//
// Logging channels can have their minimum severity modified by name, ID, or
// tag.
//
// Logging channels are not hierarchical since there are situations in which a
// channel needs to belong to multiple hierarchies.  Use tags to create
...
// 3) Sentient robot to clean up the thousands of places using the old/legacy
// logging system.

#ifndef VPC_TIER0_LOGGING_H_
#define VPC_TIER0_LOGGING_H_
...
class CSimpleLoggingListener : public ILoggingListener {
 public:
  ...
  virtual void Log(const LoggingContext_t *, const tchar *pMessage) {
#ifdef _X360
 ...
 public:
  virtual void Log(const LoggingContext_t *pContext, const tchar *pMessage) {
    ...
    if (pContext->m_Severity == LS_ERROR) {
      if (Plat_IsInDebugSession()) DebuggerBreak();
...
class CColorizedLoggingListener : public CSimpleLoggingListener {
 public:
  CColorizedLoggingListener(bool bQuietPrintf = false,
                            bool bQuietDebugger = false)
  ...
  virtual void Log(const LoggingContext_t *pContext, const tchar *pMessage) {
    if (!m_bQuietPrintf) {
      uint16 nPrevColor = UINT16_MAX;

      if (pContext->m_Color != UNSPECIFIED_LOGGING_COLOR) {
        nPrevColor = SetWin32ConsoleColor(
            &m_ColorContext, pContext->m_Color.r(), pContext->m_Color.g(),
            pContext->m_Color.b(),
            MAX(MAX(pContext->m_Color.r(), pContext->m_Color.g()),
                pContext->m_Color.b()) > 128);
      }
...
class CLoggingSystem {
 public:
  ...
  LoggingChannelID_t RegisterLoggingChannel(
      const char *pChannelName, RegisterTagsFunc registerTagsFunc,
  ...
  // The main purpose is catching m_nChannelCount being zero because no channels
  ...
  bool IsChannelEnabled(LoggingChannelID_t channelID,
                        LoggingSeverity_t severity) const {
    return IsValidChannelID(channelID) &&
           GetChannel(channelID)->IsEnabled(severity);
  }
  ...
  void SetChannelSpewLevel(LoggingChannelID_t channelID,
                           LoggingSeverity_t minimumSeverity);
  void SetChannelSpewLevelByName(const char *pName,
                                 LoggingSeverity_t minimumSeverity);
  void SetChannelSpewLevelByTag(const char *pTag,
                                LoggingSeverity_t minimumSeverity);
  ...
  void SetChannelFlags(LoggingChannelID_t channelID,
  ...
  // in the current state being cleared by default (no listeners, default
  // logging response policy). Set bClearState to false to copy the existing
  // listener pointers to the new state.
  //
  ...
  LoggingResponse_t LogDirect(LoggingChannelID_t channelID,
                              LoggingSeverity_t severity, Color color,
  ...
  struct LoggingChannel_t {
    bool HasTag(const char *pTag) const {
      ...
      while (pCurrentTag != NULL) {
        if (_stricmp(pCurrentTag->m_pTagName, pTag) == 0) {
          return true;
        }
    ...
    LoggingChannelFlags_t
        m_Flags;  // an OR'd combination of LoggingChannelFlags_t
    LoggingSeverity_t m_MinimumSeverity;  // The minimum severity level required
 ...
 private:
  // Represents the current state of the logger (registered listeners, response
  // policy class, etc.) and can vary from thread-to-thread.  It can also be
  // pushed/popped to save/restore listener/response state.
  struct LoggingState_t {
    // Index of the previous entry on the listener set stack.
    int m_nPreviousStackEntry;

    // Number of active listeners in this set.  Cannot exceed
    // MAX_LOGGING_LISTENER_COUNT. If set to -1, implies that this state
    ...
    // Specific policy class to determine behavior of logging system under
  ...
  // (which are supposed to be registered using the macros at static/global init
  // time). It is assumed that this mutex is reentrant safe on all platforms.
  CThreadFastMutex *m_pStateMutex;

  // The index of the current "global" state of the logging system.  By default,
  // all threads use this state for logging unless a given thread has pushed the
  // logging state with bThreadLocal == true. If a thread-local state has been
  // pushed, g_nThreadLocalStateIndex (a global thread-local integer) will be
...
// This macro will resolve to the most appropriate overload of
// LoggingSystem_Log() depending on the number of parameters passed in.
#ifdef DBGFLAG_STRINGS_STRIP
#define InternalMsg(Channel, Severity, /* [Color], Message, */...) \
  do {                                                             \
    if (Severity == LS_ERROR &&                                    \
        LoggingSystem_IsChannelEnabled(Channel, Severity))         \
      LoggingSystem_Log(Channel, Severity,                         \
                        /* [Color], Message, */##__VA_ARGS__);     \
  } while (0)
#else
#define InternalMsg(Channel, Severity, /* [Color], Message, */...) \
  do {                                                             \
...
#define DEFINE_LOGGING_CHANNEL_NO_TAGS(Channel, ChannelName,                  \
                                       /* [Flags], [Severity], [Color] */...) \
  LoggingChannelID_t Channel =                                                \
      LoggingSystem_RegisterLoggingChannel(ChannelName, NULL, ##__VA_ARGS__)

#define BEGIN_DEFINE_LOGGING_CHANNEL(Channel, ChannelName,                  \
                                     /* [Flags], [Severity], [Color] */...) \
  static void Register_##Channel##_Tags();                                  \
  LoggingChannelID_t Channel = LoggingSystem_RegisterLoggingChannel(        \
      ChannelName, Register_##Channel##_Tags, ##__VA_ARGS__);               \
...
// For documentation on these functions, please look at the corresponding
// function in CLoggingSystem (unless otherwise specified).
PLATFORM_INTERFACE LoggingChannelID_t LoggingSystem_RegisterLoggingChannel(
    const char *pName, RegisterTagsFunc registerTagsFunc, int flags = 0,
    LoggingSeverity_t severity = LS_MESSAGE,
    Color color = UNSPECIFIED_LOGGING_COLOR);

PLATFORM_INTERFACE void LoggingSystem_RegisterLoggingListener(
    ILoggingListener *pListener);
PLATFORM_INTERFACE void LoggingSystem_UnregisterLoggingListener(
    ILoggingListener *pListener);
PLATFORM_INTERFACE void LoggingSystem_ResetCurrentLoggingState();
PLATFORM_INTERFACE void LoggingSystem_SetLoggingResponsePolicy(
    ILoggingResponsePolicy *pResponsePolicy);
// NOTE: PushLoggingState() saves the current logging state on a stack and
// results in a new clear state (no listeners, default logging response policy).
PLATFORM_INTERFACE void LoggingSystem_PushLoggingState(
    bool bThreadLocal = false, bool bClearState = true);
PLATFORM_INTERFACE void LoggingSystem_PopLoggingState(
    bool bThreadLocal = false);

PLATFORM_INTERFACE void LoggingSystem_AddTagToCurrentChannel(
    const char *pTagName);

// Returns INVALID_LOGGING_CHANNEL_ID if not found
PLATFORM_INTERFACE LoggingChannelID_t
LoggingSystem_FindChannel(const char *pChannelName);
...
PLATFORM_INTERFACE LoggingChannelID_t
LoggingSystem_GetNextChannelID(LoggingChannelID_t channelID);
PLATFORM_INTERFACE const CLoggingSystem::LoggingChannel_t *
LoggingSystem_GetChannel(LoggingChannelID_t channelID);

PLATFORM_INTERFACE bool LoggingSystem_HasTag(LoggingChannelID_t channelID,
                                             const char *pTag);

PLATFORM_INTERFACE bool LoggingSystem_IsChannelEnabled(
    LoggingChannelID_t channelID, LoggingSeverity_t severity);
PLATFORM_INTERFACE void LoggingSystem_SetChannelSpewLevel(
    LoggingChannelID_t channelID, LoggingSeverity_t minimumSeverity);
PLATFORM_INTERFACE void LoggingSystem_SetChannelSpewLevelByName(
    const char *pName, LoggingSeverity_t minimumSeverity);
PLATFORM_INTERFACE void LoggingSystem_SetChannelSpewLevelByTag(
    const char *pTag, LoggingSeverity_t minimumSeverity);

// Color is represented as an int32 due to C-linkage restrictions
PLATFORM_INTERFACE int32
LoggingSystem_GetChannelColor(LoggingChannelID_t channelID);
PLATFORM_INTERFACE void LoggingSystem_SetChannelColor(
    LoggingChannelID_t channelID, int color);

PLATFORM_INTERFACE LoggingChannelFlags_t
LoggingSystem_GetChannelFlags(LoggingChannelID_t channelID);
PLATFORM_INTERFACE void LoggingSystem_SetChannelFlags(
    LoggingChannelID_t channelID, LoggingChannelFlags_t flags);
...
LoggingSystem_LogAssert(const char *pMessageFormat, ...) FMTFUNCTION(1, 2);

#endif  // VPC_TIER0_LOGGING_H_