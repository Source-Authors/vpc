...
#ifndef VPC_TIER0_MEMALLOC_H_
#define VPC_TIER0_MEMALLOC_H_

// These memory debugging switches aren't relevant under Linux builds since
...
#if defined(_PS3)
// Define STEAM_SHARES_GAME_ALLOCATOR to make Steam use the game's tier0 memory
...
abstract_class IMemAlloc {
 ...
 public:
  ...
  virtual void *Expand_NoLongerSupported(void *pMem, size_t nSize,
                                         const char *pFileName, int nLine) = 0;
  ...
  inline void *IndirectAlloc(size_t nSize, const char *pFileName, int nLine) {
  ...
  virtual int CrtDbgReport(int nRptType, const char *szFile, int nLine,
                           const char *szModule, const char *pMsg) = 0;
  ...
  virtual void SetStatsExtraInfo(const char *pMapName,
  ...
  // Replacement for ::GlobalMemoryStatus which accounts for unused memory in
  // our system
  virtual void GlobalMemoryStatus(size_t * pUsedMemory,
                                  size_t * pFreeMemory) = 0;

  // Obtain virtual memory manager interface
  virtual IVirtualMemorySection *AllocateVirtualMemorySection(
      size_t numMaxBytes) = 0;

  // Request 'generic' memory stats (returns a list of N named values; caller
  // should assume this list will change over time)
...
inline void *MemAlloc_AllocAlignedUnattributed(size_t size, size_t align) {
  ...
  if (!ValueIsPowerOfTwo(align)) return NULL;
  ...
  if ((pAlloc = (unsigned char *)MemAlloc_Alloc(sizeof(void *) + align +
                                                size)) == (unsigned char *)NULL)
    return NULL;

  pResult =
      (unsigned char *)((size_t)(pAlloc + sizeof(void *) + align) & ~align);
...
inline void *MemAlloc_AllocAlignedFileLine(size_t size, size_t align,
  ...
  if ((pAlloc = (unsigned char *)MemAlloc_Alloc(sizeof(void *) + align + size,
                                                pszFile, nLine)) ==
      (unsigned char *)NULL)
    return NULL;
...
#ifdef USE_MEM_DEBUG
#define MemAlloc_AllocAligned(s, a) \
  MemAlloc_AllocAlignedFileLine(s, a, __FILE__, __LINE__)
#elif defined(USE_LIGHT_MEM_DEBUG)
extern const char *g_pszModule;
#define MemAlloc_AllocAligned(s, a) \
  MemAlloc_AllocAlignedFileLine(s, a, g_pszModule, 0)
#else
...
inline void *MemAlloc_ReallocAligned(void *ptr, size_t size, size_t align) {
  ...
  if (((size_t)ptr & (align - 1)) != 0) return NULL;
...
inline size_t MemAlloc_GetSizeAligned(void *pMemBlock) {
  ...
  // pAlloc is the pointer to the start of memory block
  pAlloc = *((void **)pAlloc);
  return g_pMemAlloc()->GetSize(pAlloc) -
         ((::byte *)pMemBlock - (::byte *)pAlloc);
...
// template here to allow adding alignment at levels of hierarchy that aren't
// the base
...
class CAlignedNewDelete : public T {
 public:
  ...
  void *operator new(size_t nSize, int nBlockUse, const char *pFileName,
                     int nLine) {
    return MemAlloc_AllocAlignedFileLine(nSize, bytesAlignment, pFileName,
                                         nLine);
  }
  ...
  void operator delete(void *pData, int nBlockUse, const char *pFileName,
                       int nLine) {
    if (pData) {
...
template <typename T>
const char *MemAllocClassName(T *p) {
  static const char *pszName =
      typeid(*p).name();  // @TODO: support having debug heap ignore certain
                          // allocations, and ignore memory allocated here
                          // [5/7/2009 tom]
  return pszName;
}
...
#define MEMALLOC_DEFINE_EXTERNAL_TRACKING(tag)                                 \
  static CUtlMap<void *, MemAllocFileLine_t, int> s_##tag##Allocs(             \
      DefLessFunc(void *));                                                    \
  CUtlMap<void *, MemAllocFileLine_t, int> *g_p##tag##Allocs =                 \
      &s_##tag##Allocs;                                                        \
  static CThreadFastMutex s_##tag##AllocsMutex;                                \
  CThreadFastMutex *g_p##tag##AllocsMutex = &s_##tag##AllocsMutex;             \
  const char *g_psz##tag##Alloc = strcpy(                                      \
      (char *)MemAlloc_Alloc(strlen(#tag "Alloc") + 1, "intentional leak", 0), \
      #tag "Alloc");
...
#define MemAlloc_RegisterExternalDeallocation(tag, p, size)                    \
  if (!p)                                                                      \
    ...
    AUTO_LOCK_FM(*g_p##tag##AllocsMutex);                                      \
    MemAllocFileLine_t fileLine = {g_psz##tag##Alloc, 0};                      \
    CUtlMap<void *, MemAllocFileLine_t, int>::IndexType_t iRecordedFileLine =  \
        g_p##tag##Allocs->Find(p);                                             \
...
PLATFORM_INTERFACE void MemoryLogMessage(
    char const *s);  // throw a message into the memory log
...
// ApproximateProcessMemoryUsage returns the approximate memory footprint of
// this process.
...
inline size_t ApproximateProcessMemoryUsage(void) { return 0; }
#endif

#endif  // VPC_TIER0_MEMALLOC_H_
