...
#ifndef VPC_TIER0_PLATFORM_H_
#define VPC_TIER0_PLATFORM_H_
...
// They are mostly similar, but in a few places we need to distinguish between
// the two.
#if defined(__SNC__)
...
// We want to force the assert to be redefined, because the STD assert might
// have been included and redefined. ps3_assert.h will do a check for assert
// being redefined. #include "ps3/ps3_assert.h"
#ifndef COMPILER_PS3
#error \
...
// setup the 360 environment here !once! for much less leaf module include
// wackiness these are critical order and purposely appear *before* anything
// else
#define _XBOX
#include <xaudio2.h>
#include <xbdm.h>
...
// There are known bugs in the PS3 optimizer.  The following macros allow us to
// lower optimization for a subset of a file If you run into build problems with
// optimization on, try turning off optimization for the selected file.  If that
// fixes the problem, use process of elimination and the below macros to find
// the bare minimum that needs to be unoptimized and report the compiler issue
// to Sony as well.
//
// The correlation between optimization levels and numbers passed to the _Pragma
// xopt and postopt calls is as follows: See: Control-group reference tables /
// -Xshow
// .... xopt
// -O1 0
// -O2 5
// -O3 5
//
// These macros MUST be used in pairs - Otherwise, the compiler will barf 'At
// end of source: error 67: expected a "}"'

// xopt disables some of the miscellaneous optimizations
#if __option(xopt)
#define SN_OPT_DISABLE \
  extern "C++" {       \
  _Pragma("control %push xopt=0")
#define SN_OPT_ENABLE          \
  _Pragma("control %pop xopt") \
  }
#else  // !__option(xopt)
#define SN_OPT_DISABLE
#define SN_OPT_ENABLE
...
#if defined(LINUX) && \
    !defined(OSX)  // for havok we define both symbols, so don't let the osx
                   // build wander down here
...
inline __attribute__((always_inline)) __m128 __vec_mul(const __m128 a,
                                                       const __m128 b) {
...
inline __attribute__((always_inline)) __m128 __vec_rsqrt(const __m128 a) {
  ...
  return vec_madd(vec_nmsub(a, estimateSquared, VMX_ONE), halfEstimate,
                  estimate);
}
...
// Syntaxic sugar for multiply
inline __attribute__((always_inline)) __m128 __vec_div(const __m128 a,
                                                       const __m128 b) {
...
inline __attribute__((always_inline)) void __vec_st_unaligned(__m128 in,
                                                              float *out) {
  __m128 temp0 = vec_ld(0, out);
  __m128 temp1 = vec_ld(16, out);
  vector unsigned char align = vec_lvsr(0, out);
  vector unsigned char mask =
      vec_perm((vector unsigned char)(0), (vector unsigned char)(0xFF), align);
...
inline __attribute__((always_inline)) void __vec_st_unaligned3(__m128 in,
                                                               float *out) {
  __m128 temp0 = vec_ld(0, out);
  __m128 temp1 = vec_ld(16, out);
  vector unsigned char align = vec_lvsr(0, out);
  vector unsigned char mask =
      vec_perm((vector unsigned char)(0),
               (vector unsigned char)(0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
// MSVC CRT uses 0x7fff while gcc uses MAX_INT, leading to mismatches between
// platforms As a result, we pick the least common denominator here.  This
// should be used anywhere you might typically want to use RAND_MAX
#define VALVE_RAND_MAX 0x7fff
// Maximum and minimum representable values
#ifndef PLATFORM_OSX

// Prevent tons of warnings about limits redefinition.
#include <cstdint>
...
// standard C + windows headers. So, there's some painful overhead to disabling
// long
//-----------------------------------------------------------------------------
...
// gcc doesn't allow storage specifiers on explicit template instatiation, but
// visual studio needs them to avoid link errors.
...
#define STDCALL
#define __stdcall
#elif (CROSS_PLATFORM_VERSION >= 1) && !defined(PLATFORM_64BITS) && \
    !defined(COMPILER_PS3)
...
#else
#ifdef _PS3
// [IESTYN 7/29/2010] As of SDK 3.4.0, this causes bad code generation in
// NET_Tick::ReadFromBuffer in netmessages.cpp,
//                    which caused (seeming) random network packet corruption.
//                    It probably causes other bugs too.
...
//       It's probable that this shouldn't be an issue, but an experiment should
...
//-----------------------------------------------------------------------------
#if defined(COMPILER_SNC)
#pragma diag_suppress = 1700  // warning 1700: class "%s" has virtual functions
                              // but non-virtual destructor
// Uncomment the following line if you want to investigate a specific compiler
// remark without all the noise: #pragma diag_suppress=1700, 83, 162, 182, 192,
// 194, 229, 238, 262, 341, 382, 401, 402, 403, 481, 817, 828, 833, 1363, 1771,
...
static bool sPS3_SuppressAssertsInThisFile =
    false;  // you can throw this in the debugger to temporarily disable asserts
            // inside any particular .cpp module.
#define DebuggerBreak()                \
  if (!sPS3_SuppressAssertsInThisFile) \
    __builtin_snpause();  // <sergiy> from SNC Migration Guide, tw 31,1,1
#else
...
PLATFORM_INTERFACE void Plat_MessageBox(const char *pTitle,
...
// Visual Studio likes to put an underscore in front of anything that looks like
// a portable function.
...
inline void SetupFPUControlWord() { SetupFPUControlWordForceExceptions(); }
...
inline void SetupFPUControlWord() {
  ...
  // Favour compatibility over speed (make sure the VPU set to Java-compliant
  // mode) NOTE: the VPU *always* uses round-to-nearest
...
inline void StoreLittleDWord(uint32 *base, unsigned int dwordIndex,
                             uint32 dword) {
...
inline void StoreLittleInt64(uint64 *base, unsigned int nWordIndex,
                             uint64 nWord) {
...
PLATFORM_INTERFACE uint64
Plat_GetClockStart();  // Snapshot of the clock when app started.
...
// Convert a time_t (specified in nTime - seconds since Jan 1, 1970 UTC) to a
// local calendar time in a threadsafe and non-crash-prone way.
...
PLATFORM_INTERFACE void Plat_GetTimeString(struct tm *pTime, char *pOut,
                                           int nMaxBytes);

// converts a time_t to a struct tm without the local time conversion of
// ConvertToLocalTime
...
inline const char *GetPlatformExt(void) { return PLATFORM_EXT; }
...
// Watchdog timer support. Call BeginWatchdogTimer( nn ) to kick the timer off.
// if you don't call EndWatchdogTimer within nn seconds, the program will kick
// off an exception. This is for making sure that hung dedicated servers abort
// (and restart) instead of staying hung. Calling EndWatchdogTimer more than
// once or when there is no active watchdog is fine. Only does anything under
// linux right now. It should be possible to implement this functionality in
// windows via a thread, if desired.

#if defined(POSIX) && !defined(_PS3)

PLATFORM_INTERFACE void BeginWatchdogTimer(int nSecs);
PLATFORM_INTERFACE void EndWatchdogTimer(void);
PLATFORM_INTERFACE void ResetBaseTime(
    void);  // reset plat_floattime to 0 for a subprocess
#else
FORCEINLINE void BeginWatchdogTimer(int) {}

FORCEINLINE void EndWatchdogTimer() {}
// reset plat_floattime to 0 for a subprocess
FORCEINLINE void ResetBaseTime() {}
...
#endif
PLATFORM_INTERFACE const char *GetPlatformSpecificFileName(
    const char *FileName);
