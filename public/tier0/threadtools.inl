...
// This file is included in threadtools.h for PS3 and threadtools.cpp for all
// other platforms
//
// Do not #include other files here

#ifndef _PS3
// this is defined in the .cpp for the PS3 to avoid introducing a dependency for
// files including the header
CTHREADLOCALPTR(CThread) g_pCurThread;

#define INLINE_ON_PS3
#else
// Inlining these functions on PS3 (which are called across PRX boundaries)
// saves us over 1ms per frame
...
INLINE_ON_PS3 CThread::~CThread() {
...
#endif
  {
    if (IsAlive()) {
      ...
      (void)DoNewAssertDialog(
          __FILE__, __LINE__,
          "Illegal termination of worker thread! Threads must negotiate an end "
          "to the thread before the CThread object is destroyed.\n");
#endif
  ...
  if (m_threadZombieId) {
    // just clean up zombie threads immediately (the destructor is fired from
    // the hosting thread)
    Join();
  }
#endif
...
#elif defined(POSIX)
    _snprintf(m_szName, sizeof(m_szName) - 1, "Thread(%p/0x%x)", this,
              (uint)m_threadId);
#endif
...
INLINE_ON_PS3 bool CThread::Start(unsigned nBytesStack,
                                  ThreadPriorityEnum_t nPriority) {
  AUTO_LOCK(m_Lock);
  ...
  m_hThread = (HANDLE)CreateThread(
      NULL, nBytesStack, (LPTHREAD_START_ROUTINE)GetThreadProc(),
      new ThreadInit_t(init), 0, (LPDWORD)&m_threadId);
...
#elif PLATFORM_PS3
  // On the PS3, a stack size of 0 doesn't imply a default stack size, so we
  // need to force it to our
  //		 own default size.
  if (nBytesStack == 0) {
  ...
  if (sys_ppu_thread_create(&m_threadId, (void (*)(uint64_t))GetThreadProc(),
                            (uint64_t)(new ThreadInit_t(init)), nPriority,
                            nBytesStack, SYS_PPU_THREAD_CREATE_JOINABLE,
                            threadName) != CELL_OK) {
  ...
  if (pthread_create(&m_threadId, &attr, (void *(*)(void *))GetThreadProc(),
...
#ifdef PLATFORM_WINDOWS
  DWORD dwExitCode;
  return (m_hThread && GetExitCodeThread(m_hThread, &dwExitCode) &&
...
#endif
  {
    AssertMsg(GetCurrentCThread() != this,
              _T("Thread cannot be joined with self"));

#ifdef _WIN32
    return ThreadJoin((ThreadHandle_t)m_hThread, timeout);
#elif defined(POSIX)
    bool ret = ThreadJoin(
        (ThreadHandle_t)(m_threadId ? m_threadId : m_threadZombieId), timeout);
    m_threadZombieId = 0;
    return ret;
#endif
  }
...
INLINE_ON_PS3 int CThread::GetResult() { return m_result; }
...
#else
    AssertMsg(false,
              "Called CThread::Stop() for a platform that doesn't have it!\n");
#endif
  } else
...
INLINE_ON_PS3 unsigned CThread::Suspend() {
  AssertMsg(ThreadGetCurrentId() == (ThreadId_t)m_threadId,
...
INLINE_ON_PS3 bool CThread::Init() { return true; }

//---------------------------------------------------------

#if defined(_PS3)
INLINE_ON_PS3 int CThread::Run() { return -1; }
#endif  // _PS3
...
#endif
{
...
#if defined(THREAD_PARENT_STACK_TRACE_ENABLED)
  CStackTop_ReferenceParentStack stackTop(pInit->ParentStackTrace,
                                          ARRAYSIZE(pInit->ParentStackTrace));
#endif

  bool bInitSuccess = true;
  if (pInit->pfInitSuccess) *(pInit->pfInitSuccess) = false;
  ...
  *(pInit->pfInitSuccess) = pInit->pThread->Init();
  ...
  if (pInit->pfInitSuccess) *(pInit->pfInitSuccess) = bInitSuccess;
  pInit->pInitCompleteEvent->Set();
  if (!bInitSuccess) return 0;

  if (!Plat_IsInDebugSession() &&
      (pInit->pThread->m_flags & SUPPORT_STOP_PROTOCOL)) {
#ifndef _PS3
    try
#endif
    {
...
#error
...
#ifdef _PS3
  {
    ...
    // reacquire the lock in case thread exit didn't actually exit the thread,
    // so that AUTO_LOCK won't double-unlock the lock (to keep it paired)
    pThread->m_Lock.Lock();
  }
#endif
