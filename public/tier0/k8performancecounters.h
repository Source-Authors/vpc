...
#ifndef VPC_TIER0_K8PERFORMANCECOUNTERS_H_
#define VPC_TIER0_K8PERFORMANCECOUNTERS_H_
...
class k8BaseEvent {
 public:
  ...
  void SetFiltering(CompareState compareEnable, CompareMethod compareMethod,
                    uint8 threshold, EdgeState edgeEnable) {
    PerfEvtSel &select = eventSelect[eventSelectNum];
...
typedef union EVENT_MASK(k8_locked_op) {
  struct {
    ...
    uint16 NumCyclesInRequestGrant : 1;  // Number of cycles spent in the lock
                                         // request/grant stage
...
typedef union EVENT_MASK(k8_lock_accesses) {
  struct {
    uint16 DcacheAccesses : 1;  // Number of dcache accesses by lock
                                // instructions" },
    uint16
        DcacheMisses : 1;  // Number of dcache misses by lock instructions" } }
  };
  uint16 flat;
...
class k8Event_RETIRED_INSTRUCTIONS : public k8BaseEvent {
 public:
  k8Event_RETIRED_INSTRUCTIONS() {
    ...
    name =
        _T("Retired instructions (includes exceptions, interrupts, resyncs)");
  }
  EVENT_MASK(NULL_MASK) * eventMask;
};
...
class k8Event_RETIRED_BRANCHES : public k8BaseEvent {
 public:
  k8Event_RETIRED_BRANCHES() {
    ...
    name =
        _T("Retired branches (conditional, unconditional, exceptions, ")
        _T("interrupts)");
  }
  EVENT_MASK(NULL_MASK) * eventMask;
};
...
class k8Event_RETIRED_RESYNC_BRANCHES : public k8BaseEvent {
 public:
  k8Event_RETIRED_RESYNC_BRANCHES() {
    ...
    name =
        _T("Retired resync branches (only non-control transfer branches ")
        _T("counted)");
  }
  EVENT_MASK(NULL_MASK) * eventMask;
};
...
typedef union EVENT_MASK(k8_retired_fpu_instr) {
  struct {
    ...
    uint16 CombinedPackedSSE_SSE2 : 1;  // Combined packed SSE and SSE2
                                        // instructions" },
    uint16 CombinedScalarSSE_SSE2 : 1;  // Combined scalar SSE and SSE2
                                        // instructions" } }
  };
  uint16 flat;
...
class k8Event_DISPATCH_STALL_PENDING : public k8BaseEvent {
 public:
  k8Event_DISPATCH_STALL_PENDING() {
    eventMask = (EVENT_MASK(NULL_MASK) *)&m_eventMask;
    name =
        _T("Dispatch stall when far control transfer or resync branch is ")
        _T("pending");
    event_id = 0xDA;
    unitEncode = FR;
  }
...
typedef union EVENT_MASK(k8_bypass_counter_saturation) {
  struct {
    uint16 MEM_HighPriorityBypass : 1;  // Memory controller high priority
                                        // bypass" },
    uint16
        MEM_LowPriorityBypass : 1;  // Memory controller low priority bypass" },
...
typedef union EVENT_MASK(k8_probe_result) {
  struct {
    ...
    uint16 ProbeHitDirtyWithoutMemoryCancel : 1;  // Probe hit dirty without
                                                  // memory cancel" },
    uint16 ProbeHitDirtyWithMemoryCancel : 1;     // Probe hit dirty with memory
                                                  // cancel" } }
...
//    {
//        uint16 DcacheAccesses:1;       // Number of dcache accesses by lock
//        instructions" }, uint16 DcacheMisses:1;       // Number of dcache
//        misses by lock instructions" } }
//    };
//    uint16 flat;
//
//}EVENT_MASK(  k8_lock_accesses);
...
#endif  // VPC_TIER0_K8PERFORMANCECOUNTERS_H_
