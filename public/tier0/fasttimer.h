...
inline uint64 sys_time_get_timebase_frequency() {
  DebuggerBreak();  // Error("sys_time_get_timebase_frequency called on non-PS3
                    // platform.");
...
class CCycleCount {
 ...
 public:
  ...
  void Sample();  // Sample the clock. This takes about 34 clocks to execute (or
                  // 26,000 calls per millisecond on a P900).
...
#elif defined(_X360)
  // only need lower 32 bits, avoids doc'd read bug and 32 bit rollover is in 85
  // seconds
...
inline void CFastTimer::End() {
  ...
  if (IsX360()) {
    ...
    // more than one overflow should not have occurred, otherwise caller should
    // use a slower timer
...
inline void CLimitTimer::SetLimit(uint64 m_cMicroSecDuration) {
  uint64 dlCycles =
      ((uint64)m_cMicroSecDuration * (uint64)g_dwClockSpeed) / (uint64)1000000L;
