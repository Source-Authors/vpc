...
#ifndef VPC_TIER0_STACKSTATS_H_
#define VPC_TIER0_STACKSTATS_H_
...
#else
template <class _Ty>
class CNullStatsGathererAllocator {
  CNullStatsGathererAllocator(void) {}
};
...
struct CCallStackStatsGatherer_FunctionTable_t {
  void (*pfn_GetDumpInfo)(void *, const char *&, size_t &, size_t &, void *&,
                          size_t &, CCallStackStatsGatherer_Standardized_t *&,
                          size_t &);
  void (*pfn_PushSubTree)(void *,
                          const CCallStackStatsGatherer_Standardized_t &,
                          const CCallStackStorage &);
...
// templatized classes are fun, can't really use a base pointer effectively, so
// we'll have a translator struct and store pointers to instances of the
// translator function
class CCallStackStatsGatherer_Standardized_t {
 public:
  ...
  // helper code
  void PushSubTree(
      const CCallStackStatsGatherer_Standardized_t &SubTree,
      const CCallStackStorage &PushStack = CCallStackStorage()) const {
...
// Designed to be called by an instance of CCallStackStatsGatherer to dump it's
// data
PLATFORM_INTERFACE bool _CCallStackStatsGatherer_Internal_DumpStatsToFile(
...
class CCallStackStatsGatherer_StructAccessor_Base {
 ...
 protected:
  uint32
      m_iEntryIndex;  // index of the stat entry we want in the vector. Stored
                      // as index as the vector base address can change.
  CCallStackStatsGatherer_Standardized_t
      m_Gatherer;  // so we can lock the vector memory in place while
...
class CCallStackStatsGatherer_StatMutexBase {
 public:
  ...
  }  // true to increase lock refcount, false to decrease
};
...
// STATSTRUCT - The structure you'll use to track whatever it is you're
...
template <class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH,
          ...
          typename STATMUTEXHANDLER = CCallStackStatsGatherer_StatMutexPool<4>,
          template <typename T> class TEMPLATIZEDMEMORYALLOCATOR =
              CDefaultStatsGathererAllocator>
 ...
 public:
  ...
  CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> GetEntry(
      const CCallStackStorage &PushStack =
          CCallStackStorage(STACKACQUISITIONFUNCTION));
  CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> GetEntry(
      uint32 iEntryIndex);

  // get the entry index for the caller's current call stack. Pre-nullified
  // entries count as valid entries (and save re-nullification work)
  uint32 GetEntryIndex(
      void *const CallStack[CAPTUREDCALLSTACKLENGTH],
      uint32 iValidEntries);  // index is unchanging, safe to keep and use later
                              // (designed for exactly that purpose)
  uint32 GetEntryIndex(const CCallStackStorage &PushStack =
                           CCallStackStorage(STACKACQUISITIONFUNCTION));
  ...
  void GetCallStackForIndex(uint32 iEntryIndex,
                            void *CallStackOut[CAPTUREDCALLSTACKLENGTH]);
  ...
  static void GetDumpInfo(void *pThis, const char *&szStructName,
                          size_t &iCapturedStackLength,
                          size_t &iEntrySizeWithStack, void *&pEntries,
                          size_t &iEntryCount,
                          CCallStackStatsGatherer_Standardized_t *&pSubTrees,
                          size_t &iSubTreeCount);
  static void PushSubTree(void *pParent,
                          const CCallStackStatsGatherer_Standardized_t &SubTree,
                          const CCallStackStorage &PushStack);

  void PushSubTree(CCallStackStatsGatherer_Standardized_t &Parent,
                   const CCallStackStorage &PushStack =
                       CCallStackStorage(STACKACQUISITIONFUNCTION));

  static void PopSubTree(void *pParent);
  static void SyncMutexes(void *pParent,
                          bool bLock);  // true for lock, false for unlock
  ...
  const static FN_GetCallStack
      StackFunction;  // publish the requested acquisition function so you can
                      // easily key all your stack gathering to the class
                      // instance
...
#pragma pack(pop)
  ...
  typedef CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
                                  STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
  ...
  struct IndexMapKey_t {
    ...
    bool operator<(const IndexMapKey_t &key) const {
      if (m_Hash != key.m_Hash) return m_Hash < key.m_Hash;

      // only here if there's a hash match. Do a full check. Extremely likely to
      // be the exact same call stack. But not 100% guaranteed.
      for (int i = 0; i < CAPTUREDCALLSTACKLENGTH; ++i) {
  ...
  uint32 PatchInSubTrees(void *const CallStackIn[CAPTUREDCALLSTACKLENGTH],
                         void *CallStackOut[CAPTUREDCALLSTACKLENGTH],
    ...
    for (treeIter = m_PushedSubTrees.rbegin();
         treeIter != m_PushedSubTrees.rend(); ++treeIter) {
    ...
    // char szTemp[4096];
    // TranslateStackInfo( CallStackIn, CAPTUREDCALLSTACKLENGTH, szTemp, sizeof(
    // szTemp ), "\n\t" );
    ...
    void *pMatchAddress =
        treeIter->Stack[1];  // while the first entry is where the actual push
                             // was made. The second entry is the first that is
                             // matchable in most cases
    ...
    for (i = 0; i < iValidEntries; ++i) {
      if (CallStackIn[i] == pMatchAddress) {
        ...
        CallStackOut[i] =
            treeIter->tree
                .pGatherer;  // tag this entry as leading into the sub-tree
  ...
  typedef std::vector<StackAndStats_t,
                      TEMPLATIZEDMEMORYALLOCATOR<StackAndStats_t>>
      StatVector_t;
  typedef std::map<
      IndexMapKey_t, StatIndex_t, std::less<IndexMapKey_t>,
      TEMPLATIZEDMEMORYALLOCATOR<std::pair<const IndexMapKey_t, StatIndex_t>>>
      IndexMap_t;
  ...
  typedef std::vector<PushedSubTree_t,
                      TEMPLATIZEDMEMORYALLOCATOR<PushedSubTree_t>>
      PushedSubTreeVector_t;
  PushedSubTreeVector_t m_PushedSubTrees;

  typedef std::vector<
      CCallStackStatsGatherer_Standardized_t,
      TEMPLATIZEDMEMORYALLOCATOR<CCallStackStatsGatherer_Standardized_t>>
      StoredSubTreeVector_t;
  StoredSubTreeVector_t m_StoredSubTrees;

  CThreadFastMutex m_IndexMapMutex;
  CThreadFastMutex m_SubTreeMutex;

  // only for locking the memory in place, locked for write when the entry
  // addresses might change. Locked for read when you've claimed you're
  // manipulating a value. You're on your own for making sure two threads don't
  ...
  static size_t NULL_DescribeCallStackStatStruct(uint8 *pDescribeWriteBuffer,
                                                 size_t iDescribeMaxLength) {
    return 0;
  }
...
template <class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH,
          ...
          template <typename T> class TEMPLATIZEDMEMORYALLOCATOR>
const FN_GetCallStack CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
    STATMUTEXHANDLER, TEMPLATIZEDMEMORYALLOCATOR>::StackFunction =
    STACKACQUISITIONFUNCTION;

template <class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH,
          FN_GetCallStack STACKACQUISITIONFUNCTION, typename STATMUTEXHANDLER,
          template <typename T> class TEMPLATIZEDMEMORYALLOCATOR>
const size_t CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
    STATMUTEXHANDLER, TEMPLATIZEDMEMORYALLOCATOR>::CapturedCallStackLength =
    CAPTUREDCALLSTACKLENGTH;

#if defined(ENABLE_STACK_STATS_GATHERING)

class CallStackStatStructDescFuncs;
PLATFORM_INTERFACE size_t _CCallStackStatsGatherer_Write_FieldDescriptions(
    CallStackStatStructDescFuncs *pFieldDescriptions, uint8 *pWriteBuffer,
    size_t iWriteBufferSize);
// PLATFORM_INTERFACE size_t _CCallStackStatsGatherer_Write_FieldMergeScript(
// CallStackStatStructDescFuncs *pFieldDescriptions,
// CallStackStatStructDescFuncs::MergeScript_Language scriptMergeLanguage, uint8
// *pWriteBuffer, size_t iWriteBufferSize );

#define DECLARE_CALLSTACKSTATSTRUCT()                                    \
  static const char *STATSTRUCTSTRINGNAME;                               \
  static size_t DescribeCallStackStatStruct(uint8 *pDescribeWriteBuffer, \
                                            size_t iDescribeMaxLength);

#define BEGIN_STATSTRUCTDESCRIPTION(className)                               \
  const char *className::STATSTRUCTSTRINGNAME = #className;                  \
  size_t className::DescribeCallStackStatStruct(uint8 *pDescribeWriteBuffer, \
                                                size_t iDescribeMaxLength) { \
...
#define DECLARE_CALLSTACKSTATSTRUCT_FIELDDESCRIPTION() \
  static CallStackStatStructDescFuncs *GetStatStructFieldDescriptions(void);

#define BEGIN_STATSTRUCTFIELDDESCRIPTION(className)                        \
  CallStackStatStructDescFuncs *className::GetStatStructFieldDescriptions( \
      void) {                                                              \
...
#define _DEFINE_STATSTRUCTFIELD_VARNAME(varName, fieldName, fieldStruct,      \
                                        fieldParmsInParentheses)              \
  static fieldStruct varName##_desc##fieldParmsInParentheses;                 \
...
#define DEFINE_STATSTRUCTFIELD(fieldName, fieldStruct,               \
                               fieldParmsInParentheses)              \
  _DEFINE_STATSTRUCTFIELD_VARNAME(fieldName, fieldName, fieldStruct, \
                                  fieldParmsInParentheses)
#define DEFINE_STATSTRUCTFIELD_ARRAYENTRY(arrayName, arrayIndex, fieldStruct, \
                                          fieldParmsInParentheses)            \
  _DEFINE_STATSTRUCTFIELD_VARNAME(arrayName##_##arrayIndex,                   \
                                  arrayName##[##arrayIndex##], fieldStruct,   \
                                  fieldParmsInParentheses)

#define END_STATSTRUCTFIELDDESCRIPTION()                              \
  static CallStackStatStructDescFuncs *s_pHeadStruct = _pHeadLinkage; \
...
template <class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH,
          FN_GetCallStack STACKACQUISITIONFUNCTION, typename STATMUTEXHANDLER,
          template <typename T> class TEMPLATIZEDMEMORYALLOCATOR>
CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
...
CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
    STATMUTEXHANDLER, TEMPLATIZEDMEMORYALLOCATOR>::
    GetEntry(void *const CallStack[CAPTUREDCALLSTACKLENGTH],
             uint32 iValidEntries)  // same as above, but does the work of
                                    // nullifying invalid entries
{
#if defined(ENABLE_STACK_STATS_GATHERING)
...
template <class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH,
          FN_GetCallStack STACKACQUISITIONFUNCTION, typename STATMUTEXHANDLER,
          template <typename T> class TEMPLATIZEDMEMORYALLOCATOR>
uint32 CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
                               STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
                               TEMPLATIZEDMEMORYALLOCATOR>::
...
CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
    STATMUTEXHANDLER, TEMPLATIZEDMEMORYALLOCATOR>::GetEntry(uint32
                                                                iEntryIndex) {
#if defined(ENABLE_STACK_STATS_GATHERING)
  return CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT>(Standardized(),
                                                                 iEntryIndex);
...
uint32 CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
    ...
    GetEntryIndex(
        void *const CallStack[CAPTUREDCALLSTACKLENGTH],
        uint32 iValidEntries)  // index is unchanging, safe to keep and use
                               // later (designed for exactly that purpose)
{
#if defined(ENABLE_STACK_STATS_GATHERING)
  ...
  // if we have a sub-tree. We'll be splicing it into the original call stack as
  // if it were a return address. Then patching that when we interpret the
  // results later A stack with a sub-tree along the line is treated as
  // distinctly different than one without a sub-tree
  ...
  indexMapIter = m_IndexMap.insert(
      IndexMapEntry_t(IndexMapKey_t(PatchedStack), StatIndex_t()));
  ...
  if (indexMapIter.first->second.m_Index == -1) {
    ...
    memcpy(m_StatEntries[indexMapIter.first->second.m_Index].m_CallStack,
           PatchedStack, sizeof(void *) * CAPTUREDCALLSTACKLENGTH);
...
template <class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH,
          FN_GetCallStack STACKACQUISITIONFUNCTION, typename STATMUTEXHANDLER,
          template <typename T> class TEMPLATIZEDMEMORYALLOCATOR>
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
                             STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
                             TEMPLATIZEDMEMORYALLOCATOR>::
...
#else
  for (size_t i = 0; i != CAPTUREDCALLSTACKLENGTH; ++i) CallStackOut[i] = NULL;
#endif
}
...
template <class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH,
          FN_GetCallStack STACKACQUISITIONFUNCTION, typename STATMUTEXHANDLER,
          template <typename T> class TEMPLATIZEDMEMORYALLOCATOR>
size_t CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
    STATMUTEXHANDLER, TEMPLATIZEDMEMORYALLOCATOR>::NumEntries(void) const {
#if defined(ENABLE_STACK_STATS_GATHERING)
...
template <class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH,
          FN_GetCallStack STACKACQUISITIONFUNCTION, typename STATMUTEXHANDLER,
          template <typename T> class TEMPLATIZEDMEMORYALLOCATOR>
bool CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
    STATMUTEXHANDLER,
    TEMPLATIZEDMEMORYALLOCATOR>::DumpToFile(const char *szFileName,
                                            bool bAllowMemoryAllocations) {
#if defined(ENABLE_STACK_STATS_GATHERING)
  CCallStackStatsGatherer_Standardized_t StandardThis = Standardized();
  SyncMutexes(this, true);
  bool bRetVal = _CCallStackStatsGatherer_Internal_DumpStatsToFile(
      szFileName, StandardThis, bAllowMemoryAllocations);
...
template <class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH,
          FN_GetCallStack STACKACQUISITIONFUNCTION, typename STATMUTEXHANDLER,
          template <typename T> class TEMPLATIZEDMEMORYALLOCATOR>
const CCallStackStatsGatherer_FunctionTable_t &CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
...
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
                             STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
                             TEMPLATIZEDMEMORYALLOCATOR>::
    GetDumpInfo(void *pThis, const char *&szStructName,
                size_t &iCapturedStackLength, size_t &iEntrySizeWithStack,
                void *&pEntries, size_t &iEntryCount,
                CCallStackStatsGatherer_Standardized_t *&pSubTrees,
                size_t &iSubTreeCount) {
  ThisCast *pThisCast = (ThisCast *)pThis;
  iCapturedStackLength = CAPTUREDCALLSTACKLENGTH;
  iEntrySizeWithStack = sizeof(
      CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
                              STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
                              TEMPLATIZEDMEMORYALLOCATOR>::StackAndStats_t);
...
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
  ...
  memcpy(
      pushVal.Stack, PushStack.pStack,
      MIN(CAPTUREDCALLSTACKLENGTH, PushStack.iValidEntries) * sizeof(void *));
...
void CCallStackStatsGatherer<STATSTRUCT, CAPTUREDCALLSTACKLENGTH,
                             STACKACQUISITIONFUNCTION, STATMUTEXHANDLER,
                             TEMPLATIZEDMEMORYALLOCATOR>::
    PushSubTree(CCallStackStatsGatherer_Standardized_t &Parent,
                const CCallStackStorage &PushStack) {
#if defined(ENABLE_STACK_STATS_GATHERING)
  CCallStackStatsGatherer_Standardized_t StandardThis = Standardized();
...
void *CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
    STATMUTEXHANDLER,
    TEMPLATIZEDMEMORYALLOCATOR>::GetEntry(void *pParent, uint32 iEntryIndex) {
  ThisCast *pParentCast = (ThisCast *)pParent;
#if defined(ENABLE_STACK_STATS_GATHERING)
...
void CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
    STATMUTEXHANDLER,
    TEMPLATIZEDMEMORYALLOCATOR>::ApplyTreeAccessLock(void *pParent,
                                                     bool bLock) {
#if defined(ENABLE_STACK_STATS_GATHERING)
  ThisCast *pParentCast = (ThisCast *)pParent;
  if (bLock) {
...
void CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
    STATMUTEXHANDLER, TEMPLATIZEDMEMORYALLOCATOR>::LockEntry(void *pParent,
                                                             uint32 iEntryIndex,
                                                             bool bLock) {
#if defined(ENABLE_STACK_STATS_GATHERING)
  ThisCast *pParentCast = (ThisCast *)pParent;
...
template <class STATSTRUCT, size_t CAPTUREDCALLSTACKLENGTH,
          FN_GetCallStack STACKACQUISITIONFUNCTION, typename STATMUTEXHANDLER,
          template <typename T> class TEMPLATIZEDMEMORYALLOCATOR>
CCallStackStatsGatherer_Standardized_t CCallStackStatsGatherer<
    STATSTRUCT, CAPTUREDCALLSTACKLENGTH, STACKACQUISITIONFUNCTION,
    STATMUTEXHANDLER, TEMPLATIZEDMEMORYALLOCATOR>::Standardized(void) {
  return CCallStackStatsGatherer_Standardized_t(this, GetFunctionTable());
}
...
class PLATFORM_CLASS CallStackStatStructDescFuncs {
 public:
  ...
  // 1 byte version per entry. Assuming that the field will get more descriptive
  // over time, reserving this now.
  virtual size_t DescribeField(uint8 *pDescribeWriteBuffer,
                               size_t iDescribeMaxLength) = 0;

  enum MergeScript_Language {
    SSMSL_Squirrel,  // Only support squirrel for now, theoretically expandable
                     // to any vscript supported language
  };
  ...
  CallStackStatStructDescFuncs
      *m_pNext;  // needed for how the description macros are laid out. Couldn't
                 // figure out a way to store the static struct instances as a
                 // static array
};
...
#define BSSFT_UINT \
  (sizeof(unsigned int) == sizeof(uint32) ? BSSFT_UINT32 : BSSFT_UINT64)
#define BSSFT_SIZE_T \
  (sizeof(size_t) == sizeof(uint32) ? BSSFT_UINT32 : BSSFT_UINT64)
...
class PLATFORM_CLASS BasicStatStructFieldDesc
    : public CallStackStatStructDescFuncs {
 public:
...
// struct is locked in place while you're holding onto one of these. Get a base,
// then create one of these from it
template <class STATSTRUCT>
class CCallStackStatsGatherer_StructAccessor_AutoLock
    : CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> {
 public:
  CCallStackStatsGatherer_StructAccessor_AutoLock(
      CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> &copyFrom)
      : CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT>(copyFrom) {
    this->m_Gatherer.pFunctionTable->pfn_ApplyTreeAccessLock(
        this->m_Gatherer.pGatherer, true);
    this->m_Gatherer.pFunctionTable->pfn_LockEntry(this->m_Gatherer.pGatherer,
                                                   this->m_iEntryIndex, true);
    this->m_pStruct =
        (STATSTRUCT *)this->m_Gatherer.pFunctionTable->pfn_GetEntry(
            this->m_Gatherer.pGatherer, this->m_iEntryIndex);
  }

  ~CCallStackStatsGatherer_StructAccessor_AutoLock(void) {
    this->m_Gatherer.pFunctionTable->pfn_LockEntry(this->m_Gatherer.pGatherer,
                                                   this->m_iEntryIndex, false);
    this->m_Gatherer.pFunctionTable->pfn_ApplyTreeAccessLock(
        this->m_Gatherer.pGatherer, false);
  }

  STATSTRUCT *operator->() { return this->m_pStruct; }

  STATSTRUCT *GetStruct(
      void)  // do not hold this pointer outside the lock period
  {
...
// struct is locked in place only between Lock() and paired Unlock() calls. Get
// a base, then create one of these from it. It's safe to hold onto this for an
// extended period of time. The entry index is unchanging in the gatherer tree.
template <class STATSTRUCT>
class CCallStackStatsGatherer_StructAccessor_Manual
    : CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> {
 public:
  CCallStackStatsGatherer_StructAccessor_Manual(
      CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT> &copyFrom)
      : CCallStackStatsGatherer_StructAccessor_Base<STATSTRUCT>(copyFrom),
        m_pStruct(NULL) {}
...
class CCallStackStats_PushSubTree_AutoPop {
 public:
  CCallStackStats_PushSubTree_AutoPop(
      const CCallStackStatsGatherer_Standardized_t &Parent,
      const CCallStackStatsGatherer_Standardized_t &Child,
      const CCallStackStorage &PushStack = CCallStackStorage())
      : m_PopFrom(Parent) {
    Parent.pFunctionTable->pfn_PushSubTree(Parent.pGatherer, Child, PushStack);
  }
  ~CCallStackStats_PushSubTree_AutoPop(void) { m_PopFrom.PopSubTree(); }

  CCallStackStatsGatherer_Standardized_t m_PopFrom;
};

#endif  // VPC_TIER0_STACKSTATS_H_
