...
#ifndef VPC_TIER0_VPROF_H_
#define VPC_TIER0_VPROF_H_
...
#define VPROF_BUDGETGROUP_BRUSH_FAST_PATH_RENDERING \
  _T("Fast Path Brush Rendering")
...
#define VPROF_1(name, group, assertAccounted, budgetFlags) \
  CVProfSnMarkerScope VProfSn_(name)
#else
...
#define VPROF_SET_COUNTER(name, amount)  \
  do {                                   \
    static CVProfCounter _counter(name); \
    _counter.Set(amount);                \
  } while (0)
#define VPROF_SET_GROUP_COUNTER(name, group, amount) \
  do {                                               \
    static CVProfCounter _counter(name, group);      \
    _counter.Set(amount);                            \
  } while (0)
...
class PLATFORM_CLASS CVProfNode {
 ...
 public:
  ...
  inline bool IsL2andLHSEnabled(void) { return (m_iBitFlags & kRecordL2) != 0; }
...
enum CounterGroup_t {
  ...
  COUNTER_GROUP_NO_RESET,  // The engine doesn't reset these counters. Usually,
                           // they are used like global variables that can be
  ...
  COUNTER_GROUP_GRAPHICS_PER_FRAME,  // Misc graphics counters that are reset
                                     // each frame
};
...
class PLATFORM_CLASS CVProfile {
 public:
  ...
  // the CPU trace mode is actually a small state machine; it can be off, primed
  // for single capture, primed for everything-in-a-frame capture, or currently
  // in everything-in-a-frame capture.
  enum CPUTraceState {
    kDisabled,
    kFirstHitNode,  // record from the first time we hit the node until that
                    // node ends
    kAllNodesInFrame_WaitingForMark,  // we're going to record all the times a
                                      // node is hit in a frame, but are waiting
    ...
    kAllNodesInFrame_WaitingForMarkMultiFrame,  // we're going to record all the
                                                // times a node is hit in a
                                                // frame, but are waiting for
                                                // the frame to start
    kAllNodesInFrame_RecordingMultiFrame,
  };

  // Global switch to turn CPU tracing on or off at all. The idea is you set up
  // a node first, then trigger tracing by throwing this to true. It'll reset
  // back to false after the trace happens.
  inline CPUTraceState GetCPUTraceMode();
  inline void SetCPUTraceEnabled(CPUTraceState enabled,
                                 bool bTraceCompleteEvent = false,
                                 int nNumFrames = -1);
  inline void
  IncrementMultiTraceIndex();  // tick up the counter that gets appended to the
                               // multi-per-frame traces
...
#define MAX_GROUP_STACK_DEPTH 1024
  ...
  void HideBudgetGroup(const tchar *pszName, bool bHide = true) {
    HideBudgetGroup(BudgetGroupNameToBudgetGroupID(pszName), bHide);
  }

  int *FindOrCreateCounter(
      const tchar *pName, CounterGroup_t eCounterGroup = COUNTER_GROUP_DEFAULT);
...
#endif
{
  ...
  if (m_iUniqueNodeID > 0) {
    ...
    m_BudgetGroupID = 0;  // "m_Root" can't call BudgetGroupNameToBudgetGroupID
...
inline const tchar *CVProfNode::GetName() { return m_pszName; }
