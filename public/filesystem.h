...
#ifndef VPC_FILESYSTEM_H_
#define VPC_FILESYSTEM_H_
 ...
 public:
  ...
  void SetFileName(char const *filename) {
    ...
    if (len >= sizeof(m_szFilename)) {
      V_strncpy(m_szFilename, &filename[len - sizeof(m_szFilename) + 1],
                sizeof(m_szFilename));
    } else {
...
enum FilesystemOpenExFlags_t {
  ...
  // This makes it calculate a CRC for the file (if the file came
  // from disk) regardless of the IFileList passed to
...
enum FSAsyncFlags_t {
  ...
  // Actually perform the operation synchronously. Used to simplify client code
  // paths
...
//-----------------------------------------------------------------------------
typedef void (*FSAsyncScanAddFunc_t)(void *pContext, char *pFoundPath,
                                     char *pFoundFile);
...
struct FileAsyncRequest_t {
  ...
  const char *pszPathID;  // path ID (NOTE: this field is here to remain binary
  ...
  // custom allocator. can be null. not compatible with
...
// returns minor version component (not generally used, i.e. we dont rev dlc's
// yet)
...
abstract_class IFileSystem : public IAppSystem, public IBaseFileSystem {
 public:
  ...
  // (Was needed by the dedicated server b/c the "SteamAppId" env var only gets
  // passed to steam.dll at load time, so the dedicated couldn't pass it in that
  // way).
  ...
  // This is for optimization. If you mark a path ID as "by request only", then
  // files inside it will only be accessed if the path ID is specifically
  // requested. Otherwise, it will be ignored. If there are currently no search
  // paths with the specified path ID, then it will still remember it in case
  // you add search paths with this path ID.
  ...
  virtual const char *RelativePathToFullPath(
      ...
      int localPathBufferSize, PathTypeFilter_t pathFilter = FILTER_NONE,
      PathTypeQuery_t *pPathType = NULL) = 0;
#if IsGameConsole()
  // Given a relative path, gets the PACK file that contained this file and its
  // offset and size. Can be used to prefetch a file to a HDD for caching
  // reason.
  virtual bool GetPackFileInfoFromRelativePath(
      const char *pFileName, const char *pPathID, char *pPackPath,
      int nPackPathBufferSize, int64 &nPosition, int64 &nLength) = 0;
#endif
  // Returns the search path, each path is separated by ;s. Returns the length
  // of the string returned
  virtual int GetSearchPath(const char *pathID, bool bGetPackFiles, char *pPath,
                            int nMaxLen) = 0;
  ...
  // load/unload modules
  virtual CSysModule *LoadModule(const char *pFileName, const char *pPathID = 0,
  ...
  // Searches for a file in all paths and results absolute path names for the
  // file, works in pack files (zip and vpk) too Lets you search for something
  // like sound/sound.cache and get a list of every sound cache
  virtual void FindFileAbsoluteList(
      CUtlVector<CUtlString> & outAbsolutePathNames, const char *pWildCard,
  ...
  // Returns true on success ( based on current list of search paths, otherwise
  // false if
  ...
  // get progress on waiting for resources; progress is a float [0, 1], complete
  // is true on the waiting being done returns false if no progress is available
  // any calls after complete is true or on an invalid handle will return false,
  // 0.0f, true
  ...
  // Returns the file system statistics retreived by the implementation. Returns
  // NULL if not supported.
  virtual const FileSystemStatistics *GetFilesystemStatistics() = 0;

#if defined(_PS3)
  // EA cruft not used:   virtual Ps3FileType_t GetPs3FileType(const char* path)
  // = 0;
  virtual void LogFileAccess(const char *pFullFileName) = 0;

  // Prefetches a full file in the HDD cache.
  virtual bool PrefetchFile(const char *pFileName, int nPriority,
                            bool bPersist) = 0;
  // Prefetches a file portion in the HDD cache.
  virtual bool PrefetchFile(const char *pFileName, int nPriority, bool bPersist,
                            int64 nOffset, int64 nSize) = 0;
  // Flushes the HDD cache.
  virtual void FlushCache() = 0;
  // Suspends all prefetches (like when the game is doing a file intensive
  // operation not controlled by the HDD cache, like Bink movies).
  virtual void SuspendPrefetches(const char *pWhy) = 0;
  // Resumes prefetches. This function has to to be called as many time as
  // SuspendPrefetches() to effectively resumes prefetches.
  virtual void ResumePrefetches(const char *pWhy) = 0;

  // Gets called when we are starting / ending a save (it allows the file system
  // to reduce its HDD usage and use BluRay instead).
  virtual void OnSaveStateChanged(bool bSaving) = 0;

  // Returns the prefetching state. If true, everything has been prefetched on
  // the HDD.
  ...
  virtual FSAsyncStatus_t AsyncDirectoryScan(
      const char *pSearchSpec, bool recurseFolders, void *pContext,
      FSAsyncScanAddFunc_t pfnAdd, FSAsyncScanCompleteFunc_t pfnDone,
      FSAsyncControl_t *pControl = NULL) = 0;
  ...
  virtual bool FullPathToRelativePathEx(const char *pFullpath,
                                        const char *pPathId, char *pRelative,
                                        int maxlen) = 0;
  ...
  // This should be called ONCE at startup. Multiplayer games (gameinfo.txt does
  // not contain singleplayer_only) want to enable this so sv_pure works.
  ...
  //
  // Files listed in pWantCRCList will have CRCs calculated for them IF they
  // come off disk (and those CRCs will come out of GetUnverifiedCRCFiles).
  //
  // Files listed in pAllowFromDiskList will be allowed to load from disk. All
  // other files will be forced to come from Steam.
  //
  // The filesystem hangs onto the whitelists you pass in here, and it will
  // Release() them when it closes down or when you call this function again.
  //
  // NOTE: The whitelists you pass in here will be accessed from multiple
  // threads, so make sure the
  //       IsFileInList function is thread safe.
  //
  // If pFilesToReload is non-null, the filesystem will hand back a list of
  // files that should be reloaded because they are now "dirty". For example, if
  // you were on a non-pure server and you loaded a certain model, and then you
  // connected to a pure server that said that model had to come from Steam,
  // then pFilesToReload would specify that model and the engine should reload
  // it so it can come from Steam.
  //
  // Be sure to call Release() on pFilesToReload.
  virtual void RegisterFileWhitelist(IFileList * pWantCRCList,
  ...
  // should be marked as unverified because this server may have a different set
  // of files it wants to guarantee.
  ...
  virtual void CacheFileCRCs(const char *pPathname, ECacheCRCType eType,
                             IFileList *pFilter) = 0;
  virtual EFileCRCStatus CheckCachedFileCRC(
      const char *pPathID, const char *pRelativeFilename, CRC32_t *pCRC) = 0;

  // Fills in the list of files that have been loaded off disk and have not been
  // verified. Returns the number of files filled in (between 0 and nMaxFiles).
  //
  // This also removes any files it's returning from the unverified CRC list, so
  // they won't be returned from here again. The client sends batches of these
  // to the server to verify.
  virtual int GetUnverifiedCRCFiles(CUnverifiedCRCFile * pFiles,
                                    int nMaxFiles) = 0;
  ...
  virtual void AddVPKFile(char const *pszName,
                          SearchPathAdd_t addType = PATH_ADD_TO_TAIL) = 0;
  ...
  virtual bool GetStringFromKVPool(CRC32_t poolKey, unsigned int key,
                                   char *pOutBuff, int buflen) = 0;

  virtual bool DiscoverDLC(int iController) = 0;
  virtual int IsAnyDLCPresent(bool *pbDLCSearchPathMounted = NULL) = 0;
  virtual bool GetAnyDLCInfo(int iDLC, unsigned int *pLicenseMask,
                             wchar_t *pTitleBuff, int nOutTitleSize) = 0;
  virtual int IsAnyCorruptDLC() = 0;
  virtual bool GetAnyCorruptDLCInfo(int iCorruptDLC, wchar_t *pTitleBuff,
                                    int nOutTitleSize) = 0;
  virtual bool AddDLCSearchPaths() = 0;
  virtual bool IsSpecificDLCPresent(unsigned int nDLCPackage) = 0;

  // call this to look for CPU-hogs during loading processes. When you set this,
  // a breakpoint will be issued whenever the indicated # of seconds go by
  // without an i/o request.  Passing 0.0 will turn off the functionality.
...
#define SetLastProfileFileRead(s)          \
  V_strncpy(g_szXboxProfileLastFileOpened, \
