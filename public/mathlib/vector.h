...
#ifndef VPC_MATHLIB_VECTOR_H_
#define VPC_MATHLIB_VECTOR_H_
...
class Vector {
 public:
  ...
  // TODO (Ilya): Should there be an init that takes a single float for
  // consistency?
...
void VectorMA(const Vector& start, float scale, const Vector& direction,
              Vector& dest);
...
FORCEINLINE void ShortVector::Set(const short ix, const short iy,
                                  const short iz, const short iw) {
  x = ix;
  y = iy;
  z = iz;
  w = iw;
...
FORCEINLINE void ShortVectorMultiply(const ShortVector& src, float fl,
                                     ShortVector& res) {
  Assert(IsFinite(fl));
...
FORCEINLINE void IntVector4DMultiply(const IntVector4D& src, float fl,
                                     IntVector4D& res) {
  Assert(IsFinite(fl));
  res.x = (int)(src.x * fl);
  res.y = (int)(src.y * fl);
...
inline void VectorLerp(const Vector& src1, const Vector& src2, vec_t t,
                       Vector& dest) {
  CHECK_VALID(src1);
  CHECK_VALID(src2);
...
inline int Vector::LargestComponent() const {
  ...
  float flAbsz = fabs(z);
  if (flAbsx > flAbsy) {
    if (flAbsx > flAbsz) return X_INDEX;
    return Z_INDEX;
  }
  if (flAbsy > flAbsz) return Y_INDEX;
  return Z_INDEX;
}
...
// vecStart
//-----------------------------------------------------------------------------
inline void ComputeClosestPoint(const Vector& vecStart, float flMaxDist,
...
// return a vector perpendicular to another, with smooth variation. The
// difference between this and something like VectorVectors is that there are
// now discontinuities. _unlike_ VectorVectors, you won't get an "u
...
{                 //		which is a vec_t[4]
 public:
  ...
  inline Quaternion(vec_t ix, vec_t iy, vec_t iz, vec_t iw)
      : x(ix), y(iy), z(iz), w(iw) {}
...
inline void VectorMA(const QAngle& start, float scale, const QAngle& direction,
                     QAngle& dest) {
  CHECK_VALID(start);
...
// e.g. 	Vector vTest(7.98555446e-20,-6.85012984e-21,0); VectorNormalize(
// vTest ); so we clamp to epsilon instead of checking for zero
XMFINLINE XMVECTOR XMVector3Length_Fixed(FXMVECTOR V) {
  // Returns a QNaN on infinite vectors.
  static CONST XMVECTOR g_fl4SmallVectorEpsilon = {1e-24f, 1e-24f, 1e-24f,
                                                   1e-24f};
...
// FIXME: Change this back to a #define once we get rid of the vec_t version
FORCEINLINE float VectorNormalize(Vector& v) { return _VMX_VectorNormalize(v); }
// FIXME: Obsolete version of VectorNormalize, once we remove all the friggin
// float*s
...
FORCEINLINE void VectorNormalizeFast(Vector& vec) {
  float ool =
      FastRSqrt(FLT_EPSILON + vec.x * vec.x + vec.y * vec.y + vec.z * vec.z);
