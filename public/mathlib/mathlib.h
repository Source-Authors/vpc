...
#ifndef VPC_MATHLIB_MATH_LIB_H_
#define VPC_MATHLIB_MATH_LIB_H_
...
void GeneratePerspectiveFrustum(const Vector &origin, const Vector &forward,
                                ...
                                float flZNear, float flZFar, float flFovX,
                                float flFovY, VPlane *pPlanesOut);
// Cull the world-space bounding box to the specified frustum.
// bool R_CullBox( const Vector& mins, const Vector& maxs, const Frustum_t
// &frustum ); bool R_CullBoxSkipNear( const Vector& mins, const Vector& maxs,
// const Frustum_t &frustum );
void GenerateOrthoFrustum(const Vector &origin, const Vector &forward,
                          const Vector &right, const Vector &up, float flLeft,
                          float flRight, float flBottom, float flTop,
                          float flZNear, float flZFar, VPlane *pPlanesOut);
...
class ALIGN16 matrix3x4a_t : public matrix3x4_t {
 public:
  /*
  matrix3x4a_t() { if (((size_t)Base()) % 16 != 0) { Error( "matrix3x4a_t
  missaligned" ); } }
  */
...
// Use different side values (1, 2, 4) instead of (0, 1, 2) so we can '|' and
// '&' them, and quickly determine overall clipping without having to maintain
// counters and read / write memory.
enum Sides {
  OR_SIDE_FRONT = 1,
  OR_SIDE_BACK = 2,
  OR_SIDE_ON = 4,
...
void inline SinCos(float radians, float *RESTRICT sine,
                   float *RESTRICT cosine) {
#if defined(_X360)
...
void ConcatTransforms_Aligned(const matrix3x4a_t &m0, const matrix3x4a_t &m1,
                              matrix3x4a_t &out);
...
void QuaternionAverageExponential(Quaternion &q, int nCount,
                                  const Quaternion *pQuaternions,
                                  const float *pflWeights = NULL);
void QuaternionLookAt(const Vector &vecForward, const Vector &referenceUp,
                      Quaternion &q);
...
template <>
inline double clamp(double const &val, double const &minVal,
                    double const &maxVal) {
...
template <>
inline double clamp(double const &val, float const &minVal,
                    float const &maxVal) {
  // these typecasts are actually free since all FPU regs are 64 bit on PPC
  // anyway
...
template <>
inline float clamp(float const &val, double const &minVal,
                   double const &maxVal) {
  float diffmin = val - minVal;
  float diffmax = maxVal - val;
  float r;
...
template <>
inline float clamp(float const &val, float const &minVal,
                   double const &maxVal) {
...
//
// If you want to iterate something from 0 to n, you can use this to iterate
// non-sequentially, in such a way that you will start with widely separated
// values and then refine the gaps between them, as you would for progressive
// refinement. This works with non-power of two ranges.
...
inline bool MatrixIsIdentity(const matrix3x4_t &m) {
  return m.m_flMatVal[0][0] == 1.0f && m.m_flMatVal[0][1] == 0.0f &&
         m.m_flMatVal[0][2] == 0.0f && m.m_flMatVal[0][3] == 0.0f &&
         m.m_flMatVal[1][0] == 0.0f && m.m_flMatVal[1][1] == 1.0f &&
         m.m_flMatVal[1][2] == 0.0f && m.m_flMatVal[1][3] == 0.0f &&
         m.m_flMatVal[2][0] == 0.0f && m.m_flMatVal[2][1] == 0.0f &&
         m.m_flMatVal[2][2] == 1.0f && m.m_flMatVal[2][3] == 0.0f;
}
...
bool IsPointInBounds(const Vector &vPoint, const Vector &vMin,
                     const Vector &vMax);
...
#endif
  return static_cast<unsigned char>(clamp(nResult, 0, 255));
#endif
}
...
void ClipLineSegmentToPlane(const Vector &vNormal, const Vector &vPlanePoint,
                            Vector *p1, Vector *p2, float flBias = 0.0f);

void ComputeTrianglePlane(const Vector &v1, const Vector &v2, const Vector &v3,
                          Vector &normal, float &intercept);
int PolyFromPlane(Vector *pOutVerts, const Vector &normal, float dist,
                  float fHalfScale = 9000.0f);
void PolyFromPlane_SIMD(fltx4 *pOutVerts, const fltx4 &plane,
                        float fHalfScale = 9000.0f);
int ClipPolyToPlane(Vector *inVerts, int vertCount, Vector *outVerts,
                    const Vector &normal, float dist,
                    float fOnPlaneEpsilon = 0.1f);
int ClipPolyToPlane_SIMD(fltx4 *pInVerts, int vertCount, fltx4 *pOutVerts,
                         const fltx4 &plane, float fOnPlaneEpsilon = 0.1f);
...
float TetrahedronVolume(const Vector &p0, const Vector &p1, const Vector &p2,
                        const Vector &p3);
...
inline void MatrixTransformPlane(const matrix3x4_t &src,
  ...
  // component of transform 3) Compute the new plane dist using the dot product
  // of the normal result of #2

  // For a correct result, this should be an inverse-transpose matrix
  // but that only matters if there are nonuniform scale or skew factors in this
  // matrix.
...
FORCEINLINE unsigned int *PackNormal_SHORT2(float nx, float ny, float nz,
  ...
  // format. This simply reconstructs Z from X & Y. It uses the sign bits of the
  // X & Y coords to reconstruct the sign of Z and, if this is a tangent vector,
  // the sign of the binormal (this is needed because tangent/binormal vectors
  // are supposed to follow UV gradients, but shaders reconstruct the binormal
  // from the tangent and normal assuming that they form a right-handed basis).
  ...
  // NOTE: The mask is necessary (if sX is negative and cast to an int...)
...
FORCEINLINE void RGB2YUV(int &nR, int &nG, int &nB, float &fY, float &fU,
                         float &fV, bool bApplySaturationCurve) {
  // YUV conversion:
  //  |Y|   |  0.299f     0.587f     0.114f   |   |R|
  //  |U| = | -0.14713f  -0.28886f   0.436f   | x |G|
  //  |V|   |  0.615f    -0.51499f  -0.10001f |   |B|
  //
  // The coefficients in the first row sum to one, whereas the 2nd and 3rd rows
  // each sum to zero (UV (0,0) means greyscale). Ranges are Y [0,1], U
  // [-0.436,+0.436] and V [-0.615,+0.615]. We scale and offset to [0,1] and
  // allow the caller to round as they please.

  fY = (0.29900f * nR + 0.58700f * nG + 0.11400f * nB) / 255;
  fU = (-0.14713f * nR + -0.28886f * nG + 0.43600f * nB) * (0.5f / 0.436f) /
           255 +
       0.5f;
  fV = (0.61500f * nR + -0.51499f * nG + -0.10001f * nB) * (0.5f / 0.615f) /
           255 +
       0.5f;

  if (bApplySaturationCurve) {
    // Apply a curve to saturation, and snap-to-grey for low saturations
    const float SNAP_TO_GREY =
        0;  // 0.0125f; Disabled, saturation curve seems sufficient
    float dX, dY, sat, scale;
    dX = 2 * (fU - 0.5f);
    dY = 2 * (fV - 0.5f);
    sat = sqrtf(dX * dX + dY * dY);
    sat = clamp((sat * (1 + SNAP_TO_GREY) - SNAP_TO_GREY), 0.0F, 1.0F);
...
#ifdef _X360
// Used for direct CPU access to VB data on 360 (used by shaderapi, studiorender
// and engine)
struct VBCPU_AccessInfo_t {
  // Points to the GPU data pointer in the CVertexBuffer struct (VB data can be
  // relocated during level transitions)
...
inline float FastPow10(float i) {
  return exp2f(i * LOGBASE2OF10);
}  // 10^i, transform to base two, so log2(10^y) = y log2(10) . log2(10)
   // = 3.3219280948873623478703194294893901758648313930
#endif
...
inline float Approach(float target, float value, float speed) {
  ...
  return fsel(delta - speed,  // delta >= speed ?
              value + speed,  // if delta == speed, then value + speed == value
                              // + delta == target
              fsel((-speed) - delta,  // delta <= -speed
                   value - speed,
                   target));  // delta < speed && delta > -speed
