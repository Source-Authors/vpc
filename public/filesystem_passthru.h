...
#ifndef VPC_FILESYSTEM_PASSTHRU_H_
#define VPC_FILESYSTEM_PASSTHRU_H_
...
class CInternalFileSystemPassThru : public Base {
 public:
  ...
  virtual bool ReadFile(const char *pFileName, const char *pPath,
                        ...
                        int nStartingByte = 0, FSAllocFunc_t pfnAlloc = NULL) {
    return m_pBaseFileSystemPassThru->ReadFile(pFileName, pPath, buf, nMaxBytes,
...
class CFileSystemPassThru : public CInternalFileSystemPassThru<IFileSystem> {
 public:
  ...
  virtual const AppSystemInfo_t *GetDependencies() {
    return m_pFileSystemPassThru->GetDependencies();
  }
  ...
  virtual void Reconnect(CreateInterfaceFn factory,
    ...
    m_pFileSystemPassThru->Reconnect(factory, pInterfaceName);
  }
  ...
  virtual void FindFileAbsoluteList(
      ...
      const char *pPathID) {
    m_pFileSystemPassThru->FindFileAbsoluteList(outAbsolutePathNames, pWildCard,
                                                pPathID);
  }
  virtual const char *GetLocalPath(const char *pFileName, char *pLocalPath,
                                   int localPathBufferSize) {
    return m_pFileSystemPassThru->GetLocalPath(pFileName, pLocalPath,
                                               localPathBufferSize);
  }
  virtual bool FullPathToRelativePath(const char *pFullpath, char *pRelative,
                                      int maxlen) {
    return m_pFileSystemPassThru->FullPathToRelativePath(pFullpath, pRelative,
                                                         maxlen);
  }
  ...
  virtual FSAsyncStatus_t AsyncReadMultiple(const FileAsyncRequest_t *pRequests,
                                            int nRequests,
  ...
  virtual FSAsyncStatus_t AsyncDirectoryScan(
    ...
    return m_pFileSystemPassThru->AsyncDirectoryScan(
        pSearchSpec, recurseFolders, pContext, pfnAdd, pfnDone, pControl);
  }
  ...
  virtual bool GetWaitForResourcesProgress(WaitForResourcesHandle_t handle,
    ...
    return m_pFileSystemPassThru->GetWaitForResourcesProgress(handle, progress,
                                                              complete);
  }
  ...
  virtual int HintResourceNeed(const char *hintlist, int forgetEverything) {
    return m_pFileSystemPassThru->HintResourceNeed(hintlist, forgetEverything);
  }
  ...
  virtual FSAsyncStatus_t AsyncWrite(const char *pFileName, const void *pSrc,
    ...
    return m_pFileSystemPassThru->AsyncWrite(pFileName, pSrc, nSrcBytes,
                                             bFreeMemory, bAppend, pControl);
  }
  ...
  virtual FSAsyncStatus_t AsyncAppendFile(const char *pDestFileName,
                                          ...
                                          FSAsyncControl_t *pControl) {
    return m_pFileSystemPassThru->AsyncAppendFile(pDestFileName, pSrcFileName,
                                                  pControl);
  }
  ...
  virtual const char *RelativePathToFullPath(
    ...
    return m_pFileSystemPassThru->RelativePathToFullPath(
        pFileName, pPathID, pLocalPath, localPathBufferSize, pathFilter,
        pPathType);
  }
#if IsGameConsole()
  ...
  virtual bool GetPackFileInfoFromRelativePath(
      ...
      int nPackPathBufferSize, int64 &nPosition, int64 &nLength) {
    return m_pFileSystemPassThru->GetPackFileInfoFromRelativePath(
        pFileName, pPathID, pPackPath, nPackPathBufferSize, nPosition, nLength);
  }
#endif

  virtual int GetSearchPath(const char *pathID, bool bGetPackFiles, char *pPath,
                            int nMaxLen) {
    return m_pFileSystemPassThru->GetSearchPath(pathID, bGetPackFiles, pPath,
                                                nMaxLen);
  }
  ...
  virtual int ReadEx(void *pOutput, int destSize, int size, FileHandle_t file) {
  ...
  virtual bool LoadKeyValues(KeyValues &head, KeyValuesPreloadType_t type,
    ...
    return m_pFileSystemPassThru->LoadKeyValues(head, type, filename, pPathID);
  }
  ...
  virtual bool ReadToBuffer(FileHandle_t hFile, CUtlBuffer &buf,
    ...
    return m_pFileSystemPassThru->ReadToBuffer(hFile, buf, nMaxBytes, pfnAlloc);
  }
  virtual bool FullPathToRelativePathEx(const char *pFullPath,
                                        const char *pPathId, char *pRelative,
                                        int nMaxLen) {
    return m_pFileSystemPassThru->FullPathToRelativePathEx(pFullPath, pPathId,
                                                           pRelative, nMaxLen);
  }
  ...
  virtual void RegisterFileWhitelist(IFileList *pForceMatchList,
                                     IFileList *pAllowFromDiskList,
                                     IFileList **pFilesToReload) {
    m_pFileSystemPassThru->RegisterFileWhitelist(
        pForceMatchList, pAllowFromDiskList, pFilesToReload);
  }
  ...
  virtual EFileCRCStatus CheckCachedFileCRC(const char *pPathID,
                                            const char *pRelativeFilename,
                                            CRC32_t *pCRC) {
    return m_pFileSystemPassThru->CheckCachedFileCRC(pPathID, pRelativeFilename,
                                                     pCRC);
  }
  ...
  virtual bool IsInstalledToXboxHDDCache() {
    return m_pFileSystemPassThru->IsInstalledToXboxHDDCache();
  }
  virtual bool IsDVDHosted() { return m_pFileSystemPassThru->IsDVDHosted(); }
  virtual bool IsInstallAllowed() {
    return m_pFileSystemPassThru->IsInstallAllowed();
  }
  virtual int GetSearchPathID(char *pPath, int nMaxLen) {
    return m_pFileSystemPassThru->GetSearchPathID(pPath, nMaxLen);
  }
  virtual bool FixupSearchPathsAfterInstall() {
    return m_pFileSystemPassThru->FixupSearchPathsAfterInstall();
  }

  virtual FSDirtyDiskReportFunc_t GetDirtyDiskReportFunc() {
    return m_pFileSystemPassThru->GetDirtyDiskReportFunc();
  }

  virtual void AddVPKFile(char const *pPkName,
                          SearchPathAdd_t addType = PATH_ADD_TO_TAIL) {
    m_pFileSystemPassThru->AddVPKFile(pPkName, addType);
  }
  virtual void RemoveVPKFile(char const *pPkName) {
    m_pFileSystemPassThru->RemoveVPKFile(pPkName);
  }
  ...
  virtual void RemoveAllMapSearchPaths(void) {
    m_pFileSystemPassThru->RemoveAllMapSearchPaths();
  }

  virtual void SyncDvdDevCache(void) {
    m_pFileSystemPassThru->SyncDvdDevCache();
  }

  virtual bool GetStringFromKVPool(CRC32_t poolKey, unsigned int key,
                                   char *pOutBuff, int buflen) {
    return m_pFileSystemPassThru->GetStringFromKVPool(poolKey, key, pOutBuff,
                                                      buflen);
  }

  virtual bool DiscoverDLC(int iController) {
    return m_pFileSystemPassThru->DiscoverDLC(iController);
  }
  virtual int IsAnyDLCPresent(bool *pbDLCSearchPathMounted = NULL) {
    return m_pFileSystemPassThru->IsAnyDLCPresent(pbDLCSearchPathMounted);
  }
  virtual bool GetAnyDLCInfo(int iDLC, unsigned int *pLicenseMask,
                             wchar_t *pTitleBuff, int nOutTitleSize) {
    return m_pFileSystemPassThru->GetAnyDLCInfo(iDLC, pLicenseMask, pTitleBuff,
                                                nOutTitleSize);
  }
  virtual int IsAnyCorruptDLC() {
    return m_pFileSystemPassThru->IsAnyCorruptDLC();
  }
  virtual bool GetAnyCorruptDLCInfo(int iCorruptDLC, wchar_t *pTitleBuff,
                                    int nOutTitleSize) {
    return m_pFileSystemPassThru->GetAnyCorruptDLCInfo(iCorruptDLC, pTitleBuff,
                                                       nOutTitleSize);
  }
  virtual bool AddDLCSearchPaths() {
    return m_pFileSystemPassThru->AddDLCSearchPaths();
  }
  virtual bool IsSpecificDLCPresent(unsigned int nDLCPackage) {
    return m_pFileSystemPassThru->IsSpecificDLCPresent(nDLCPackage);
  }
  virtual void SetIODelayAlarm(float flThreshhold) {
    m_pFileSystemPassThru->SetIODelayAlarm(flThreshhold);
  }
...
#endif  // VPC_FILESYSTEM_PASSTHRU_H_
