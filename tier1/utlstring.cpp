...
void CUtlBinaryBlock::SetLength(intp nLength) {
  ...
  if (m_Memory.NumAllocated() > m_nActualLength) {
    memset(((char *)m_Memory.Base()) + m_nActualLength, 0xEB,
           m_Memory.NumAllocated() - m_nActualLength);
  }
#endif
}
...
void CUtlBinaryBlock::Set(const void *pValue, intp nLen) {
  ...
  if (m_nActualLength) {
    if (((const char *)m_Memory.Base()) >= ((const char *)pValue) + nLen ||
        ((const char *)m_Memory.Base()) + m_nActualLength <=
            ((const char *)pValue)) {
...
CUtlString::operator const char *() const { return Get(); }
...
CUtlString CUtlString::Slice(intp nStart, intp nEnd) const {
  ...
  if (nStart >= nEnd) return CUtlString("");
