...
void ConVar_Register(int nCVarFlag, IConCommandBaseAccessor *pAccessor) {
  if (!g_pCVar || s_bRegistered) return;
...
//			flags -
//-----------------------------------------------------------------------------
void ConCommandBase::Create(const char *pName, const char *pHelpString /*= 0*/,
                            int flags /*= 0*/) {
...
bool CCommand::Tokenize(const char *pCommand, characterset_t *pBreakSet) {
  ...
  while (bufParse.IsValid() && (m_nArgc < COMMAND_MAX_ARGC)) {
    ...
    if (m_nArgc >= COMMAND_MAX_ARGC) {
      Warning(
          "CCommand::Tokenize: Encountered command which overflows the "
          "argument buffer.. Clamped!\n");
    }
...
const char *CCommand::FindArg(const char *pName) const {
  ...
  for (int i = 1; i < nArgC; i++) {
    if (!V_stricmp(Arg(i), pName)) return (i + 1) < nArgC ? Arg(i + 1) : "";
  }
  return 0;
}
...
ConCommand::ConCommand(const char *pName, FnCommandCallbackV1_t callback,
...
ConCommand::ConCommand(
    const char *pName, ICommandCallback *pCallback,
    const char *pHelpString /*= 0*/, int flags /*= 0*/,
...
int ConCommand::AutoCompleteSuggest(const char *partial,
  ...
  if (m_bUsingCommandCallbackInterface) {
    ...
    return m_pCommandCompletionCallback->CommandCompletionCallback(partial,
                                                                   commands);
  }
...
int ConVar::GetSplitScreenPlayerSlot(void) const {
  // Default implementation (certain FCVAR_USERINFO derive a new type of convar
  // and set this)
  return 0;
}
...
bool ConVar::InternalSetColorFromString(const char *value) {
  ...
  int nRGBA[4];
  int nParamsRead = sscanf(value, "%i %i %i %i", &(nRGBA[0]), &(nRGBA[1]),
                           &(nRGBA[2]), &(nRGBA[3]));
  ...
  if (nParamsRead >= 3) {
    ...
    if (nRGBA[0] >= 0 && nRGBA[0] <= 255 && nRGBA[1] >= 0 && nRGBA[1] <= 255 &&
        nRGBA[2] >= 0 && nRGBA[2] <= 255 && nRGBA[3] >= 0 && nRGBA[3] <= 255) {
...
void ConVar::InternalSetIntValue(int nValue) {
  if (nValue == m_Value.m_nValue) return;
...
void ConVar::Create(const char *pName, const char *pDefaultValue,
  ...
  // FIXMEL4DTOMAINMERGE: will need to assess if this hides too many convars for
  // TF and other projects in main
  if (!(flags &
        (FCVAR_CHEAT | FCVAR_ARCHIVE | FCVAR_RELEASE | FCVAR_USERINFO))) {
