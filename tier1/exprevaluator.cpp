...
// Syntax is $<name>. $0 evaluates to false. $<number> evaluates to true, e.g: (
// $1 || ( $FOO || $WHATEVER ) && !$BAR )
...
//-----------------------------------------------------------------------------
//	Sets mCurToken to the next token in the input string. Skips all
// whitespace.
//-----------------------------------------------------------------------------
char CExpressionEvaluator::GetNextToken(void) {
  // while whitespace, Increment CurrentPosition
  while (m_pExpression[m_CurPosition] == ' ') ++m_CurPosition;
...
//	Utility funcs
//-----------------------------------------------------------------------------
void CExpressionEvaluator::FreeNode(ExprNode *pNode) { delete pNode; }

ExprNode *CExpressionEvaluator::AllocateNode(void) { return new ExprNode; }
...
bool CExpressionEvaluator::IsConditional(bool &bConditional, const char token) {
  ...
  if (token == OR_OP || token == AND_OP) {
    ...
    if ((token & nextchar) == token) {
      ...
      m_pSyntaxErrorProc(
          CFmtStr("Bad expression operator: '%c%c', expected C style operator",
                  token, nextchar));
      return false;
    }
  } else {
...
bool CExpressionEvaluator::IsIdentifierOrConstant(const char token) {
  ...
  if (token == '$') {
    ...
    while ((isalnum(m_pExpression[m_CurPosition]) ||
            m_pExpression[m_CurPosition] == '_') &&
    ...
    if (isdigit(token)) {
      int i = 0;
      m_Identifier[i++] = token;
      while (isdigit(m_pExpression[m_CurPosition]) &&
             (i < MAX_IDENTIFIER_LEN)) {
        m_Identifier[i] = m_pExpression[m_CurPosition];
        ++m_CurPosition;
        ++i;
...
bool CExpressionEvaluator::MakeExprNode(ExprTree &tree, char token, Kind kind,
                                        ExprTree left, ExprTree right) {
  tree = AllocateNode();
  tree->left = left;
  tree->right = right;
  tree->kind = kind;
...
bool CExpressionEvaluator::MakeFactor(ExprTree &tree) {
  if (m_CurToken == '(') {
    ...
    // Make a literal node, set Tree to point to it, set left/right children to
    // NULL.
...
bool CExpressionEvaluator::MakeTerm(ExprTree &tree) {
  ...
  while (IsNotOp(m_CurToken)) {
    // Make an operator node, setting left child to Tree and right to NULL.
    // (Tree points to new node)
...
bool CExpressionEvaluator::MakeExpression(ExprTree &tree) {
  ...
  while (1) {
    ...
    // Make a conditional node, setting left child to Tree and right to NULL.
    // (Tree points to new node)
...
// Result is undefined.
//-----------------------------------------------------------------------------
bool CExpressionEvaluator::Evaluate(bool &bResult, const char *pInfixExpression,
                                    GetSymbolProc_t pGetSymbolProc,
                                    SyntaxErrorProc_t pSyntaxErrorProc) {
  if (!pInfixExpression) {
    return false;
  }
  ...
  if (pInfixExpression[0] == '[') {
    ...
    if (len + 1 > static_cast<intp>(V_ARRAYSIZE(szCleanToken))) {
      return false;
    }

    // SECURIY: Because this starts one character late, it picks up the null
    // termination from pInfixExpression.
  ...
  m_pGetSymbolProc =
      pGetSymbolProc ? pGetSymbolProc : DefaultConditionalSymbolProc;
  m_pSyntaxErrorProc =
      pSyntaxErrorProc ? pSyntaxErrorProc : DefaultConditionalErrorProc;
  m_ExprTree = 0;
  m_CurPosition = 0;
  m_CurToken = 0;
