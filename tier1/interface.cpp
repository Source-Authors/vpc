...
// ------------------------------------------------------------------------------------
// // CreateInterface. This is the primary exported function by a dll,
// referenced by name via dynamic binding that exposes an opqaue function
// pointer to the interface.
//
// We have the Internal variant so Sys_GetFactoryThis() returns the correct
// internal symbol under GCC/Linux/Mac as CreateInterface is DLL_EXPORT so its
// global so the loaders on those OS's pick exactly 1 of the CreateInterface
// symbols to be the one that is process wide and all Sys_GetFactoryThis() calls
// find that one, which doesn't work. Using the internal walkthrough here makes
// sure Sys_GetFactoryThis() has the dll specific symbol and GetProcAddress()
// returns the module specific function for CreateInterface again getting the
// dll specific symbol we need.
// ------------------------------------------------------------------------------------
// //
...
struct ThreadedLoadLibaryContext_t {
  ...
  HMODULE m_hLibrary;
  DWORD m_nError;
  ThreadedLoadLibaryContext_t()
      : m_pLibraryName(NULL), m_hLibrary(0), m_nError(0) {}
};
...
static HMODULE Sys_LoadLibraryGuts(const char *pLibraryName) {
  ...
  PS3_LoadAppSystemInterface_Parameters_t *pPRX =
      new PS3_LoadAppSystemInterface_Parameters_t;
...
#if 0  // you can enable this block to help track down why a module isn't
       // loading:
		else
		{
#ifdef _WINDOWS
...
static HMODULE Sys_LoadLibrary(const char *pLibraryName) {
  // load a library. If a library suffix is set, look for the library first with
  // that name
...
CSysModule *Sys_LoadModule(const char *pModuleName) {
  ...
  if (IsPS3()) {
    // PS3's load module *must* be fed extensions. If the extension is missing,
    // add it.
  ...
  if (!V_IsAbsolutePath(pModuleName)) {
    ...
    if (g_pPS3PathInfo->IsPatched()) {
      V_snprintf(szAbsoluteModuleName, sizeof(szAbsoluteModuleName),
                 "%s/bin/%s", g_pPS3PathInfo->GamePatchBasePath(), pModuleName);
...
// So you can see what the error is in the debugger...
  ...
  if (!IsGameConsole() && Sys_GetProcAddress(hDLL, "BuiltDebug")) {
    ...
    if (!s_bRunningWithDebugModules) {
      ...
      (void)CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0,
                              1024, chMemoryName);
...
CreateInterfaceFn Sys_GetFactory(CSysModule *pModule) {
...
#elif defined(_PS3)
  return reinterpret_cast<CreateInterfaceFn>(
      Sys_GetProcAddress(hDLL, CREATEINTERFACE_PROCNAME));
#elif defined(POSIX)
  ...
  return (CreateInterfaceFn)(GetProcAddress((void *)hDLL,
...
// Output :
//-----------------------------------------------------------------------------
bool Sys_LoadInterface(const char *pModuleName,
                       const char *pInterfaceVersionName,
                       CSysModule **pOutModule, void **pOutInterface) {
...
//  then it'll call Sys_UnloadModule on the module so that the refcount is
//  decremented and the .dll actually can unload from memory.
//-----------------------------------------------------------------------------
CDllDemandLoader::CDllDemandLoader(char const *pchModuleName)
    : m_pchModuleName(pchModuleName), m_hModule(0), m_bLoadAttempted(false) {}
