...
void CUtlMemoryPool::AddNewBlob() {
  ...
  CBlob *pBlob =
      (CBlob *)malloc(sizeof(CBlob) - 1 + blobSize + (m_nAlignment - 1));
  Assert(pBlob);
...
void *CUtlMemoryPool::Alloc() { return Alloc(m_BlockSize); }

void *CUtlMemoryPool::AllocZero() { return AllocZero(m_BlockSize); }
...
void *CUtlMemoryPool::Alloc(size_t amount) {
  ...
  if (amount > (size_t)m_BlockSize) return NULL;
...
void CUtlMemoryPool::Free(void *memBlock) {
  ...
  for (CBlob *pCur = m_BlobHead.m_pNext; pCur != &m_BlobHead;
       pCur = pCur->m_pNext) {
    if (memBlock >= pCur->m_Data &&
        (char *)memBlock < (pCur->m_Data + pCur->m_NumBytes)) {
      bOK = true;
    }
  }
  Assert(bOK);
