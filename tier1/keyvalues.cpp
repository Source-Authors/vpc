...
intp (*KeyValues::s_pfGetSymbolForString)(const char *name, bool bCreate) =
    &KeyValues::GetSymbolForStringClassic;
const char *(*KeyValues::s_pfGetStringForSymbol)(intp symbol) =
    &KeyValues::GetStringForSymbolClassic;
  ...
  void ReportError(const char *pError) {
    ...
    for (int i = 0; i < m_maxErrorIndex; i++) {
      if (m_errorStack[i] != INVALID_KEY_SYMBOL) {
        if (i < m_errorIndex) {
          Warning("%s, ",
                  KeyValuesSystem()->GetStringForSymbol(m_errorStack[i]));
        } else {
          Warning("(*%s*), ",
                  KeyValuesSystem()->GetStringForSymbol(m_errorStack[i]));
        }
...
class CKeyErrorContext {
 public:
  ~CKeyErrorContext() { g_KeyValuesErrorStack.Pop(); }
  explicit CKeyErrorContext(int symName) { Init(symName); }
  void Reset(int symName) {
    g_KeyValuesErrorStack.Reset(m_stackLevel, symName);
  }

 private:
  void Init(int symName) { m_stackLevel = g_KeyValuesErrorStack.Push(symName); }
...
class CKeyValuesGrowableStringTable {
 public:
  ...
  intp GetSymbolForString(const char *name, bool bCreate = true) {
    ...
    if (bCreate) {
      ...
      if (bInserted) {
        intp iIndex = m_vecStrings.AddMultipleToTail(V_strlen(name) + 1, name);
 ...
 private:
  // A class plugged into CUtlHash that allows us to change the behavior of the
  // table and store only the index in the table.
...
void KeyValues::Init() {
  ...
  m_pExpressionGetSymbolProc = nullptr;
}
...
const char *KeyValues::GetName(void) const {
  return KeyValuesSystem()->GetStringForSymbol(MAKE_3_BYTES_FROM_1_AND_2(
      m_iKeyNameCaseSensitive1, m_iKeyNameCaseSensitive2));
}
...
int KeyValues::GetNameSymbolCaseSensitive() const {
  return MAKE_3_BYTES_FROM_1_AND_2(m_iKeyNameCaseSensitive1,
                                   m_iKeyNameCaseSensitive2);
}
...
const char *KeyValues::ReadToken(CUtlBuffer &buf, bool &wasQuoted,
  ...
  while (true) {
    buf.EatWhiteSpace();
    if (!buf.IsValid()) return NULL;  // file ends after reading whitespaces

    // stop if it's not a comment; a new token starts here
    if (!buf.EatCPPComment()) break;
  }
  ...
  if (*c == '\"') {
    wasQuoted = true;
    buf.GetDelimitedString(m_bHasEscapeSequences ? GetCStringCharConversion()
  ...
  while (const char *v = (const char *)buf.PeekGet(sizeof(char), 0)) {
    ...
    if (V_isspace(*v) && !bConditionalStart) break;
...
bool KeyValues::LoadFromFile(IBaseFileSystem *filesystem,
  ...
  unsigned bufSize =
      ((IFileSystem *)filesystem)->GetOptimalReadSize(f, fileSize + 2);
  ...
  bool bRetOK =
      (((IFileSystem *)filesystem)->ReadEx(buffer, bufSize, fileSize, f) != 0);
  ...
  if (bRetOK) {
    ...
    buffer[fileSize + 1] =
        0;  // double NULL terminating in case this is a unicode file

    CUtlBuffer utlBuffer;
    if (IsGameConsole() &&
        (unsigned int)((unsigned char *)buffer)[0] == KV_BINARY_POOLED_FORMAT) {
      // kv contents are compiled binary
      utlBuffer.SetExternalBuffer(buffer, bufSize, fileSize,
                                  CUtlBuffer::READ_ONLY);
    } else {
      // kv contents are text
      intp nLen = V_strlen(buffer);
      utlBuffer.SetExternalBuffer(
          buffer, bufSize, nLen,
          CUtlBuffer::READ_ONLY | CUtlBuffer::TEXT_BUFFER);
    }

    bRetOK = LoadFromBuffer(resourceName, utlBuffer, filesystem, pathID,
                            pfnEvaluateSymbolProc);
  }
      ...
      switch (dat->m_iDataType) {
        ...
        case TYPE_WSTRING: {
          if (dat->m_wsValue) {
            ...
            int result =
                V_UnicodeToUTF8(dat->m_wsValue, buf, KEYVALUES_TOKEN_SIZE);
            if (result) {
              WriteIndents(filesystem, f, pBuf, indentLevel + 1);
              INTERNALWRITE("\"", 1);
        ...
        case TYPE_COLOR:
          DevMsg(
              "KeyValues::RecursiveSaveToFile: TODO, missing code for "
              "TYPE_COLOR.\n");
          break;
...
KeyValues *KeyValues::FindKey(int keySymbol) const {
  for (KeyValues *dat = m_pSub; dat != NULL; dat = dat->m_pPeer) {
    if (dat->m_iKeyName == (uint32)keySymbol) return dat;
  }
...
KeyValues *KeyValues::FindKey(const char *keyName, bool bCreate) {
  ...
  // because if the key is found, then it will be found by case-insensitive
  // lookup if the key is not found and needs to be created we will pass the
  // actual searchStr and have the new KeyValues constructor get/create the
  // case-sensitive symbol
  HKeySymbol iSearchStr =
      KeyValuesSystem()->GetSymbolForString(searchStr, bCreate);
  ...
  for (dat = m_pSub; dat != NULL; dat = dat->m_pPeer) {
    lastItem = dat;  // record the last item looked at (for if we need to append
...
KeyValues *KeyValues::CreateKey(const char *keyName) {
  ...
  dat->UsesEscapeSequences(m_bHasEscapeSequences !=
                           0);  // use same format as parent does
...
void KeyValues::InsertSubKey(int nIndex, KeyValues *pSubKey) {
  ...
  if (nIndex == 0) {
    ...
    int nCurrentIndex = 0;
    for (KeyValues *pIter = GetFirstSubKey(); pIter != NULL;
...
void KeyValues::SwapSubKey(KeyValues *pExistingSubkey, KeyValues *pNewSubKey) {
  ...
  if (m_pSub == pExistingSubkey) {
    ...
    // Existing sub key should always be found, otherwise it's a bug in the
    // calling code.
...
KeyValues *KeyValues::GetFirstTrueSubKey() {
  ...
  while (pRet && pRet->m_iDataType != TYPE_NONE) pRet = pRet->m_pPeer;
...
const wchar_t *KeyValues::GetWString(const char *keyName,
  ...
  if (dat) {
    ...
    switch (dat->m_iDataType) {
      ...
      case TYPE_PTR:
        ...
        break;
      case TYPE_UINT64: {
        swprintf(wbuf, V_ARRAYSIZE(wbuf), L"%llu",
                 *((uint64 *)(dat->m_sValue)));
        SetWString(keyName, wbuf);
      } break;
...
void KeyValues::SetName(const char *setName) {
  HKeySymbol hCaseSensitiveKeyName = INVALID_KEY_SYMBOL,
             hCaseInsensitiveKeyName = INVALID_KEY_SYMBOL;
  hCaseSensitiveKeyName = KeyValuesSystem()->GetSymbolForStringCaseSensitive(
      hCaseInsensitiveKeyName, setName);

  m_iKeyName = hCaseInsensitiveKeyName;
  SPLIT_3_BYTES_INTO_1_AND_2(m_iKeyNameCaseSensitive1, m_iKeyNameCaseSensitive2,
...
void KeyValues::RecursiveCopyKeyValues(KeyValues &src) {
  ...
  if (!src.m_pSub) {
    ...
    switch (src.m_iDataType) {
      ...
      case TYPE_STRING:
        if (src.m_sValue) {
          intp len = V_strlen(src.m_sValue) + 1;
...
bool KeyValues::IsEmpty(const char *keyName) {
  ...
  if (dat->m_iDataType == TYPE_NONE && dat->m_pSub == NULL) return true;
...
void KeyValues::ParseIncludedKeys(char const *resourceName,
                                  const char *filetoinclude,
  ...
  if (newKV->LoadFromFile(pFileSystem, fullpath, pPathID,
    ...
    DevMsg(
        "KeyValues::ParseIncludedKeys: Couldn't load included keyvalue file "
        "%s\n",
        fullpath);
    newKV->deleteThis();
  }
...
void KeyValues::RecursiveMergeKeyValues(KeyValues *baseKV) {
  ...
  for (KeyValues *baseChild = baseKV->m_pSub; baseChild != NULL;
       baseChild = baseChild->m_pPeer) {
    // for each child in base, see if we have a matching kv

    bool bFoundMatch = false;

    // If we have a child by the same name, merge those keys
    for (KeyValues *newChild = m_pSub; newChild != NULL;
         newChild = newChild->m_pPeer) {
...
//-----------------------------------------------------------------------------
// Returns whether a keyvalues conditional expression string evaluates to true
// or false
//-----------------------------------------------------------------------------
bool KeyValues::EvaluateConditional(const char *pExpressionString,
                                    GetSymbolProc_t pfnEvaluateSymbolProc) {
  // evaluate the infix expression, calling the symbol proc to resolve each
  // symbol's value
  bool bResult = false;
  bool bValid = g_ExpressionEvaluator.Evaluate(bResult, pExpressionString,
                                               pfnEvaluateSymbolProc);
  if (!bValid) {
...
// prevent two threads from entering this at the same time and trying to share
// the global error reporting and parse buffers
...
bool KeyValues::LoadFromBuffer(char const *resourceName, CUtlBuffer &buf,
  ...
  if (IsGameConsole()) {
    ...
    unsigned char *pData =
        buf.Size() > 0 ? (unsigned char *)buf.PeekGet() : NULL;
    if (pData && (unsigned int)pData[0] == KV_BINARY_POOLED_FORMAT) {
      ...
      return ReadAsBinaryPooledFormat(buf, pFileSystem, poolKey,
                                      pfnEvaluateSymbolProc);
    }
  }
...
bool KeyValues::LoadFromBuffer(char const *resourceName, const char *pBuffer,
  ...
  if (IsGameConsole() &&
      (unsigned int)((unsigned char *)pBuffer)[0] == KV_BINARY_POOLED_FORMAT) {
    ...
    // need to have caller supply buffer length (strlen not valid), this
    // interface change was never plumbed
  ...
  if (nLen > 2 && (uint8)pBuffer[0] == 0xFF && (uint8)pBuffer[1] == 0xFE) {
    ...
    buf.AssumeMemory(pUTF8Buf, nUTF8Len, nUTF8Len,
                     CUtlBuffer::READ_ONLY | CUtlBuffer::TEXT_BUFFER);
  }
  ...
  while (1) {
    ...
    if (*value == '{' && !wasQuoted) {
      ...
      if (*value == 0) {
        ...
        // an 18-byte value prefixed with "0x" (followed by 16 hex digits) is an
        // int64 value
        int64 retVal = 0;
...
bool KeyValues::ReadAsBinary(CUtlBuffer &buffer) {
  ...
  // loop through all our peers
  while (true) {
    if (type == TYPE_NUMTYPES) break;  // no more peers
...
//-----------------------------------------------------------------------------
bool KeyValues::ReadAsBinaryPooledFormat(
    CUtlBuffer &buffer, IBaseFileSystem *pFileSystem, unsigned int poolKey,
    GetSymbolProc_t pfnEvaluateSymbolProc) {
  // xbox only support
  if (!IsGameConsole()) {
    Assert(0);
  ...
  while (true) {
    ...
    if (!((IFileSystem *)pFileSystem)
             ->GetStringFromKVPool(poolKey, stringKey, token, sizeof(token)))
      return false;
    dat->SetName(token);
    ...
    switch (type) {
      case TYPE_NONE: {
        dat->m_pSub = new KeyValues("");
        if (!dat->m_pSub->ReadAsBinaryPooledFormat(buffer, pFileSystem, poolKey,
                                                   pfnEvaluateSymbolProc))
          return false;
        break;
      }
      ...
      case TYPE_STRING: {
        ...
        if (!((IFileSystem *)pFileSystem)
                 ->GetStringFromKVPool(poolKey, stringKeyI, token,
                                       sizeof(token)))
          return false;
        intp len = V_strlen(token);
        dat->m_sValue = new char[len + 1];
      ...
      case TYPE_COMPILED_INT_0: {
        // only for dense storage purposes, flip back to preferred internal
        // format
        dat->m_iDataType = TYPE_INT;
        dat->m_iValue = 0;
        break;
    ...
    if (!buffer.GetBytesRemaining()) break;
...
void KeyValues::UnpackIntoStructure(
    KeyValuesUnpackStructure const *pUnpackTable, void *pDest) {
  ...
  while (pUnpackTable->m_pKeyName) {
    ...
    switch (pUnpackTable->m_eDataType) {
      ...
      case UNPACK_TYPE_STRING: {
        ...
        strncpy(dest_s, GetString(pUnpackTable->m_pKeyName, pDefault),
...
void KeyValues::MergeFrom(KeyValues *kvMerge,
                          MergeKeyValuesOp_t eOp /* = MERGE_KV_ALL */) {
  if (!kvMerge) return;
  ...
  switch (eOp) {
    ...
    case MERGE_KV_UPDATE: {
      for (KeyValues *sub = kvMerge->GetFirstTrueSubKey(); sub;
           sub = sub->GetNextTrueSubKey()) {
      ...
      for (KeyValues *val = kvMerge->GetFirstValue(); val;
...
static char const *ParseStringToken(char const *szStringVal,
                                    char const **ppEndOfParse) {
  // Eat whitespace
  while (V_isspace(*szStringVal)) ++szStringVal;

  char const *pszResult = szStringVal;

  while (*szStringVal && !V_isspace(*szStringVal)) ++szStringVal;
...
bool KeyValues::Dump(IKeyValuesDumpContext *pDump, int nIndentLevel /* = 0 */) {
  ...
  // Dump subkeys
  for (KeyValues *sub = GetFirstTrueSubKey(); sub;
       sub = sub->GetNextTrueSubKey()) {
    if (!sub->Dump(pDump, nIndentLevel + 1)) return false;
  }

  return pDump->KvEndKey(this, nIndentLevel);
}

bool IKeyValuesDumpContextAsText::KvBeginKey(KeyValues *pKey,
                                             int nIndentLevel) {
  if (pKey) {
...
bool IKeyValuesDumpContextAsText::KvWriteValue(KeyValues *val,
                                               int nIndentLevel) {
  if (!val) {
    return KvWriteIndent(nIndentLevel) && KvWriteText("<< NULL >>\n");
  }

  if (!KvWriteIndent(nIndentLevel)) return false;

  if (!KvWriteText(val->GetName())) return false;

  if (!KvWriteText(" ")) return false;

  switch (val->GetDataType()) {
    case KeyValues::TYPE_STRING: {
      if (!KvWriteText(val->GetString())) return false;
    } break;
    ...
    case KeyValues::TYPE_WSTRING: {
      ...
      V_snprintf(chBuffer, numBytes, "%ls [wstring, len = %zi]", wsz, nLen);
      if (!KvWriteText(chBuffer)) return false;
    } break;
    ...
    default:
      break;
#if 0  // this code was accidentally stubbed out by a mis-integration in
       // CL722860; it hasn't been tested
		{
...
bool CKeyValuesDumpContextAsDevMsg::KvBeginKey(KeyValues *pKey,
  ...
  if (r_developer.IsValid() && r_developer.GetInt() < m_nDeveloperLevel)
    // If "developer" is not the correct level, then avoid evaluating KeyValues
    // tree early
    return false;
  else
